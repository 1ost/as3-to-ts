...
export class AS3ByteArray {
    ...
    public writeBytes(bytes: AS3ByteArray, offset: number = 0, length: number = 0): void {
        ...
        const count = length === 0 ? bytes._length - sourceOffset : uint32(length, "writeBytes length");
        if (sourceOffset + count > bytes._length) throw new RangeError("writeBytes range exceeds source length");
    ...
    private _requireReadable(count: number): void {
        if (!Number.isSafeInteger(count) || count < 0 || this._position + count > this._length) {
            throw new RangeError("ByteArray read exceeds bytesAvailable");
        }
    }
