    ...
    if (node.kind === "NEW") {
        ...
        if (name === "Date" && context.className !== name && !context.locals[name] && !context.parameters[name]
            ...
            if(args.length!==0 && args.length!==1 && args.length!==4 && args.length!==6)
                fail("HARDENED_DATE_CONSTRUCTOR_ARITY","Date construction admits only zero arguments, one exact Number, four numeric, or six proven primitive calendar components",call);
            if(args.length===1) {
            ...
            if(args.length===4 && args.some(argument=>{
                const type=assignmentType(argument,context,call);
                return type.nullable || !["Number","int","uint"].includes(type.sourceName)
                    || type.emittedName!=="number";
            })) fail("HARDENED_DATE_CONSTRUCTOR_TYPE",
                "Four-component Date construction requires non-null numeric arguments",call);
