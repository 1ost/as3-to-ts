...
const intrinsicTypeNames = ['Number', 'int', 'uint', 'Boolean', 'Object', '*', 'String', 'Array'];
...
export function nativeSourceTypeIdentity(type: Node, qname: string, imports: string[]): string {
    ...
    if (name === qname.split('.').pop() && intrinsicTypeNames.indexOf(name) >= 0)
    ...
    if (intrinsicTypeNames.indexOf(name) >= 0) return name;
