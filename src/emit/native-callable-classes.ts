        ...
        cls.members.forEach((member: any) => {
            ...
            if (this.generated && [S.MethodDeclaration,S.GetAccessor,S.SetAccessor].indexOf(member.kind)>=0) {
                ...
                if(returns && returns.text !== 'void' && returns.text !== '*') {
                    ...
                    if(reference&&reference.kind==='native'&&reference.identity!=='flash.utils.Dictionary'&&reference.identity!=='flash.net.SharedObject'&&!(this.dateReference&&reference.identity==='Date')&&!(this.displayReference&&reference.identity==='flash.display.DisplayObject')&&!(reference.identity==='flash.events.Event'
