...
const evidence=path.join(engine,'tests/nativeFlashOracle','instance-constant-initialization');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-instance-constants');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Trace','Base','Derived','Values','Writer'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/constants',name+'.as'),'utf8');return ['constants.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-instance-constants',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),sources});
...
for(const declaration of ['public const bad:int=1+2;','public const bad:int;','public const bad:Object=null;','private const bad:int=3;','public const bad:int=01;','public const bad:String="a\\n";']){
 const source=sources['constants.Values'].source.replace('public const i:int=4.8;',declaration),changed={...sources,'constants.Values':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'constant-guard',providerModule:provider('AS3GeneratedClass'),sources:changed});
 assert.throws(()=>emit(parse('Values.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-instance-constants/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,10);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='write-own').value[1]=0]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
