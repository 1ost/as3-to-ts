...
const evidence=path.join(engine,'tests/nativeFlashOracle','dispatcher-construction');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-dispatcher');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='DispatcherProbe.as'){
...
const nativeProviders={
 'flash.events.EventDispatcher':{module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'EventDispatcher',nativeBase:'EventDispatcher'},
 'flash.events.Event':{module:provider('AS3CanonicalEventConstruction'),exportName:'Event',nativeBase:'Event'}
};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'dispatcher-construction',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
const guardSource='package {import flash.events.EventDispatcher;public class Guard extends EventDispatcher {public function Guard(){super();}}}';
for(const binding of [
 {module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'EventDispatcher'},
 {module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'Fake',nativeBase:'EventDispatcher'},
 {module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'EventDispatcher',nativeBase:'Event'},
]) {
 assert.throws(()=>api.createNativeGeneratedDeclarationPlan({scope:'guard',providerModule:provider('AS3GeneratedClass'),providers:{'flash.events.EventDispatcher':binding},sources:{Guard:{source:guardSource,sourceSha256:hash(guardSource)}}}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
for(const body of ['public function Guard(){}','public function Guard(){super();super();}','public function Guard(){if(true)super();}',
 'public function Guard(){super();}public var hasEventListener:Boolean;']) {
 const source='package {import flash.events.EventDispatcher;import flash.events.Event;public class Guard extends EventDispatcher {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'guard',providerModule:provider('AS3GeneratedClass'),providers:nativeProviders,sources:{Guard:{source,sourceSha256:hash(source)}}});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}),/AS3_.*UNSUPPORTED/,body);rejectionGuards++;
}
...
const program=modern.createProgram(files,{target:modern.ScriptTarget.ES2020,module:modern.ModuleKind.CommonJS,strict:true,strictNullChecks:false,experimentalDecorators:true,noEmit:true,skipLibCheck:true,types:[],lib:['lib.es2020.d.ts','lib.dom.d.ts','lib.dom.iterable.d.ts']});
...
const names=['AS3CanonicalEventDispatcherConstruction','AS3CanonicalEventConstruction','AS3CanonicalDisplayTypes','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
const moduleFor=n=>'src/layaAir/flash/'+(['AS3SourceError','IllegalOperationError'].includes(n)?'errors':'utils')+'/'+n;
const built=esbuild.buildSync({absWorkingDir:engine,stdin:{contents:fs.readFileSync(path.join(engine,'tests/nativeCanonicalDisplayAncestry/init-imports.ts'),'utf8').replaceAll('@laya/engine/','./src/layaAir/')+'\nexport {Laya} from "./src/layaAir/Laya";export {Shape} from "./src/layaAir/flash/display/Shape";export {Sprite} from "./src/layaAir/flash/display/Sprite";export {EventDispatcher} from "./src/layaAir/flash/events/EventDispatcher";export {IEventDispatcher} from "./src/layaAir/flash/events/IEventDispatcher";export {Rectangle} from "./src/layaAir/flash/geom/Rectangle";\n'+names.map(n=>'export * from "./'+moduleFor(n)+'";').join('\n'),resolveDir:engine,loader:'ts'},bundle:true,write:false,format:'cjs',platform:'browser',target:'es2020',loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},metafile:true});
...
const driverFile=path.resolve('tests/native-generated-dispatcher/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,10);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='self-fields').value[0]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
