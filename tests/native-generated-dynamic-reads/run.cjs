...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-dynamic-reads');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-dynamic-reads');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Reader','Record'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/reads',name+'.as'),'utf8');return ['reads.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-dynamic-reads',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['reads.Reader'],sources});
...
for(const expression of ['target[key]()','(target[key])()','delete target[key]','target[key]++','++target[key]','target[key]+=1']){
 const source=sources['reads.Reader'].source.replace('return target[key];','return '+expression+';'),changed={...sources,'reads.Reader':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'read-guard',providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['reads.Reader'],sources:changed});
 assert.throws(()=>emit(parse('Reader.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const module of ['', 'bad"module']){const source=sources['reads.Reader'].source;assert.throws(()=>emit(parse('Reader.as',source),source,{...options,nativeDynamicPropertyReadsModule:module}),/explicit common AS3Property module/);rejectionGuards++;}
...
const driverFile=path.resolve('tests/native-generated-dynamic-reads/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,15);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='ignored-catch').value[2]=true]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
