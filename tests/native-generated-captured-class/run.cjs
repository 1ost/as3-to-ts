...
const evidence=path.join(engine,'tests/nativeFlashOracle','captured-class-callback');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-captured-class');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['ClassCallbacks','First','Second'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/cases',name+'.as'),'utf8');return ['cases.'+name,{source,sourceSha256:hash(source)}];}));
const source=sources['cases.ClassCallbacks'].source;
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'captured-class-callback',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['cases.ClassCallbacks'],sources});
...
for(const key of ['nativeTypedLocalReferenceModule','nativeTypedLocalAdditionModule','nativeCallableCoercionModule','nativeCallableStringModule','nativeObjectCreationModule']){
 assert.throws(()=>emit(parse('ClassCallbacks.as',source),source,{...options,[key]:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
assert.throws(()=>emit(parse('ClassCallbacks.as',source),source,{...options,importModules:{...options.importModules,'compiler.AS3Invocation':undefined}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
const guarded=(changed,extra={})=>{
 const changedSources={...sources,'cases.ClassCallbacks':{source:changed,sourceSha256:hash(changed)}};
 const altered=api.createNativeGeneratedDeclarationPlan({scope:'captured-class-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['cases.ClassCallbacks'],sources:changedSources,...extra});
 assert.throws(()=>emit(parse('ClassCallbacks.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:altered,module:'./guard'},...(combined?{nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:altered,module:'./guard'}}:{})}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
};
guarded(source,{scriptGlobalSources:[]});
assert.ok(source.includes('public class ClassCallbacks'));
guarded(source.replace('public class ClassCallbacks {','public class ClassCallbacks { public static var initialization:* = null;'));
for(const [before,after] of [
 ['function(argument:*):*','function(argument:int):*'],
 ['function(argument:*):*','function(argument:*=null):*'],
 ['function(argument:*):*','function(...argument):*'],
 ['function(argument:*):*','function(argument:*):First'],
 ['return new selected(argument);','return function():* {return selected;};'],
 ['return new selected(argument);','var selected:*;return selected;'],
 ['return new selected(argument);','var local:int;return local;'],
 ['return new selected(argument);','return arguments;'],
 ['return new selected(argument);','return this.local(argument);'],
 ['return new selected(argument);','try {return selected;}catch(error:*){return null;}'],
 ['function(argument:*):* {return new selected(argument);}','function(selected:*):* {return new selected();}'],
 ['return value;','value=null;return value;']
]){assert.ok(source.includes(before));try{guarded(source.replace(before,after));}catch(e){console.error({before,after});throw e;}}
...
const driverFile=path.resolve('tests/native-generated-captured-class/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const lifecycleFile=path.resolve('tests/native-generated-captured-class/script-factory-driver.js'),lifecycle=fs.readFileSync(lifecycleFile,'utf8');
const wanted=captured;assert.equal(wanted.length,22);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='class-local-undefined').value[2]=true]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
try{for(const target of [ts.ScriptTarget.ES5,ts.ScriptTarget.ES2015]){
 ...
 const page=await browser.newPage();await page.addScriptTag({content:script});const web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.result)));const browserChecks=await page.evaluate(()=>globalThis.scriptFactoryChecks);assert.equal(browserChecks,globalThis.scriptFactoryChecks);await page.close();
 for(const actual of [node,web]){assert.deepEqual(actual,wanted);}
 assert.ok(globalThis.scriptFactoryChecks>40);
 results.push({target,node,web,scriptFactoryChecks:globalThis.scriptFactoryChecks});
}}finally{await browser.close();}
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,lifecycleObserver:{file:lifecycleFile,sha256:hash(lifecycle)},held:['Full error text; typed or nested anonymous signatures; anonymous local shadowing; native Class metadata and native subclass construction']},null,2));console.log(JSON.stringify({run,sourceClasses:3,airRows:wanted.length,rejectionGuards,scriptFactoryChecks:results.map(r=>r.scriptFactoryChecks),targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
