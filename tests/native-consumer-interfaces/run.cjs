...
const evidence=path.join(engine,'tests/nativeFlashOracle','consumer-interfaces');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-consumer-interfaces');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InterfaceConsumerProbe.as'){
...
for(const body of [
 'public var stored:IValue;',
 'public function Guard(v:IValue) {}',
 'public function get value():IValue {return null;}',
 'public function f(v:IValue,...rest):IValue {return v;}',
 'public function f(v:IValue=undefined):IValue {return v;}',
...
for(const declaration of [
 'public class Guard implements IValue {public function value():int {return 7;}}',
 'public class Guard extends Value {}'
]){
 const source='package contracts {'+declaration+'}';
 assert.throws(()=>emit(parse('Guard.as',source),source,consumerOptions),/AS3_REFERENCE_COERCION_UNSUPPORTED: source (?:interface implementation|reference ancestry) requires generated class registration/);rejectionGuards++;
}
...
for(const binding of Object.keys(sources).map(qname=>({qname}))){const source=sources[binding.qname].source,file=path.join(run,fileFor(binding.qname)+'.ts');fs.mkdirSync(path.dirname(file),{recursive:true});
 ...
 if(binding.qname==='contracts.Consumer')for(const key of ['nativeArrayCreationModule','nativeGeneratedDeclarations','nativeClassTraitsModule','nativeTypedLocals','nativeLexicalMembersModule','nativeGeneratedPropertyModule','nativeCallableMethodBindingModule','nativeCallableCoercionModule','nativeCallableStringModule','nativeTypedLocalReferenceModule'])delete opts[key];
 const output=emit(parse(binding.qname+'.as',source),source,opts);if(binding.qname==='contracts.Consumer'){assert.ok(!output.includes('declareNativeClass'));assert.ok(output.includes('as3CoerceReference')); }fs.writeFileSync(file,output);emitted.push({qname:binding.qname,file,sourceSha256:hash(source),outputSha256:hash(output)});
}
...
const driverFile=path.resolve('tests/native-consumer-interfaces/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,75);
