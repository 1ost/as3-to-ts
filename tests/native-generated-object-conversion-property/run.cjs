...
const evidence=path.join(engine,'tests/nativeFlashOracle/object-conversion-property'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-object-conversion-property');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['ObjectAccess'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const modules=['AS3Vector','AS3StringIntrinsics','Dictionary','AS3GeneratedClass','AS3ScriptGlobal','AS3Type','AS3Class','AS3Invocation','AS3LexicalMembers','AS3Property','AS3MethodBinding','AS3Coercion','AS3String','AS3Addition','AS3ArrayCreation','NativeSourceClassLoadingSession'];
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   for(const patch of [{nativeObjectCreationModule:undefined},{nativeObjectPropertyModule:'./wrong'},
    {importModules:{...options.importModules,'compiler.AS3Property':undefined}}]){
    assert.throws(()=>api.emitNativeSourceClassModule({...config,emitterOptions:{...options,...patch}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
   }
   for(const transform of [s=>s.replace('Object(value).flag=rhs','Object().flag=rhs'),s=>s.replace('Object(value).flag=rhs','Object(value,rhs).flag=rhs')]){
    const source=transform(sources.ObjectAccess.source);assert.notEqual(source,sources.ObjectAccess.source);
    const guardPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{ObjectAccess:{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeVectorTypes:{plan:guardPlan,module:'./__native_declarations'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
   }
   // A lexical Function named Object must not gain intrinsic conversion/dispatch.
   {
    const source='package { public class ObjectAccess { public function write(value:*,rhs:*,Object:Function):* { return Object(value).flag=rhs; } } }';
    const guardPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{ObjectAccess:{source,sourceSha256:hash(source)}}});
    const shadow=api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeVectorTypes:{plan:guardPlan,module:'./__native_declarations'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}});
    assert.ok(shadow.generatedSources.every(item=>!item.source.includes('as3SetProperty')&&!item.source.includes('as3CallClass')));rejectionGuards++;
   }
  ...
  const node=await execute(code);const compared=rows=>rows;assert.deepEqual(compared(node.rows),expected);
  ...
  const mutation=originalFactory.replace(/[A-Za-z_$][\w$]*\.as3CallClass\(Object, \[([^()[\]]+)\]\)/g,'($1)');assert.notEqual(mutation,originalFactory);
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Full ResourceLoader and game account flow']},null,2));
