...
const evidence=path.join(engine,'tests/nativeFlashOracle/optional-method-overrides'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-optional-overrides');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['optov.Base']),child:read('source',['optov.Base','optov.Child','optov.Grandchild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/child-factory\.js$/},args=>{
   ...
   if(mutation==='missing-signature'){
    ...
    const matches=[...contents.matchAll(re)];assert.equal(matches.length,3);
    ...
    assert.equal([...contents.matchAll(re)].length,3);contents=contents.replace(re,'undefined');
   }else if(mutation==='wrong-required-count'||mutation==='invalid-required-count'){
    const re=/(name: "range", parameters: \["int", "int"\], returns: "void", requiredCount: )0/g;
    assert.equal([...contents.matchAll(re)].length,3);
    contents=contents.replace(re,(_,prefix)=>prefix+(mutation==='wrong-required-count'?'1':'3'));
  ...
  const negatives=['missing-signature','missing-parent-selection','wrong-required-count','invalid-required-count'];
  for(const mutation of negatives){
   const mutated=(await build(mutation)).outputFiles[0].text;
   const expectedFailure=mutation==='missing-parent-selection'?/cohort did not reuse selected declaration: optov::Base/
    :mutation==='invalid-required-count'?/AS3_GENERATED_CLASS_UNSUPPORTED: invalid method required count/
    :/AS3_GENERATED_CLASS_UNSUPPORTED: selected parent override/;
   await assert.rejects(()=>execute(mutated),expectedFailure);
  }
