...
const castTarget=process.argv.includes('--cast');
const evidence=path.join(engine,'tests/nativeFlashOracle',castTarget?'class-cast-construction':'dynamic-construction');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-dynamic-construction');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const plan=api.createNativeGeneratedDeclarationPlan({scope:'dynamic-construction',interfaceProviderModule:provider('AS3Type'),providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
for(const changed of [{...options,nativeDynamicConstructionModule:undefined},
 {...options,nativeDynamicConstructionModule:'./wrong'}, {...options,useNamespaces:true}]) {
 assert.throws(()=>emit(parse('Reader.as',source),source,changed),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}

const noArgsSource=source.replace(castTarget?'new (value as Class)()':'new value()',castTarget?'new (value as Class)':'new value');
const noArgsPlan=api.createNativeGeneratedDeclarationPlan({scope:'construction-no-args',interfaceProviderModule:provider('AS3Type'),providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{...sources,'cases.Reader':{source:noArgsSource,sourceSha256:hash(noArgsSource)}}});
assert.throws(()=>emit(parse('Reader.as',noArgsSource),noArgsSource,{...options,nativeGeneratedDeclarations:{plan:noArgsPlan,module:'./noArgs'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:noArgsPlan,module:'./noArgs'}}),/AS3_DYNAMIC_CONSTRUCTION_UNSUPPORTED/);rejectionGuards++;
const emitted=[];
...
const driverFile=path.resolve('tests/native-dynamic-construction/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,31);
