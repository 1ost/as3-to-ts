...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-class-predicates');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-class-predicates');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Reader','Subject','Contract'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/cases',name+'.as'),'utf8');return ['cases.'+name,{source,sourceSha256:hash(source)}];}));
...
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-class-predicates',interfaceProviderModule:provider('AS3Type'),providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
for(const changed of [{...options,nativeClassTypeOperationsModule:undefined},
 {...options,nativeClassTypeOperationsModule:'./wrong'}, {...options,useNamespaces:true}]) {
 assert.throws(()=>emit(parse('Reader.as',source),source,changed),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
const changedSource=source.replace('check(value:Object)','check(value:Object,Class:*)');
const changedSources={...sources,'cases.Reader':{source:changedSource,sourceSha256:hash(changedSource)}};
const shadowPlan=api.createNativeGeneratedDeclarationPlan({scope:'class-shadow',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources:changedSources});
assert.throws(()=>emit(parse('Reader.as',changedSource),changedSource,{...options,nativeGeneratedDeclarations:{plan:shadowPlan,module:'./shadow'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:shadowPlan,module:'./shadow'}}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
...
const driverFile=path.resolve('tests/native-class-predicates/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,23);
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({consumerOnly,combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,hostControls:4,comparisonNegativeControls:3,held:['Dynamic Object construction; full UIComponent']},null,2));console.log(JSON.stringify({run,consumerOnly,sourceClasses:2,interfaces:1,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
