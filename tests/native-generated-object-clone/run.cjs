...
const evidence=path.join(engine,'tests/nativeFlashOracle','native-object-clone');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-object-clone');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Base','Record'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/clonecase',name+'.as'),'utf8');return ['clonecase.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-object-clone',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
for(const binding of [...plan.bindings,...plan.interfaces]){const source=sources[binding.qname].source,file=path.join(run,fileFor(binding.qname)+'.ts');fs.mkdirSync(path.dirname(file),{recursive:true});
 ...
 let output;try{output=emit(parse(binding.qname+'.as',source),source,opts);}catch(error){
 const report={status:'held',qname:binding.qname,error:String(error),emitted,sourceSha256:hash(source),scope:'Complete original source; no trimmed constants or dynamic members'};
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify(report,null,2));console.error(JSON.stringify({run,...report}));process.exit(1);
 }
fs.writeFileSync(file,output);emitted.push({qname:binding.qname,file,sourceSha256:hash(source),outputSha256:hash(output)});
}
...
const names=['ClassAlias','NativeObjectCodec','AS3Date','AS3JSON','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
const moduleFor=n=>'src/layaAir/flash/'+(n==='ClassAlias'?'net':['AS3SourceError','IllegalOperationError'].includes(n)?'errors':'utils')+'/'+n;
...
const driverFile=path.resolve('tests/native-generated-object-clone/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,12);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='date-0').value[2]=0]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
try{for(const target of [ts.ScriptTarget.ES5,ts.ScriptTarget.ES2015]){
 ...
 fs.writeFileSync(path.join(run,'bundle-'+target+'.js'),script);const node=JSON.parse(JSON.stringify(new Function(script+';return {rows:globalThis.result,guards:globalThis.cloneGuards};')()));
 const page=await browser.newPage();await page.addScriptTag({content:script});const web=await page.evaluate(()=>JSON.parse(JSON.stringify({rows:globalThis.result,guards:globalThis.cloneGuards})));await page.close();
 for(const actual of [node,web]){assert.deepEqual(actual.rows,wanted);assert.equal(actual.guards,5);}
 results.push({target,node,web});
}}finally{await browser.close();}
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,runtimeGuards:5,comparisonNegativeControls:3,held:['Registered aliases, Date prototypes and complex getter order']},null,2));console.log(JSON.stringify({run,sourceClasses:2,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
