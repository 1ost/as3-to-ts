...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-avmplus-name');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-avmplus-name');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const driverFile=path.resolve('tests/native-generated-avmplus-name/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},comparisonNegativeControls:3,held:['Full reflection and untested AVMPlus APIs','Complete Debug and Game integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:wanted.length,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
