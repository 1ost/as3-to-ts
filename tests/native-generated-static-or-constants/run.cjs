...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-static-or-constants');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-static-or-constants');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='OrConstantsProbe.as'){
...
for(const body of [
 'public static var A:uint=1;public static const B:uint=A|A;',
 'public static const A:uint=Flags.ALL|Flags.DEBUG;',
 'public static const A:uint=B|B;public static const B:uint=A|A;',
 'public static const A:uint=1;public static const B:uint=A&A;',
 'public static const A:uint=1;public static const B:uint=(A|A);',
 'public static const A:int=1;public static const B:uint=A|A;',
 'public static const A:uint=1;public static const B:uint=A|2;',
 'public static const A:uint=4294967296;public static const B:uint=A|A;'
]){
 const source='package orcases {public class Guard {'+body+'}}';
 assert.throws(()=>{const p=api.createNativeGeneratedDeclarationPlan({scope:'or-guard',providerModule:provider('AS3GeneratedClass'),sources:{...sources,'orcases.Guard':{source,sourceSha256:hash(source)}}});emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard-domain'},nativeReferenceCoercion:{plan:p,module:'./guard-domain',coercionModule:provider('AS3Type')}});},/AS3_[A-Z_]+UNSUPPORTED/,body);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-static-or-constants/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,4);
