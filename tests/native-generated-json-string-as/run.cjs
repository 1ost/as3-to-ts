...
const evidence=path.join(engine,'tests/nativeFlashOracle/json-string-as'),air=require(path.join(evidence,'verify.cjs'));
...
const cache=path.resolve('.cache/native-generated-json-string-as');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['jsonas.Parser'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const options={customVisitors:[],definitionsByNamespace,nativeGlobalModules:{trace},nativeVectorTypes:{plan,module:'./__native_declarations'},nativeStringIntrinsicsModule:provider('AS3StringIntrinsics'),nativeStringLocalCoercionModule:provider('AS3String'),nativeEnumeration:{dictionaryModule:provider('Dictionary'),coercionModule:provider('AS3Coercion'),stringModule:provider('AS3String')},importModules:{...Object.fromEntries(Object.entries(nativeProviders).map(([q,p])=>[q,p.module])),'flash.utils.describeType':provider('describeType'),'compiler.AS3Property':provider('AS3Property'),'compiler.AS3Class':provider('AS3Class'),'compiler.AS3Invocation':provider('AS3Invocation')},
    ...
    nativeJSONModule:provider('AS3JSON'),nativeObjectPropertyModule:provider('AS3Property'),nativeTypedLocals:true,nativeTypedLocalReferenceModule:provider('AS3Type'),nativeTypedLocalAdditionModule:provider('AS3Addition'),nativeArrayCreationModule:provider('AS3ArrayCreation'),
   ...
   for(const transform of [s=>s.replace('JSON.parse(value as String)','JSON.parse(value)'),s=>s.replace('parse(value:*)','parse(value:*,String:Class)')]){
    const source=transform(sources['jsonas.Parser'].source),guardPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{'jsonas.Parser':{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeVectorTypes:{plan:guardPlan,module:'./__native_declarations'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}}),/AS3_JSON_UNSUPPORTED/);rejectionGuards++;
   }
   assert.throws(()=>api.emitNativeSourceClassModule({...config,emitterOptions:{...options,nativeComputedTypeTestModule:undefined}}),/AS3_JSON_UNSUPPORTED/);rejectionGuards++;
