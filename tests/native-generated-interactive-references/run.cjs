...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-interactive-references');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-interactive-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InteractiveReferenceProbe.as'){
...
const consumerOnly=process.argv.includes('--consumer');if(consumerOnly)sources['interactivecases.Reader'].referenceOnly=true;
...
const options={customVisitors:[],nativeInteractiveObjectReferenceModule:provider('AS3CanonicalInteractiveReference'),importModules:{"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
const reader=sources['interactivecases.Reader'].source;
for(const patch of [
 {nativeInteractiveObjectReferenceModule:undefined},
 {nativeReferenceCoercion:undefined},

 {nativeInteractiveObjectReferenceModule:'./wrong-provider'},
 {importModules:{...options.importModules,'flash.display.InteractiveObject':'./wrong-provider'}},
...
for(const body of [
 'public static var value:Boolean=null is InteractiveObject;',
 'public static var value:InteractiveObject=null as InteractiveObject;',
 'public function test(value:*,InteractiveObject:Class):Boolean{return value is InteractiveObject;}',
 'public function test(value:*,InteractiveObject:*):*{return value as InteractiveObject;}'
]) {
 const source='package interactivecases {import flash.display.InteractiveObject;public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'display-reference-guard',providerModule:provider('AS3GeneratedClass'),sources:{'interactivecases.Guard':{source,sourceSha256:hash(source),...(consumerOnly?{referenceOnly:true}:{})}},providers:nativeProviders});
...
for(const binding of Object.keys(sources).map(qname=>({qname}))){const source=sources[binding.qname].source,file=path.join(run,fileFor(binding.qname)+'.ts');fs.mkdirSync(path.dirname(file),{recursive:true});
 const opts=consumerOnly&&binding.qname==='interactivecases.Reader'?consumerOptions(options):{...options};
...
const program=modern.createProgram(files,{target:modern.ScriptTarget.ES2020,module:modern.ModuleKind.CommonJS,strict:true,strictNullChecks:false,experimentalDecorators:true,noEmit:true,skipLibCheck:true,resolveJsonModule:true,esModuleInterop:true,lib:['lib.es2020.d.ts','lib.dom.d.ts','lib.dom.iterable.d.ts']});
...
const driverFile=path.resolve('tests/native-generated-interactive-references/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,36);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='identity-0').value[0]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
