...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-internal-methods');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-internal-methods');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InternalMethodProbe.as'){
...
walk(path.join(evidence,'source'));assert.equal(Object.keys(sources).length,7);
...
const holder=sources['methodcases.Holder'].source;
rejected(holder,'methodcases.Holder',false);
for(const changed of [
 holder.replace('internal function parse(', 'internal static function parse('),
 holder.replace('internal function parse(input:IInput):Boolean', 'internal function parse(input:Object):Boolean'),
 holder.replace('internal function parse(input:IInput):Boolean', 'internal function parse(input:IInput):uint'),
 holder.replace('internal function parse(input:IInput):Boolean', 'internal function parse(input:IInput=null):Boolean'),
 holder.replace('internal function parse(input:IInput):Boolean', 'internal function parse(input:IInput,other:IInput):Boolean'),
 holder.replace('internal function parse(input:IInput):Boolean', 'internal function parse(...input):Boolean'),
 holder.replace('internal function parse(', 'customNS function parse('),
 holder.replace('return parse(input);','return delete parse;'),
 holder.replace('return parse(input);','return parse=null;'),
 holder.replace('return parse(input);','return ++parse;'),
 holder.replace('internal function parse(', 'override internal function parse(')
])rejected(changed);
const peer=sources['methodcases.Peer'].source;
for(const expression of ['value.parse=null','delete value.parse','++value.parse'])
 rejected(peer.replace('return value.parse(input);','return '+expression+';'),'methodcases.Peer');
const child=sources['methodcases.Child'].source;
rejected(child.replace('override internal','internal'),'methodcases.Child');
rejected(child.replace('return input.read()<0;','return super.parse(input);'),'methodcases.Child');
rejected(child.replace('):Boolean','):void'),'methodcases.Child');
rejected('package othermethod {import methodcases.Holder; import methodcases.IInput; public class Bad {public function read(value:Holder,input:IInput):Boolean{return value.parse(input);}}}','othermethod.Bad');
...
const driverFile=path.resolve('tests/native-generated-internal-methods/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
