...
commands=[['java','-Xmx384m','-jar',str(flex/'lib/mxmlc.jar'),'+flexlib='+str(flex/'frameworks'),'-target-player=26.0','-swf-version=37','-source-path='+str(here/'order-original'),'-output='+str(out/'oracle.swf'),str(here/'order-original/ConstructorOracle.as')],
...
inputs={str(p.relative_to(here)).replace('order-original','original'):p for p in [*sorted((here/'order-original').rglob('*.as')),here/'capture.cjs',Path(__file__)]}
