...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-wildcard-forin');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-wildcard-forin');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Enumeration','Sealed'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/cases',name+'.as'),'utf8');return ['cases.'+name,{source,sourceSha256:hash(source)}];}));
const source=sources['cases.Enumeration'].source;
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-wildcard-forin',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['cases.Enumeration'],sources});
...
for(const key of ['nativeEnumeration','nativeDictionaryPropertyModule']){
 assert.throws(()=>emit(parse('Enumeration.as',source),source,{...options,[key]:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const [before,after] of [
 ['var key:*;var out:Array=[];','var key:String;var out:Array=[];'],
 ['var key:*;var out:Array=[];','var out:Array=[];'],
 ['for(key in value){out.push([typeof key,key]);}','for(value.key in value){out.push([typeof key,key]);}'],
 ['for(key in value){out.push([typeof key,key]);}','try{throw null;}catch(key:*){for(key in value){out.push(key);}}'],
 ['for(key in value){out.push([typeof key,key]);}','for(var key2:* in value){out.push(key2);}']
]){
 assert.ok(source.includes(before));const changed=source.replace(before,after),changedSources={...sources,'cases.Enumeration':{source:changed,sourceSha256:hash(changed)}};
 const altered=api.createNativeGeneratedDeclarationPlan({scope:'forin-guard',providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['cases.Enumeration'],sources:changedSources});
 assert.throws(()=>emit(parse('Enumeration.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:altered,module:'./guard'},...(combined?{nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:altered,module:'./guard'}}:{})}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-wildcard-forin/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,17);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='last-numeric').value[0]='string']){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
