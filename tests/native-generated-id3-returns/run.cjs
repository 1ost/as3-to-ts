...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-id3-returns');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-id3-returns');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&['ID3Returns.as'].includes(item.name)){
...
const source=sources['returncases.ID3Returns'].source;
for(const changed of [source.replace('return raw;', 'return;'),source.replace('return raw;', 'if (raw) return raw;')]) {
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'id3-return-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'returncases.ID3Returns':{source:changed,sourceSha256:hash(changed)}}});
  emit(parse('ID3Returns.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
const unbound=api.createNativeGeneratedDeclarationPlan({scope:'id3-return-unbound',providerModule:provider('AS3GeneratedClass'),sources});
assert.throws(()=>emit(parse('ID3Returns.as',source),source,{...options,nativeGeneratedDeclarations:{plan:unbound,module:'./unbound'},nativeReferenceCoercion:{plan:unbound,module:'./unbound',coercionModule:provider('AS3Type')}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
const other=source.replaceAll('flash.media.ID3Info','flash.errors.IOError').replaceAll('ID3Info','IOError');
const otherPlan=api.createNativeGeneratedDeclarationPlan({scope:'other-native-return',providers:{'flash.errors.IOError':{module:modulePath(path.join(engine,'src/layaAir/flash/errors/IOError.ts')),exportName:'IOError'}},providerModule:provider('AS3GeneratedClass'),sources:{'returncases.ID3Returns':{source:other,sourceSha256:hash(other)}}});
assert.throws(()=>emit(parse('ID3Returns.as',other),other,{...options,nativeGeneratedDeclarations:{plan:otherPlan,module:'./other'},nativeReferenceCoercion:{plan:otherPlan,module:'./other',coercionModule:provider('AS3Type')}}),/generated native return type requires separate qualification/);rejectionGuards++;
...
const names=['ID3Info','AS3CanonicalErrorConstruction','trace','Dictionary','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
const moduleFor=n=>'src/layaAir/flash/'+(n==='ID3Info'?'media':n==='trace'?'debug':['IOError','AS3SourceError','IllegalOperationError'].includes(n)?'errors':'utils')+'/'+n;
...
const driverFile=path.resolve('tests/native-generated-id3-returns/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Other native return types, ID3Info native member dispatch and generated native subclasses','Complete SoundPlayer and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:26,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
