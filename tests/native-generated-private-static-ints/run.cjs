...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-private-static-ints');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-private-static-ints');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='PrivateStaticIntsProbe.as'){
...
for(const body of ['private static const value:int=2147483648;','private static const value:int=-2147483649;','private static const value:int=1+1;','private static const value:int=1.5;','private static const value:int=7;public static function change():void{value=8;}','private static const value:String=String(7);','private static const value:String;','private static const value:String="a";public static function change():void{value="b";}','private const value:String="hidden";','protected const value:Number=7;','protected static const value:int=7;','protected const value:String=String(7);','protected const value:int=1+1;','protected const value:int=2147483648;','protected const value:Object=null;','protected const value:String;','protected const value:Vector.<int>;','protected static var value:String=String(7);','protected const value:String="a";public function change():void{value="b";}']){
...
const driverFile=path.resolve('tests/native-generated-private-static-ints/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,11);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='write').value[2]=1]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
