...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-private-static-ints'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-private-static-int-factories');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['constantcases.PrivateConstants']),child:read('source',['constantcases.PrivateConstants','constantcases.ChildConstants','constantcases.ValueSpy'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const input={scope:'private-static-ints-'+cohort,sources,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['constantcases.PrivateConstants']};
   ...
   let compilerGuards=0;
   for(const body of ['private static var STATE:int=1;','public static var STATE:int=1;',
    'private static const STATE:int=1+1;','private static const STATE:int=2147483648;',
    'protected static const STATE:int=1;','private static const STATE:int=1.5;']){
    const source='package constantcases {public class Guard {'+body+'}}';
    const guardPlan=api.createNativeGeneratedDeclarationPlan({...input,classScriptSources:undefined,sources:{'constantcases.Guard':{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}}),/AS3_[A-Z_]+UNSUPPORTED/,body);compilerGuards++;
   }
   assert.equal(compilerGuards,6);
  ...
  results.push({target,node,web,typechecks,artifacts,compilerGuards:6,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
