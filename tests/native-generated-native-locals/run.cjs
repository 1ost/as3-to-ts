...
const evidence=path.join(engine,'tests/nativeFlashOracle','native-reference-locals');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-native-locals');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=fs.readFileSync(path.join(evidence,'source/cases/NativeLocals.as'),'utf8');
const sources={'cases.NativeLocals':{source,sourceSha256:hash(source)}};
const nativeProviders={'flash.utils.ByteArray':{module:provider('ByteArray'),exportName:'ByteArray'}};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'native-reference-locals',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources});
...
const options={customVisitors:[],importModules:{...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
for(const key of ['nativeTypedLocalReferenceModule','nativeTypedLocalAdditionModule','nativeCallableCoercionModule','nativeCallableStringModule']){
 assert.throws(()=>emit(parse('NativeLocals.as',source),source,{...options,[key]:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
assert.throws(()=>emit(parse('NativeLocals.as',source),source,{...options,nativeTypedLocals:false}),/typed local initialization/);rejectionGuards++;
assert.throws(()=>{const missing=api.createNativeGeneratedDeclarationPlan({scope:'missing-provider',providerModule:provider('AS3GeneratedClass'),sources});emit(parse('NativeLocals.as',source),source,{...options,nativeGeneratedDeclarations:{plan:missing,module:'./missing'}});},/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
...
// Existing engine host declarations are recorded exactly; do not hide new dependency errors.
assert.deepEqual(diagnostics.map(d=>[d.file.replaceAll('\\','/').split('/').pop(),d.code,d.text]),[
 ['ByteArray.ts',7017,"Element implicitly has an 'any' type because type 'typeof globalThis' has no index signature."],
 ['ByteArray.ts',2304,"Cannot find name 'DecompressionStream'."],
 ['WebGLInternalRT.ts',2307,"Cannot find module 'console' or its corresponding type declarations."]
]);
const names=['AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass','ByteArray'];
...
const driverFile=path.resolve('tests/native-generated-native-locals/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,20);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='assign-undefined-raw').value[1]=true]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Native provider dependency type diagnostics remain recorded','Full error text, native Class metadata and native subclass construction']},null,2));console.log(JSON.stringify({run,sourceClasses:1,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
