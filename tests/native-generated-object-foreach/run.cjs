...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-object-foreach');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-object-foreach');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name==='Enumeration.as'){
...
const source=sources['objecteach.Enumeration'].source;
...
for(const changed of [
 source.replace('var value:Object="seed"','var value:int=0'),
 source.replace('var value:Object="seed"','var value:Array=null'),
 source.replace('values(input:*):Array','values(input:*,value:Object):Array').replace('var value:Object="seed";',''),
 source.replace('for each(value in input)','for each(input.text in input)'),
 source.replace('for each(value in input)','for each(var inline:Object in input)'),
 source.replace('for each(value in input){result.push([typeof value,value]);}','try{throw null;}catch(value:*){for each(value in input){result.push(value);}}')
]){
 assert.notEqual(changed,source,'guard must mutate its source');
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'object-foreach-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'objecteach.Enumeration':{source:changed,sourceSha256:hash(changed)}}});
  emit(parse('Enumeration.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
...
const driverFile=path.resolve('tests/native-generated-object-foreach/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,14);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='array').value[0][1]='wrong']){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
