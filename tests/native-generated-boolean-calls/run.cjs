...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-boolean-calls');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-boolean-calls');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&['Convert.as','Shadow.as'].includes(item.name)){
...
const source=sources['booleanops.Convert'].source;
assert.throws(()=>emit(parse('Convert.as',source),source,{...options,nativeObjectCreationModule:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
for(const changed of [source.replace('Boolean(value)','Boolean()'),source.replace('Boolean(value)','Boolean(value,value)')]){
 assert.throws(()=>{
  const guardSources={...sources,'booleanops.Convert':{source:changed,sourceSha256:hash(changed)}};
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'boolean-call-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:guardSources});
  emit(parse('Convert.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_BOOLEAN_CALL_UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-boolean-calls/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
