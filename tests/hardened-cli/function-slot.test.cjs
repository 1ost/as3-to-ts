...
test("Function slots preserve original source and deterministic emission",t=>{
 assert.ok(air&&laya&&ffdec,"AIR, Laya and FFDec fixture paths required");
 const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),"function-slot-")));
 t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));
 const source=path.join(dir,"source"),fixture=path.join(laya,"tests/nativeFlashOracle/function-slot");fs.mkdirSync(source);
 ...
 const makeProfile=()=>run("python3",["-B","tools/create-fixture-profile.py","--source",source,"--entry","FunctionSlotProbe","--air-sdk",air,"--laya",laya,"--ffdec-jar",ffdec,"--output",profile]);
 ...
 for(const output of ['first','second']) {
  ...
  const code=fs.readFileSync(path.join(dir,output,'__as3_runtime/application/FunctionSlotProbe.ts'),'utf8');
  assert.match(code,/__as3FunctionArgument\(value, "Function"\)/);
