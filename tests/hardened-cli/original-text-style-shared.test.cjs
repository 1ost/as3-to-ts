...
test('Original TextStyle and VectorUtilities run unchanged through shared providers',async()=>{
 assert.ok(laya&&process.env.HARDENED_FIXTURE_AIR_SDK&&process.env.HARDENED_FIXTURE_FFDEC,'AIR, Laya and FFDec required');
 const evidence=path.join(laya,'tests/nativeFlashOracle/original-text-style'),read=name=>fs.readFileSync(path.join(evidence,name));
 for(const [file,expected] of Object.entries(JSON.parse(read('shared-evidence-pin.json'))))assert.equal(hash(read(file)),expected);
 const receipt=JSON.parse(read('shared-native-receipt.json')),captured=JSON.parse(read('shared-native-capture.json'));
 ...
 assert.equal(hash(read('OriginalTextStyleProbe.as')),receipt.artifacts['source/OriginalTextStyleProbe.as']);
 assert.equal(hash(read('scenario.json')),receipt.scenario.sha256);
 for(const run of [1,2])assert.equal(hash(read('shared-native-capture.json')),receipt.artifacts[`run-${run}/capture.json`]);
 const base=path.join(root,'.cache/original-text-style');fs.mkdirSync(base,{recursive:true});
 ...
 const source=path.join(dir,'source');fs.mkdirSync(source);for(const file of ['OriginalTextStyleProbe.as','util/TextStyle.as','util/VectorUtilities.as']){fs.mkdirSync(path.dirname(path.join(source,file)),{recursive:true});fs.writeFileSync(path.join(source,file),read(file));assert.equal(hash(read(file)),receipt.artifacts['source/'+file]);}
 const make=()=>run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','OriginalTextStyleProbe',
  '--air-sdk',process.env.HARDENED_FIXTURE_AIR_SDK,'--laya',laya,'--ffdec-jar',process.env.HARDENED_FIXTURE_FFDEC,'--native-date','--shared-date','--native-regexp','--native-regexp-members','--shared-string-ranges','--output',profile]);
...
globalThis.originalTextStyle=${JSON.stringify(steps)}.map(step=>{for(const call of step.calls)probe[call.method](...call.args);return {id:step.id,result:probe.result};});`;
 const manifest=JSON.parse(fs.readFileSync(path.join(dir,'emitted/manifest.json')));
 assert.equal(manifest.files.length,3);for(const row of manifest.files)assert.ok(row.typescriptPath,JSON.stringify(row));
 ...
 const node=JSON.parse(JSON.stringify(new Function(bundle+';return globalThis.originalTextStyle;')()));assert.deepEqual(node,wanted);
 const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||'playwright'),browser=await chromium.launch({headless:true});
 let web;try{const page=await browser.newPage({timezoneId:'Europe/Rome'});await page.addScriptTag({content:bundle});web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.originalTextStyle)));assert.deepEqual(web,wanted);}finally{await browser.close();}
 const canonical=value=>value===null||typeof value!=='object'?JSON.stringify(value):Array.isArray(value)?'['+value.map(canonical).join(',')+']':'{'+Object.keys(value).sort().map(key=>JSON.stringify(key)+':'+canonical(value[key])).join(',')+'}';
 const lockPath=path.join(profile,'profile-lock.json'),lock=JSON.parse(fs.readFileSync(lockPath)),proofPath=path.join(profile,lock.files.stringPatternProvider.path),proof=JSON.parse(fs.readFileSync(proofPath));
 const savedProof=fs.readFileSync(proofPath),savedLock=fs.readFileSync(lockPath);
 try {
 assert.equal(proof.schema,'as3-string-pattern-provider-target@3');proof.schema='as3-string-pattern-provider-target@2';proof.targets=proof.targets.filter(row=>row.export!=='as3RegExpReceiver');
 fs.writeFileSync(proofPath,canonical(proof)+'\n');lock.files.stringPatternProvider.sha256=hash(fs.readFileSync(proofPath));fs.writeFileSync(lockPath,canonical(lock)+'\n');
 run(process.execPath,['bin/as3-frontend','qualify',...args('without-regexp-members')]);
 const held=JSON.parse(fs.readFileSync(path.join(dir,'without-regexp-members/manifest.json'))).files.find(row=>row.sourcePath==='OriginalTextStyleProbe.as');assert.equal(held.status,'held');assert.equal(held.code,'HARDENED_MEMBER_TARGET');
 } finally { fs.writeFileSync(proofPath,savedProof);fs.writeFileSync(lockPath,savedLock); }
 assert.equal(hash(fs.readFileSync(path.join(source,'OriginalTextStyleProbe.as'))),hash(read('OriginalTextStyleProbe.as')));
 fs.writeFileSync(path.join(dir,'report.json'),JSON.stringify({rows:6,node,web,sourceUnchanged:true,applicationStart:true,originalClasses:2,negativeControls:['RegExp member authority downgrade held'],typecheck:{strict:true,strictNullChecks:false,diagnostics:0}},null,2)+'\n');
