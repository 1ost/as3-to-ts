...
test("Declared enumeration preserves native fixture bytes and deterministic slot initialization",t=>{
 assert.ok(air&&laya&&ffdec,"AIR, Laya and FFDec fixture paths required");
 const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),"declared-enumeration-")));
 t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));
 const source=path.join(dir,"source"),fixture=path.join(laya,"tests/nativeFlashOracle/declared-enumeration");fs.mkdirSync(source);
 ...
 const makeProfile=()=>run("python3",["-B","tools/create-fixture-profile.py","--source",source,"--entry","DeclaredEnumerationProbe","--air-sdk",air,"--laya",laya,"--ffdec-jar",ffdec,"--output",profile,"--intrinsic-type","flash.utils.Dictionary"]);
 ...
 for(const output of ['first','second']) {
  ...
  const code=fs.readFileSync(path.join(dir,output,'__as3_runtime/application/DeclaredEnumerationProbe.ts'),'utf8');
  assert.match(code,/var item: unknown = null;/);
  assert.match(code,/\bintItem: number = 0,/);
  assert.match(code,/\bnumberItem: number = 0 \/ 0,/);
  assert.match(code,/__as3ArrayValues\(values, "Object"\)/);
  assert.match(code,/__as3ReferenceValues/);
