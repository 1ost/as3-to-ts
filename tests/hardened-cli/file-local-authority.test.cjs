...
const { loadLocalMemberAuthority } = load(path.join(ROOT, "src/hardened/local-members.ts"));

test("generated file-local authority stays scoped, deterministic and authenticated", t => {
    ...
    const entries = ["first", "second"].map((packageName, index) => {
        ...
        const source = `package ${packageName} { public class Owner {} }
