...
test('Array comparator sorting retains native invocation order, mutations and sparse ownership',async()=>{
 assert.ok(laya&&process.env.HARDENED_FIXTURE_AIR_SDK&&process.env.HARDENED_FIXTURE_FFDEC,'AIR, Laya and FFDec required');
 const evidence=path.join(laya,'tests/nativeFlashOracle/array-sort-callback'),read=name=>fs.readFileSync(path.join(evidence,name));
 ...
 assert.equal(receipt.capture.observationCount,16);assert.equal(captured.runtime.version,'MAC 51,3,3,2');
 assert.equal(hash(read('source/ArraySortCallbackProbe.as')),receipt.artifacts['source/ArraySortCallbackProbe.as']);
 ...
 const base=path.join(root,'.cache/array-sort-callback');fs.mkdirSync(base,{recursive:true});
 ...
 const source=path.join(dir,'source');fs.mkdirSync(source);fs.writeFileSync(path.join(source,'ArraySortCallbackProbe.as'),read('source/ArraySortCallbackProbe.as'));
 const make=()=>run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','ArraySortCallbackProbe',
...
globalThis.arraySortCallback=${JSON.stringify(steps)}.map(step=>{for(const call of step.calls)probe[call.method](...call.args);return {id:step.id,result:probe.result};});`;
 const manifest=JSON.parse(fs.readFileSync(path.join(dir,'emitted/manifest.json')));
 const applicationSource=fs.readFileSync(path.join(dir,'emitted',manifest.files.find(row=>row.sourcePath==='ArraySortCallbackProbe.as').typescriptPath),'utf8');
 assert.match(applicationSource,/laya\/flash\/utils\/AS3ArraySort/);assert.doesNotMatch(applicationSource,/@bleach\/as3-runtime\/AS3ArraySort/);
 const typeConfig=path.join(dir,'generated-tsconfig.json');
 fs.writeFileSync(typeConfig,JSON.stringify({compilerOptions:{target:'ES2020',module:'CommonJS',moduleResolution:'node',strict:true,skipLibCheck:true,noEmit:true,types:[],lib:['ES2020','DOM'],baseUrl:root,paths:{'@laya/as3-runtime/*':['src/hardened-runtime/*'],'laya/*':[path.join(laya,'src/layaAir/*')]}},files:[path.join(dir,'emitted',manifest.files.find(row=>row.sourcePath==='ArraySortCallbackProbe.as').typescriptPath)]}));
 ...
 const node=JSON.parse(JSON.stringify(new Function(bundle+';return globalThis.arraySortCallback;')()));assert.deepEqual(node,wanted);
 const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||'playwright'),browser=await chromium.launch({headless:true});
 let web;try{const page=await browser.newPage({timezoneId:'Europe/Rome'});await page.addScriptTag({content:bundle});web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.arraySortCallback)));assert.deepEqual(web,wanted);}finally{await browser.close();}
 // Removing the verified provider must hold the newly requested shared Array comparator surface.
 ...
 const originalLock=fs.readFileSync(lockPath);delete lock.files.arraySortProvider;
 fs.writeFileSync(lockPath,canonical(lock)+"\n");run(process.execPath,['bin/as3-frontend','qualify',...args('legacy')]);
 const legacy=JSON.parse(fs.readFileSync(path.join(dir,'legacy/manifest.json'))).files.find(row=>row.sourcePath==='ArraySortCallbackProbe.as');
 assert.equal(legacy.status,'held');assert.equal(legacy.code,'HARDENED_ARRAY_SORT');fs.writeFileSync(lockPath,originalLock);
 // A changed provider source closure is rejected even when an attacker rehashes the profile file.
 const providerFile=path.join(profile,JSON.parse(originalLock).files.arraySortProvider.path),proof=JSON.parse(fs.readFileSync(providerFile));
 const savedProof=fs.readFileSync(providerFile);
 try {
 proof.targetSources['src/layaAir/flash/utils/AS3ArraySort.ts']='0'.repeat(64);
 fs.writeFileSync(providerFile,canonical(proof)+'\n');const forged=JSON.parse(originalLock);forged.files.arraySortProvider.sha256=hash(fs.readFileSync(providerFile));fs.writeFileSync(lockPath,canonical(forged)+'\n');
 const rejected=cp.spawnSync(process.execPath,['bin/as3-frontend','qualify',...args('forged')],{cwd:root,encoding:'utf8',timeout:120000});
 assert.notEqual(rejected.status,0);assert.match(rejected.stdout+rejected.stderr,/Array sort provider|ARRAY_SORT_PROVIDER/);
 } finally { fs.writeFileSync(providerFile,savedProof);fs.writeFileSync(lockPath,originalLock); }
 assert.equal(hash(fs.readFileSync(path.join(source,'ArraySortCallbackProbe.as'))),hash(read('source/ArraySortCallbackProbe.as')));
 fs.writeFileSync(path.join(dir,'report.json'),JSON.stringify({rows:16,node,web,sourceUnchanged:true,applicationStart:true,negativeControls:2},null,2)+'\n');
 console.log(JSON.stringify({dir,rows:16,realms:['Node','Chromium'],status:'passed'}));
});
