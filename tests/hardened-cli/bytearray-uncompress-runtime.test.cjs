...
test('Generated ByteArray uncompress delegates to the common provider with native state and errors',async()=>{
 assert.ok(laya&&process.env.HARDENED_FIXTURE_AIR_SDK&&process.env.HARDENED_FIXTURE_FFDEC,'AIR, Laya and FFDec required');
 const evidence=path.join(laya,'tests/nativeFlashOracle/bytearray-intrinsic-uncompress'),read=name=>fs.readFileSync(path.join(evidence,name));
 ...
 assert.equal(hash(read('ByteArrayIntrinsicUncompressProbe.as')),receipt.artifacts['source/ByteArrayIntrinsicUncompressProbe.as']);
 ...
 const base=path.join(root,'.cache/bytearray-intrinsic-uncompress');fs.mkdirSync(base,{recursive:true});
 ...
 const source=path.join(dir,'source');fs.mkdirSync(source);for(const file of ['ByteArrayIntrinsicUncompressProbe.as']){fs.mkdirSync(path.dirname(path.join(source,file)),{recursive:true});fs.writeFileSync(path.join(source,file),read(file));assert.equal(hash(read(file)),receipt.artifacts['source/'+file]);}
 const make=()=>run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','ByteArrayIntrinsicUncompressProbe',
  '--air-sdk',process.env.HARDENED_FIXTURE_AIR_SDK,'--laya',laya,'--ffdec-jar',process.env.HARDENED_FIXTURE_FFDEC,'--intrinsic-type','flash.utils.ByteArray','--bytearray-native-uncompress','--output',profile]);
 ...
 const authority=fs.readFileSync(path.join(dir,'emitted/__as3_runtime/AS3Authority.generated.js'),'utf8');assert.match(authority,/laya\/flash\/utils\/ByteArray/);assert.match(authority,/\.uncompress\(\)/);
...
globalThis.byteArrayUncompress=${JSON.stringify(steps)}.map(step=>{for(const call of step.calls)probe[call.method](...call.args);return {id:step.id,result:probe.result};});`;
 ...
 const node=JSON.parse(JSON.stringify(new Function(bundle+';return globalThis.byteArrayUncompress;')()));assert.deepEqual(node,wanted);
 const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||'playwright'),browser=await chromium.launch({headless:true});
 let web;try{const page=await browser.newPage({timezoneId:'Europe/Rome'});await page.addScriptTag({content:bundle});web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.byteArrayUncompress)));assert.deepEqual(web,wanted);}finally{await browser.close();}
 ...
 try {
 delete lock.files.byteArrayNative;fs.writeFileSync(lockPath,canonical(lock)+'\n');
 run(process.execPath,['bin/as3-frontend','qualify',...args('without-native-bytearray')]);
 const held=JSON.parse(fs.readFileSync(path.join(dir,'without-native-bytearray/manifest.json'))).files.find(row=>row.sourcePath==='ByteArrayIntrinsicUncompressProbe.as');assert.equal(held.status,'held');assert.equal(held.code,'HARDENED_INTRINSIC_MEMBER');
 } finally { fs.writeFileSync(lockPath,savedLock); }
 assert.equal(hash(fs.readFileSync(path.join(source,'ByteArrayIntrinsicUncompressProbe.as'))),hash(read('ByteArrayIntrinsicUncompressProbe.as')));
 fs.writeFileSync(path.join(dir,'report.json'),JSON.stringify({rows:18,node,web,sourceUnchanged:true,applicationStart:true,probeClasses:1,negativeControls:['Missing shared ByteArray proof held'],typecheck:{strict:true,strictNullChecks:false,diagnostics:0}},null,2)+'\n');
