...
test('Original TimeManager classes execute unchanged with shared Date properties',async()=>{
 assert.ok(laya&&process.env.HARDENED_FIXTURE_AIR_SDK&&process.env.HARDENED_FIXTURE_FFDEC,'AIR, Laya and FFDec required');
 const evidence=path.join(laya,'tests/nativeFlashOracle/original-time-manager'),read=name=>fs.readFileSync(path.join(evidence,name));
 ...
 assert.equal(hash(read('OriginalTimeManagerProbe.as')),receipt.artifacts['source/OriginalTimeManagerProbe.as']);
 ...
 const base=path.join(root,'.cache/original-time-manager');fs.mkdirSync(base,{recursive:true});
 ...
 const source=path.join(dir,'source');fs.mkdirSync(source);for(const file of ['OriginalTimeManagerProbe.as','util/time/TimeManager.as','util/time/TimeManagerClass.as','util/time/ITimeManager.as']){fs.mkdirSync(path.dirname(path.join(source,file)),{recursive:true});fs.writeFileSync(path.join(source,file),read(file));assert.equal(hash(read(file)),receipt.artifacts['source/'+file]);}
 const make=()=>run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','OriginalTimeManagerProbe',
  '--air-sdk',process.env.HARDENED_FIXTURE_AIR_SDK,'--laya',laya,'--ffdec-jar',process.env.HARDENED_FIXTURE_FFDEC,'--native-date','--shared-date','--shared-string-ranges','--output',profile]);
...
globalThis.originalTimeManager=${JSON.stringify(steps)}.map(step=>{for(const call of step.calls)probe[call.method](...call.args);return {id:step.id,result:probe.result};});`;
 ...
 const node=JSON.parse(JSON.stringify(new Function(bundle+';return globalThis.originalTimeManager;')()));assert.deepEqual(node,wanted);
 const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||'playwright'),browser=await chromium.launch({headless:true});
 let web;try{const page=await browser.newPage({timezoneId:'Europe/Rome'});await page.addScriptTag({content:bundle});web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.originalTimeManager)));assert.deepEqual(web,wanted);}finally{await browser.close();}
 ...
 try {
 delete lock.files.dateProvider;fs.writeFileSync(lockPath,canonical(lock)+'\n');
 run(process.execPath,['bin/as3-frontend','qualify',...args('without-date-provider')]);
 const held=JSON.parse(fs.readFileSync(path.join(dir,'without-date-provider/manifest.json'))).files.find(row=>row.sourcePath==='util/time/TimeManagerClass.as');assert.equal(held.status,'held');assert.equal(held.code,'HARDENED_DATE_MEMBER');
 } finally { fs.writeFileSync(lockPath,savedLock); }
 assert.equal(hash(fs.readFileSync(path.join(source,'OriginalTimeManagerProbe.as'))),hash(read('OriginalTimeManagerProbe.as')));
 fs.writeFileSync(path.join(dir,'report.json'),JSON.stringify({rows:6,node,web,sourceUnchanged:true,applicationStart:true,originalClasses:2,originalInterfaces:1,negativeControls:['Missing shared Date provider held'],typecheck:{strict:true,strictNullChecks:false,diagnostics:0}},null,2)+'\n');
