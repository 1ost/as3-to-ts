...
test('generated inherited lexical reads preserve native owner and getter effects',{skip:!laya&&!air&&!ffdec},t=>{
 assert.ok(laya&&air&&ffdec);const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'inherited-lexical-read-')));let completed=false;t.after(()=>{if(completed)fs.rmSync(dir,{recursive:true,force:true});else console.error('Retained inherited lexical evidence: '+dir);});
 const fixture=process.env.HARDENED_INHERITED_FIXTURE_SOURCE||path.join(laya,'tests/nativeFlashOracle/inherited-lexical-receiver'),native=JSON.parse(fs.readFileSync(path.join(fixture,'native-air.json')));
 const entry='InheritedLexicalProbe';
...
exports.entry=require('./__as3_runtime/ApplicationEntry.generated.js');`,resolveDir:out,sourcefile:'runner.js'},outfile:bundle,bundle:true,platform:'node',format:'cjs',alias:{laya:path.join(laya,'src/layaAir')},loader:{'.vs':'text','.fs':'text','.glsl':'text'},logLevel:'silent'});
 ...
 const negatives={ExplicitThis:'public function make():Function{return function():Object{return this.stage;};}',StaticCapture:'public static function make():Function{return function():Object{return stage;};}'};
 for(const [name,body] of Object.entries(negatives))fs.writeFileSync(path.join(negative,name+'.as'),'package {import flash.display.Sprite;public class '+name+' extends Sprite {'+body+'}}');
 ...
 for(const name of Object.keys(negatives))assert.equal(rows.find(row=>row.sourcePath===name+'.as').code,'HARDENED_LAMBDA_THIS');
 completed=true;
});
