...
test("canonical numeric Object keys preserve AIR lookup and first duplicate value",{skip:!air||!laya},async t=>{
 const fixture=path.join(fs.realpathSync(laya),"tests/nativeFlashOracle/object-numeric-keys");
 for(const [name,hash] of Object.entries({"ObjectNumericKeysProbe.as":"f85a051042bd7e7cfa3fb41af73ace9da8da8f293ca2cea6e7619e69716cfb06","native-air.json":"308eae19bf4ce42c49282e677f8e24f1ae53d22b4f7c408d8ecace14ed8338bc"}))
  assert.equal(sha(fs.readFileSync(path.join(fixture,name))),hash,name);
 const expected=JSON.parse(fs.readFileSync(path.join(fixture,"native-air.json"))).state;
 assert.equal(expected.ready,true);assert.equal(expected.failure,"");
 const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),"object-numeric-keys-")));t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));
 const source=path.join(dir,"source"),profile=path.join(dir,"profile"),output=path.join(dir,"output");fs.mkdirSync(source);
 fs.copyFileSync(path.join(fixture,"ObjectNumericKeysProbe.as"),path.join(source,"ObjectNumericKeysProbe.as"));
 ...
 run("python3",["-B","tools/create-fixture-profile.py","--source",source,"--entry","ObjectNumericKeysProbe","--air-sdk",air,"--laya",laya,"--output",profile]);
 ...
 const generated=fs.readFileSync(path.join(output,"__as3_runtime/application/ObjectNumericKeysProbe.ts"),"utf8");
 assert.match(generated,/__as3ObjectLiteral\(\[\["2", "first"\], \["10", "ten"\], \["2", "second"\], \["0", "zero"\]\]\)/);
...
const probe=startAS3Application(new AbortController().signal);globalThis.numericObjectKeys=probe.snapshot();`;
 ...
 assert.deepEqual(JSON.parse(JSON.stringify(new Function(bundle+";return globalThis.numericObjectKeys;")())),expected);
 if(process.env.LAYA_PLAYWRIGHT_MODULE){const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE),browser=await chromium.launch({headless:true});
  try{const page=await browser.newPage();await page.addScriptTag({content:bundle});assert.deepEqual(await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.numericObjectKeys))),expected);}finally{await browser.close();}}
});
