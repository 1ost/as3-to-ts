...
test("inherited super fields retain readonly, visibility, receiver and collision checks",{skip:!sdk||!laya},t=>{
 ...
 fs.writeFileSync(path.join(source,"base/Base.as"),'package base { public class Base { public var amount:int; protected var value:int; public const fixed:int=7; private var secret:int=8; var inside:int; protected function callback():void {amount++;} public function set writeOnly(v:int):void { value=v; } } }\n');
 const cases={
  ...
  BadSuperCallback:['public function run():Function {return super.callback;}','HARDENED_LOCAL_METHOD_CLOSURE'],
  ...
  BadCollision:['private var value:int;public function run():int {return super.value;}','HARDENED_LOCAL_FIELD_ANCESTRY'],
  BadRead:['public function run():void {this.writeOnly;}','HARDENED_LOCAL_INSTANCE_READ']
 };
 for(const [name,[body]] of Object.entries(cases))fs.writeFileSync(path.join(source,name+'.as'),`package {import base.Base;public class ${name} extends Base {${body}}}\n`);
