...
test('Function cast expression calls retain native receiver binding, evaluation order and failures',async()=>{
 assert.ok(laya&&process.env.HARDENED_FIXTURE_AIR_SDK&&process.env.HARDENED_FIXTURE_FFDEC,'AIR, Laya and FFDec required');
 const evidence=path.join(laya,'tests/nativeFlashOracle/function-cast-call'),read=name=>fs.readFileSync(path.join(evidence,name));
 ...
 assert.equal(hash(read('FunctionCastCallProbe.as')),receipt.artifacts['source/FunctionCastCallProbe.as']);
 ...
 const base=path.join(root,'.cache/function-cast-call');fs.mkdirSync(base,{recursive:true});
 ...
 const source=path.join(dir,'source');fs.mkdirSync(source);for(const file of ['FunctionCastCallProbe.as']){fs.mkdirSync(path.dirname(path.join(source,file)),{recursive:true});fs.writeFileSync(path.join(source,file),read(file));assert.equal(hash(read(file)),receipt.artifacts['source/'+file]);}
 const make=()=>run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','FunctionCastCallProbe',
 ...
 const emittedRecord=fs.readFileSync(path.join(dir,'emitted/__as3_runtime/application/FunctionCastCallProbe.ts'),'utf8');
 assert.match(emittedRecord,/__as3FunctionInvoke/);assert.match(emittedRecord,/__as3As/);
...
globalThis.functionCastCall=${JSON.stringify(steps)}.map(step=>{for(const call of step.calls)probe[call.method](...call.args);return {id:step.id,result:probe.result};});`;
 ...
 const node=JSON.parse(JSON.stringify(new Function(bundle+';return globalThis.functionCastCall;')()));assert.deepEqual(node,wanted);
 const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||'playwright'),browser=await chromium.launch({headless:true});
 let web;try{const page=await browser.newPage({timezoneId:'Europe/Rome'});await page.addScriptTag({content:bundle});web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.functionCastCall)));assert.deepEqual(web,wanted);}finally{await browser.close();}
 assert.equal(hash(fs.readFileSync(path.join(source,'FunctionCastCallProbe.as'))),hash(read('FunctionCastCallProbe.as')));
 const boundaries={
  ObjectCast:['public function call(value:Object):* {return (value as Object)();}','HARDENED_CALL_TARGET'],
  ClassCast:['public function call(value:Object):* {return (value as Class)();}','HARDENED_CALL_TARGET'],
  VoidArgument:['private function empty():void{} public function call(value:Object):* {return (value as Function)(empty());}','HARDENED_FUNCTION_ARGUMENT']
 };
 ...
 run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','FunctionCastCallProbe',
 ...
 for(const name of ['FunctionCastCallProbe'])assert.equal(rows.find(row=>row.sourcePath===name+'.as').status,'admitted',name);
 fs.writeFileSync(path.join(dir,'report.json'),JSON.stringify({rows:10,node,web,sourceUnchanged:true,applicationStart:true,probeClasses:1,rejectionGuards:3,typecheck:{strict:true,strictNullChecks:true,diagnostics:0}},null,2)+'\n');
