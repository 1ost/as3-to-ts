...
test('Date uses the authenticated shared Laya provider and retains native local time semantics',async()=>{
 assert.ok(laya&&process.env.HARDENED_FIXTURE_AIR_SDK&&process.env.HARDENED_FIXTURE_FFDEC,'AIR, Laya and FFDec required');
 const evidence=path.join(laya,'tests/nativeFlashOracle/date-shared-profile'),read=name=>fs.readFileSync(path.join(evidence,name));
 ...
 assert.equal(hash(read('source/DateSharedProbe.as')),receipt.artifacts['source/DateSharedProbe.as']);
 ...
 const base=path.join(root,'.cache/date-shared-profile');fs.mkdirSync(base,{recursive:true});
 ...
 const source=path.join(dir,'source');fs.mkdirSync(source);fs.writeFileSync(path.join(source,'DateSharedProbe.as'),read('source/DateSharedProbe.as'));
 const make=()=>run('python3',['-B','tools/create-fixture-profile.py','--source',source,'--entry','DateSharedProbe',
...
globalThis.dateShared=${JSON.stringify(steps)}.map(step=>{for(const call of step.calls)probe[call.method](...call.args);return {id:step.id,result:probe.result};});`;
 const manifest=JSON.parse(fs.readFileSync(path.join(dir,'emitted/manifest.json')));
 const applicationSource=fs.readFileSync(path.join(dir,'emitted',manifest.files.find(row=>row.sourcePath==='DateSharedProbe.as').typescriptPath),'utf8');
 ...
 const node=JSON.parse(JSON.stringify(new Function(bundle+';return globalThis.dateShared;')()));assert.deepEqual(node,wanted);
 const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||'playwright'),browser=await chromium.launch({headless:true});
 let web;try{const page=await browser.newPage({timezoneId:'Europe/Rome'});await page.addScriptTag({content:bundle});web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.dateShared)));assert.deepEqual(web,wanted);}finally{await browser.close();}
 ...
 const legacy=JSON.parse(fs.readFileSync(path.join(dir,'legacy/manifest.json'))).files.find(row=>row.sourcePath==='DateSharedProbe.as');
 ...
 assert.equal(hash(fs.readFileSync(path.join(source,'DateSharedProbe.as'))),hash(read('source/DateSharedProbe.as')));
