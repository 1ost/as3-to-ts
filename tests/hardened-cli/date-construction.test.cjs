...
test('Date native proof gates original zeroarg, numeric epoch and six-number calendar construction, methods, property and identity',t=>{
 ...
 fs.rmSync(profile,{recursive:true,force:true});make();compile('qualify','holds');const rows=JSON.parse(fs.readFileSync(path.join(dir,'holds/manifest.json'))).files;
	 for(const cls of Object.keys(holds).filter(cls=>!['OneArgument','FourArguments','SixStringArgument','Mutation'].includes(cls))){const row=rows.find(r=>r.sourcePath===cls+'.as');assert.equal(row?.status,'held',JSON.stringify(row));}
	 for(const cls of ['TwoArguments','ThreeArguments','FiveArguments','SevenArguments'])assert.equal(rows.find(r=>r.sourcePath===cls+'.as').code,'HARDENED_DATE_CONSTRUCTOR_ARITY');
	 for(const cls of ['OneArgument','FourArguments','SixStringArgument','Mutation'])assert.equal(rows.find(r=>r.sourcePath===cls+'.as').status,'admitted');
