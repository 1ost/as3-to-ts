...
test('generated callee identities preserve native callback removal and declaring method ownership',{skip:!laya&&!air&&!ffdec},t=>{
 assert.ok(laya&&air&&ffdec);const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'callback-callee-')));let completed=false;t.after(()=>{if(completed)fs.rmSync(dir,{recursive:true,force:true});else console.error('Retained callee evidence: '+dir);});
 const fixture=process.env.HARDENED_CALLEE_FIXTURE_SOURCE||path.join(laya,'tests/nativeFlashOracle/callback-callee-only'),native=JSON.parse(fs.readFileSync(path.join(fixture,'native-air.json')));
 const entry='CallbackCalleeOnlyProbe';
...
exports.entry=require('./__as3_runtime/ApplicationEntry.generated.js');`,resolveDir:out,sourcefile:'runner.js'},outfile:bundle,bundle:true,platform:'node',format:'cjs',alias:{laya:path.join(laya,'src/layaAir')},loader:{'.vs':'text','.fs':'text','.glsl':'text'},logLevel:'silent'});
 ...
 const negatives={StaticCallee:'public static function run():Function{return arguments.callee;}',ConstructorCallee:'public function ConstructorCallee(){super();var value:Function=arguments.callee;}',WriteCallee:'public function run():void{arguments.callee=null;}',GeneralArguments:'public function run(value:Object):Object{return arguments[0];}',ClassShadow:'public function run(ClassShadow:Object):Function{return arguments.callee;}',LocalClassShadow:'public function run():Function{var LocalClassShadow:Object=null;return arguments.callee;}'};
 ...
 const negativeProfile=path.join(dir,'negative-profile');run('python3',['-B','tools/create-fixture-profile.py','--source',negative,'--entry','StaticCallee','--air-sdk',air,'--laya',laya,'--ffdec-jar',ffdec,'--output',negativeProfile]);
 ...
 for(const name of Object.keys(negatives))assert.equal(rows.find(row=>row.sourcePath===name+'.as').code,name==='GeneralArguments'?'HARDENED_IDENTIFIER_SCOPE':'HARDENED_ARGUMENTS_CALLEE_SCOPE',name);
 completed=true;
});
