...
test('generated dynamic unary Number conversions match all native controls',{skip:!laya&&!air&&!ffdec},t=>{
 assert.ok(laya&&air&&ffdec);const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'dynamic-unary-number-')));let completed=false;t.after(()=>{if(completed)fs.rmSync(dir,{recursive:true,force:true});else console.error('Retained unary evidence: '+dir);});
 const fixture=process.env.HARDENED_UNARY_FIXTURE_SOURCE||path.join(laya,'tests/nativeFlashOracle/dynamic-unary-number-error-identity'),native=JSON.parse(fs.readFileSync(path.join(fixture,'native-air.json')));
 const entry='DynamicUnaryNumberProbe';
...
require('laya/laya/ModuleDef');exports.entry=require('./__as3_runtime/ApplicationEntry.generated.js');`,resolveDir:out,sourcefile:'runner.js'},outfile:bundle,bundle:true,platform:'node',format:'cjs',alias:{laya:path.join(laya,'src/layaAir')},loader:{'.vs':'text','.fs':'text','.glsl':'text'},logLevel:'silent'});
 ...
 fs.writeFileSync(path.join(negative,'VoidOperand.as'),'package {public class VoidOperand {public function nothing():void {} public function run():Number{return -nothing();}}}');
 const negativeProfile=path.join(dir,'negative-profile');run('python3',['-B','tools/create-fixture-profile.py','--source',negative,'--entry','VoidOperand','--air-sdk',air,'--laya',laya,'--ffdec-jar',ffdec,'--output',negativeProfile]);
 ...
 assert.equal(JSON.parse(fs.readFileSync(path.join(negativeOut,'manifest.json'))).files[0].code,'HARDENED_UNARY_NUMBER');
 completed=true;
});
