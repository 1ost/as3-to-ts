...
test('generated zero-argument integer toString preserves native decimal values and receiver effects',{skip:!laya&&!air&&!ffdec},t=>{
...
exports.entry=require('./__as3_runtime/ApplicationEntry.generated.js');`,resolveDir:out,sourcefile:'runner.js'},outfile:bundle,bundle:true,platform:'node',format:'cjs',alias:{laya:path.join(laya,'src/layaAir')},loader:{'.vs':'text','.fs':'text','.glsl':'text'},logLevel:'silent'});
 ...
 const negatives={ExplicitRadix:'public function run(value:int):String{return value.toString(16);}',NumberReceiver:'public function run(value:Number):String{return value.toString();}'};
 ...
 for(const name of Object.keys(negatives))assert.equal(rows.find(row=>row.sourcePath===name+'.as').code,name==='ExplicitRadix'?'HARDENED_INTEGER_STRING_ARITY':'HARDENED_MEMBER_TARGET',name);
 completed=true;
});
