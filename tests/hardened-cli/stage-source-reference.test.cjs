...
test('generated Stage references preserve native subtype identity and delegated lifecycle',{skip:!laya&&!air&&!ffdec},t=>{
 ...
 const fixture=process.env.HARDENED_STAGE_REFERENCE_FIXTURE_SOURCE||path.join(laya,'tests/nativeFlashOracle/stage-source-reference-explicit'),native=JSON.parse(fs.readFileSync(path.join(fixture,'native-air.json')));
 const entry='StageReferenceProbe';
 ...
 assert.equal(native.receipt.capture.identical,true);assert.equal(native.receipt.capture.observationCount,4);
