...
test("dynamic local updates preserve original source and deterministic emission",t=>{
 assert.ok(air&&laya&&ffdec,"AIR, Laya and FFDec fixture paths required");
 const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),"dynamic-local-update-")));
 t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));
 const source=path.join(dir,"source"),fixture=path.join(laya,"tests/nativeFlashOracle/dynamic-local-update");fs.mkdirSync(source);
 ...
 const makeProfile=()=>run("python3",["-B","tools/create-fixture-profile.py","--source",source,"--entry","DynamicLocalUpdateProbe","--air-sdk",air,"--laya",laya,"--ffdec-jar",ffdec,"--output",profile]);
 ...
 for(const output of ['first','second']) {
  ...
  const code=fs.readFileSync(path.join(dir,output,'__as3_runtime/application/DynamicLocalUpdateProbe.ts'),'utf8');
  assert.match(code,/__as3UpdatePrior/);
  assert.match(code,/__as3Number\(value\)/);
