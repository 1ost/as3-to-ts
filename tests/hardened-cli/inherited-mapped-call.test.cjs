...
test('generated inherited mapped calls preserve native owner and argument effects',{skip:!laya&&!air&&!ffdec},t=>{
 assert.ok(laya&&air&&ffdec);const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'inherited-mapped-call-')));let completed=false;t.after(()=>{if(completed)fs.rmSync(dir,{recursive:true,force:true});else console.error('Retained inherited mapped-call evidence: '+dir);});
 const fixture=process.env.HARDENED_MAPPED_CALL_FIXTURE_SOURCE||path.join(laya,'tests/nativeFlashOracle/mapped-call-lexical-receiver'),native=JSON.parse(fs.readFileSync(path.join(fixture,'native-air.json')));
 const entry='MappedCallLexicalProbe';
...
exports.entry=require('./__as3_runtime/ApplicationEntry.generated.js');`,resolveDir:out,sourcefile:'runner.js'},outfile:bundle,bundle:true,platform:'node',format:'cjs',alias:{laya:path.join(laya,'src/layaAir')},loader:{'.vs':'text','.fs':'text','.glsl':'text'},logLevel:'silent'});
  ...
  try {loaded.attach(probe);assert.ok(probe.stage,'real attached Flash Stage required');assert.deepEqual(JSON.parse(JSON.stringify(probe.snapshot())),native.capture.state);}finally{loaded.dispose();}

 }
 assert.deepEqual(...snapshots);
 const negative=path.join(dir,'negative-source');fs.mkdirSync(negative);
 const negatives={ExplicitThis:'public function make(child:DisplayObject):Function{return function():DisplayObject{return this.removeChild(child);};}',StaticCapture:'public static function make(child:DisplayObject):Function{return function():DisplayObject{return removeChild(child);};}',WrongArity:'public function make():Function{return function():DisplayObject{return removeChild();};}',AccessorShadow:'public function get removeChild():Function{return null;} public function make(child:DisplayObject):Function{return function():DisplayObject{return removeChild(child);};}'};
 ...
 const negativeProfile=path.join(dir,'negative-profile');run('python3',['-B','tools/create-fixture-profile.py','--source',negative,'--entry','ExplicitThis','--air-sdk',air,'--laya',laya,'--ffdec-jar',ffdec,'--output',negativeProfile]);
 ...
 for(const name of Object.keys(negatives))assert.equal(rows.find(row=>row.sourcePath===name+'.as').code,name==='WrongArity'?'HARDENED_CAPABILITY_CALL_ARITY':'HARDENED_LAMBDA_THIS',name);
 completed=true;
});
