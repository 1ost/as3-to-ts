...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-internal-storage');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-internal-storage');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InternalStorageProbe.as'){
...
function rejected(changed,owner='internalcases.Entry',providerEnabled=true){
...
const entry=sources['internalcases.Entry'].source;
rejected(entry,'internalcases.Entry',false);
for(const changed of [
 entry.replace('internal var _crc32:uint','internal var _crc32:Number'),
 entry.replace('internal var _crc32:uint','internal static var _crc32:uint'),
 entry.replace('_sizeCompressed:uint=7','_sizeCompressed:uint=1+6'),
 entry.replace('_sizeCompressed:uint=7','_sizeCompressed:uint=-1'),
 entry.replace('HEADER:uint=0x04034b50','HEADER:uint=4294967296'),
 entry.replace('HEADER:uint=0x04034b50','HEADER:uint=1+2'),
 entry.replace('return _crc32=value','return HEADER=value'),
 entry.replace('public function read():Array','internal function read():Array'),
 entry.replace('public function assign(value:*):*','internal function assign(value:*):*'),
 entry.replace('internal var _crc32:uint','private const _crc32:uint')
])rejected(changed);
const peer=sources['internalcases.Peer'].source;
rejected(peer.replace('entry._sizeCompressed=value','entry._sizeCompressed+=value'),'internalcases.Peer');
rejected(peer.replace('return [Entry.HEADER,Entry.MAXIMUM]','return [Entry.HEADER=1,Entry.MAXIMUM]'),'internalcases.Peer');
const outsider='package othercases {import internalcases.Entry;public class Bad {public function Bad(){} public function read(entry:Entry):uint{return entry._crc32;}}}';
rejected(outsider,'othercases.Bad');
rejected(sources['internalcases.Child'].source.replace('return [_crc32,_sizeCompressed,_sizeUncompressed]','return [Child.HEADER]'),'internalcases.Child');
assert.equal(rejectionGuards,15);
...
const driverFile=path.resolve('tests/native-generated-internal-storage/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
