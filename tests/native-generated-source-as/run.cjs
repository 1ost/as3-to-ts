...
const sources={};
for(const q of ["castcases.Tracker","castcases.Target","castcases.NullTarget","castcases.UndefinedTarget","castcases.WrongTarget","castcases.Child","castcases.Consumer"]){
...
for(const [body,missingHelper,reason] of [
 ...
 ['public function test(value:*):Boolean{return value is Target;}',false,/reference type operation requires class-evaluation authority/]
]) {
 ...
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeComputedTypeTestModule:missingHelper?undefined:options.nativeComputedTypeTestModule,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}),reason);
 guards++;
}
