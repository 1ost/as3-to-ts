...
const root=path.resolve('.cache/native-generated-json-decoder');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&['JSONDecoder.as','JSONTokenizer.as','JSONToken.as','JSONTokenType.as','JSONParseError.as'].includes(item.name)){
...
const driverFile=path.resolve('tests/native-generated-json-decoder/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured.filter(row=>/^(decode-|invalid-|tokens$)/.test(row.id));assert.equal(wanted.length,23);
