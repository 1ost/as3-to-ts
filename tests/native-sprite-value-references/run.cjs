...
const evidence=path.join(engine,'tests/nativeFlashOracle','sprite-value-references');const expected=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-sprite-value-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const text=fs.readFileSync(path.join(evidence,'source/SpriteValueHolder.as'),'utf8');const sources={'SpriteValueHolder':{source:text,sourceSha256:hash(text)}};
const source=sources['SpriteValueHolder'].source;
const applicationDomain=provider('AS3CanonicalSpriteValueReferences');
const names=['flash.geom.Transform','flash.media.SoundTransform','flash.accessibility.AccessibilityProperties'];
const nativeProviders=Object.fromEntries(names.map(name=>[name,{module:applicationDomain,exportName:name.split('.').pop()}]));
const input={scope:'generated-sprite-value-references',inheritScriptClasses:true,providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},sources};
...
Object.assign(options,{nativeSpriteValueReferenceModule:applicationDomain,nativeReferenceCoercion:{plan,module:"./declarationDomain",coercionModule:provider("AS3Type")},nativeSignaturePropertyModule:provider("AS3Property"),nativeDynamicConstructionModule:provider('AS3Invocation')});
...
let bindingGuards=0;
for(const patch of [
 {nativeSpriteValueReferenceModule:undefined},
 {nativeSpriteValueReferenceModule:'./wrong'},
 {nativeReferenceCoercion:undefined},
 {nativeGeneratedDeclarations:undefined},
 {importModules:{...options.importModules,'flash.geom.Transform':'./wrong'}},
 ...names.slice(1).map(name=>({importModules:{...options.importModules,[name]:'./wrong'}})),
]){
 assert.throws(()=>emit(parse('SpriteValueHolder.as',text),text,{...options,...patch}),/AS3_[A-Z_]+UNSUPPORTED/,Object.keys(patch).join(','));bindingGuards++;
}
const altered=source=>{
 const changed=api.createNativeGeneratedDeclarationPlan({...input,sources:{SpriteValueHolder:{source,sourceSha256:hash(source)}}});
...
for(const name of names.map(name=>name.split('.').pop())){
 ...
 assert.throws(()=>altered(text.replace('public class SpriteValueHolder {','public class SpriteValueHolder extends '+name+' {')),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);bindingGuards++;
}
(async()=>{
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  await page.route('http://sprite-value-references.test/**',route=>route.request().url().endsWith('/bundle.js')?route.fulfill({contentType:'text/javascript',body:code}):route.fulfill({contentType:'text/html',headers:{'Content-Security-Policy':"script-src 'self'"},body:'<script src="/bundle.js"></script>'}));
  await page.goto('http://sprite-value-references.test/');const web=await page.evaluate(async()=>{await globalThis.done;return globalThis.result;});await page.close();
  assert.deepEqual(errors,[]);assert.deepEqual(node.rows,expected);assert.deepEqual(web.rows,expected);assert.equal(node.guards,36);assert.equal(web.guards,36);
 ...
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({scope:'complete generated SpriteValueHolder source, three native reference bindings; no generated native subclass or full Sprite admission',sourceSha256:hash(text),expected,results,bindingGuards},null,2));
 console.log(JSON.stringify({run,targets:results.map(r=>r.target),rowsPerRuntime:expected.length,guardsPerRuntime:36,bindingGuards,typeErrors:0}));
