...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-mixed-catch');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-mixed-catch');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&['Catcher.as'].includes(item.name)){
...
const consumerOnly=process.argv.includes('--consumer');if(consumerOnly)sources['catchcases.Catcher'].referenceOnly=true;
...
const source=sources['catchcases.Catcher'].source;
for(const changed of [
 source.replaceAll('catch (error:SecurityError)', 'catch (error:TypeError)'),
 source.replace('finally { log.push("finally"); }', 'catch (other:*) { return other; } finally { log.push("finally"); }'),
 source.replace('select(value:*, log:Array)', 'select(value:*, log:Array, SecurityError:*)'),
 source.replace('package catchcases {','package catchcases { import foreign.SecurityError;')
]) {
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'security-catch-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'catchcases.Catcher':{source:changed,sourceSha256:hash(changed),...(consumerOnly?{referenceOnly:true}:{})}}});
  const guardOptions={...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}};
  emit(parse('Catcher.as',changed),changed,consumerOnly?consumerOptions(guardOptions):guardOptions);
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
const substituted=api.createNativeGeneratedDeclarationPlan({scope:'security-catch-substitution',providerModule:provider('AS3GeneratedClass'),providers:{SecurityError:{module:provider('AS3CanonicalErrorConstruction'),exportName:'Error'}},sources});
const substitutedOptions={...options,nativeGeneratedDeclarations:{plan:substituted,module:'./substituted'},nativeReferenceCoercion:{plan:substituted,module:'./substituted',coercionModule:provider('AS3Type')}};
assert.throws(()=>emit(parse('Catcher.as',source),source,consumerOnly?consumerOptions(substitutedOptions):substitutedOptions),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
assert.throws(()=>emit(parse('Catcher.as',source),source,{...(consumerOnly?consumerOptions(options):options),nativeSourceErrorModule:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
const namesake='package catchcases {public class SecurityError {}}';
const shadowPlan=api.createNativeGeneratedDeclarationPlan({scope:'security-catch-namesake',providerModule:provider('AS3GeneratedClass'),sources:{...sources,'catchcases.SecurityError':{source:namesake,sourceSha256:hash(namesake)}}});
const shadowOptions={...options,nativeGeneratedDeclarations:{plan:shadowPlan,module:'./shadow'},nativeReferenceCoercion:{plan:shadowPlan,module:'./shadow',coercionModule:provider('AS3Type')}};
assert.throws(()=>emit(parse('Catcher.as',source),source,consumerOnly?consumerOptions(shadowOptions):shadowOptions),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
...
for(const binding of Object.keys(sources).map(qname=>({qname}))){const source=sources[binding.qname].source,file=path.join(run,fileFor(binding.qname)+'.ts');fs.mkdirSync(path.dirname(file),{recursive:true});
 const opts=consumerOnly?consumerOptions(options):{...options};
...
const driverFile=path.resolve('tests/native-generated-mixed-catch/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,60);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v[0].value[0]=true]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({consumerOnly,combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Other typed catches, multiple sibling catches and SecurityError Class/constructor/subclass entry','Complete UIComponent and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:60,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
