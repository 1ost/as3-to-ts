...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-xml-boundaries');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-xml-boundaries');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['xmlcases.Reader'].source;
...
for(const changed of [
 source.replace('node.@name.length()','node.children()'),
 source.replace('var node:XML=value','var node:XML=new XML(value)'),
 source.replace('node..*.(name()=="variable"||name()=="accessor")','node..*.(name()==value)'),
 source.replace('node..*.(name()=="variable"||name()=="accessor")','node..*.(name()=="ns:variable")'),
 source.replace('return [typeof node.@name','node.@name="bad";return [typeof node.@name'),
 source.replace('return [typeof node.@name','(node.@name)="bad";return [typeof node.@name'),
 source.replace('return [typeof node.@name','return [node..*.(name()=="variable"),typeof node.@name'),
 source.replace('attributes(node:XML)','attributes(node:*)')
]){
 assert.notEqual(changed,source);
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'xml-boundary-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'xmlcases.Reader':{source:changed,sourceSha256:hash(changed)}}});
...
const driverFile=path.resolve('tests/native-generated-xml-boundaries/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,30);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='xml-0').value[0]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
