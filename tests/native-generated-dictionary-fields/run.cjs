...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-dictionary-fields');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-dictionary-fields');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='DictionaryFieldsProbe.as'){
...
for(const expression of ['this.privateMap[key]-=1','this.publicMap[key]++','--this.protectedMap[key]']){
 const q='fieldcases.Fields',source=sources[q].source.replace(' public function shadow',' public function unsupported(key:*):void{'+expression+';} public function shadow');
 const p=api.createNativeGeneratedDeclarationPlan({scope:'dictionary-field-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources:{...sources,[q]:{source,sourceSha256:hash(source)}}});
 assert.throws(()=>emit(parse(q+'.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard-domain'},nativeReferenceCoercion:{plan:p,module:'./guard-domain',coercionModule:provider('AS3Type')}}),/AS3_DICTIONARY_PROPERTY_UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-dictionary-fields/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,47);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='private-identity').value[2]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Static and arbitrary object receivers','Full EMVC and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:47,rejectionGuards,comparisons:wanted.length,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
