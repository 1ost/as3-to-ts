...
export async function run(module){
 ...
 const peer=as3ConstructClass(domain.getDefinition('retrycases.Peer'),[]),failedClass=item('classes',0),failedInstance=as3ConstructClass(failedClass,[]);
 const inspect=(instance,type)=>as3CallValue(get(peer,'read'),()=>[instance,type]);
 const change=(instance,value)=>as3CallValue(get(peer,'change'),()=>[instance,value]);
 change(failedInstance,10);row('failed-internal',inspect(failedInstance,failedClass));
 ...
 const successInstance=as3ConstructClass(selected,[]);
 row('successful-internal',inspect(successInstance,selected));
 change(failedInstance,20);row('retained-internal',[inspect(failedInstance,failedClass),inspect(successInstance,selected)]);
