...
const pin='9fd3c0ab0480e09907da5974c484a1db3875570e';
...
assert.equal(expected.length,14);assert.deepEqual(json(path.join(__dirname,'evidence-repeat/flash.json')).rows,expected);
const cache=path.join(compiler,'.cache/native-array-field-types');fs.mkdirSync(cache,{recursive:true});const run=fs.mkdtempSync(path.join(cache,'run-'));console.log('Evidence: '+run);
...
(async()=>{
 ...
 const qname='probe.ArrayFields',source=read(path.join(evidence,'sources/original/probe/ArrayFields.as'));
 ...
 const options={customVisitors:[],definitionsByNamespace:{probe:['ArrayFields']},nativeClassInitialization:{classes:{[qname]:'lazy'}},nativeCallableClasses:{[qname]:source},nativeCallableMethodBindingModule:'./AS3MethodBinding',nativeCallableCoercionModule:'./AS3MethodBinding',nativeCallableStringModule:'./AS3MethodBinding',nativeCallableMetadata:metadata};
 const generated=emit(parse('ArrayFields.as',source),source,options);fs.writeFileSync(path.join(run,'generated.ts'),generated);
 const guards=[];
 for(const [id,mutate] of [['wrong-instance-type',r=>r.instanceTraits[0].type='Object'],['wrong-static-type',r=>r.staticTraits[0].type='Object'],['wrong-source-sha',r=>r.sourceSha256='0'.repeat(64)]]){
  const wrong=structuredClone(metadata);mutate(wrong.classes[qname]);assert.throws(()=>emit(parse('ArrayFields.as',source),source,{...options,nativeCallableMetadata:wrong}),/AS3_CLASS_METADATA_UNSUPPORTED/);guards.push(id);
 }
 const reports=[],browser=await chromium.launch({headless:true});try{for(const target of [ts.ScriptTarget.ES5,ts.ScriptTarget.ES2015]){
  ...
  reports.push({target,originalRows:14,node:true,chromium:true,actualTypeFiles:program.getSourceFiles().length,actualDeclarationDiagnostics:0,bundleSHA256:sha(path.join(dir,'bundle.js'))});
 }}finally{await browser.close();}
 const report={ok:true,scope:'Authenticated builtin Array instance/static field reference binding; no foreign reference, lexical or whole timing admission',engineCommit:pin,generatedSHA256:sha(path.join(run,'generated.ts')),receipts,guards,reports,compilerInputs:['src/emit/native-callable-classes.ts','lib/emit/native-callable-classes.js','utils/callableClass.ts'].map(p=>({path:p,sha256:sha(path.join(compiler,p))})),providerInputs:Object.keys(providerBuild.metafile.inputs).filter(p=>p!=='<stdin>').map(p=>({path:path.relative(providerSource,path.resolve(p)),sha256:sha(path.resolve(p))}))};
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({ok:true,originalRows:14,guards:guards.length,run}));
