...
const dispatchMode=process.argv.includes('--dispatch');
const evidence=path.join(engine,'tests/nativeFlashOracle',dispatchMode?'signal-dispatch':'reference-static-initialization');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-reference-constants');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const includeSignal=dispatchMode||process.argv.includes('--signal');
const sources={};function visit(dir){for(const file of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,file.name);if(file.isDirectory()&&(includeSignal||file.name!=='org'))visit(full);else if(file.name.endsWith('.as')&&file.name!=='ReferenceStaticInitializationProbe.as'&&file.name!=='SignalDispatchProbe.as'){const qname=path.relative(path.join(evidence,'source'),full).replaceAll('\\','.').replaceAll('/','.').slice(0,-3),source=fs.readFileSync(full,'utf8');sources[qname]={source,sourceSha256:hash(source)};}}}visit(path.join(evidence,'source'));assert.equal(Object.keys(sources).length,dispatchMode?7:includeSignal?11:4);
const nativeProviders=includeSignal?{'flash.errors.IllegalOperationError':{module:modulePath(path.join(engine,'src/layaAir/flash/errors/IllegalOperationError.ts')),exportName:'IllegalOperationError'}}:{};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'reference-static-initialization',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),...(includeSignal?{scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['org.osflash.signals.Slot']}:{}) ,sources});
...
const options={customVisitors:[],importModules:{...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module])),'flash.utils.getQualifiedClassName':provider('getQualifiedClassName')},definitionsByNamespace,
...
function reject(body,extra={},additional={}) {
 const source='package guard { public class Guard { '+body+' } }';
 const sources={'guard.Guard':{source,sourceSha256:hash(source)},...additional};
 assert.throws(()=>{const p=api.createNativeGeneratedDeclarationPlan({scope:'reference-constant-guard',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources,...extra});
  emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
reject('public static const C:int=make();public static function make():int{return 7;}');
reject('public static const C:Function=null;');
reject('public static const C:Guard;');
reject('public static const C:Guard=new Guard(); C;');
reject('public static const C:Guard=new Guard();',{scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['guard.Guard']});
const contract='package guard { public interface IGuard {} }';
reject('public static const C:IGuard=null;',{}, {'guard.IGuard':{source:contract,sourceSha256:hash(contract)}});
reject('public static const C:Native=null;', {providers:{'guard.Native':{module:modulePath(path.join(engine,'src/layaAir/flash/errors/IllegalOperationError.ts')),exportName:'IllegalOperationError'}}});
reject('public static const C:Guard=new Guard(); public static function write():void{Guard.C=null;}');
...
const names=['AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass',...(includeSignal?['IllegalOperationError','getQualifiedClassName']:[])];
...
const driverFile=path.resolve('tests/native-generated-reference-constants',dispatchMode?'dispatch-driver.js':'runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=includeSignal?captured:captured.filter(row=>!row.id.startsWith('slotlist-')&&!row.id.startsWith('signal-'));  
assert.equal(wanted.length,dispatchMode?44:includeSignal?31:22);
const mutations=dispatchMode?[v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='dispatch-zero').value[0].reverse(),v=>v.find(r=>r.id==='once-removed-before-call').value[0][0][1]=1,v=>v.find(r=>r.id==='remove-during-dispatch').value[0].pop()]:[v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='self-constructor-defaults').value[0]=false,v=>v.find(r=>r.id==='retry-fresh-identity').value[0]=false,v=>v.find(r=>r.id==='constant-delete').value=true];
for(const mutate of mutations){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,includeSignal,dispatchMode,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:5,held:['Full error text and builtin Error catch identity are not compared',...(dispatchMode?['Builtin error catch brands, full messages, recursive dispatch and custom Class arguments remain unqualified']:includeSignal?['Signal dispatch and argument validation are checked by the separate --dispatch run']:['Original SlotList/Signal integration is a separate gate']),'Interface/vector/native reference constants and mixed class-body statements remain held']},null,2));console.log(JSON.stringify({run,sourceClasses:dispatchMode?4:includeSignal?8:4,sourceInterfaces:includeSignal?3:0,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],typeErrors:0}));
