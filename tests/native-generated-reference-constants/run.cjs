...
const evidence=path.join(engine,'tests/nativeFlashOracle/reference-static-initialization');const captured=require(path.join(evidence,'verify.cjs'));
...
const includeSignal=process.argv.includes('--signal');
const sources={};function visit(dir){for(const file of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,file.name);if(file.isDirectory()&&(includeSignal||file.name!=='org'))visit(full);else if(file.name.endsWith('.as')&&file.name!=='ReferenceStaticInitializationProbe.as'){const qname=path.relative(path.join(evidence,'source'),full).replaceAll('\\','.').replaceAll('/','.').slice(0,-3),source=fs.readFileSync(full,'utf8');sources[qname]={source,sourceSha256:hash(source)};}}}visit(path.join(evidence,'source'));assert.equal(Object.keys(sources).length,includeSignal?11:4);
...
const driverFile=path.resolve('tests/native-generated-reference-constants/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=includeSignal?captured:captured.filter(row=>!row.id.startsWith('slotlist-')&&!row.id.startsWith('signal-'));  
assert.equal(wanted.length,includeSignal?31:22);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='self-constructor-defaults').value[0]=false,v=>v.find(r=>r.id==='retry-fresh-identity').value[0]=false,v=>v.find(r=>r.id==='constant-delete').value=true]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
