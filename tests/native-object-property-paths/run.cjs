...
const evidence=path.join(engine,'tests/nativeFlashOracle',order?'property-call-order':'nested-property-call');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-object-property-paths');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries((order?['Reader','Subject']:['Reader','Root','Domain']).map(name=>{const source=fs.readFileSync(path.join(evidence,'source/cases',name+'.as'),'utf8');return ['cases.'+name,{source,sourceSha256:hash(source)}];}));
const source=sources['cases.Reader'].source;
const counterpartSource=order?sources['cases.Subject']:null;
if(order)delete sources['cases.Subject'];
const consumerOnly=process.argv.includes('--consumer');
const consumers=new Set(consumerOnly?['cases.Reader']:[]);
...
const planInput={scope:'object-property-paths',interfaceProviderModule:provider('AS3Type'),providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources};
...
for(const changed of [{...options,nativeObjectPropertyModule:'./wrong'},
 ...
 assert.throws(()=>emit(parse('Reader.as',source),source,changed),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
if(order){
 const held=api.createNativeGeneratedDeclarationPlan({...planInput,scriptGlobalSources:['cases.Subject'],sources:{...sources,'cases.Subject':counterpartSource}});
 assert.throws(()=>emit(parse('Subject.as',counterpartSource.source),counterpartSource.source,{...options,
...
if(!order)for(const replacement of ['return value.applicationDomain -= 1;','return value.applicationDomain++;','return new value.applicationDomain();']){
 ...
 assert.throws(()=>emit(parse('Reader.as',alteredSource),alteredSource,{...options,
...
const counterpartFile=order?path.join(run,'Subject.ts'):null;
if(order)fs.writeFileSync(counterpartFile,fs.readFileSync(path.resolve('tests/native-object-property-paths/order-subject.ts'),'utf8')
 .replace('__FLASH_METADATA__',provider('FlashTypeMetadata')).replace('__AS3_PROPERTY__',provider('AS3Property')));
const files=[path.join(run,'declarationDomain.ts'),...emitted.map(e=>e.file),...(order?[counterpartFile]:[]),...['glsl.d.ts','spine.d.ts'].map(f=>path.join(engine,'src/layaAir/tslibs',f))];
...
const driverFile=path.resolve('tests/native-object-property-paths',order?'order-driver.js':'runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,order?12:30);
...
try{for(const target of [ts.ScriptTarget.ES5,ts.ScriptTarget.ES2015]){
 const specs=[];for(const file of files.filter(f=>!f.endsWith('.d.ts'))){const source=fs.readFileSync(file,'utf8'),out=ts.transpileModule(source,{compilerOptions:{target:file===counterpartFile?ts.ScriptTarget.ES2015:target,module:ts.ModuleKind.CommonJS,experimentalDecorators:true},reportDiagnostics:true});assert.deepEqual(out.diagnostics,[]);const relative=path.relative(run,file).replaceAll('\\','/').replace(/\.ts$/,'');specs.push({name:relative,code:out.outputText});}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({consumerOnly,combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,hostControls:0,comparisonNegativeControls:3,order,counterpartSource,counterpartOutputSha256:order?hash(fs.readFileSync(counterpartFile)):null,held:['Full UIComponent']},null,2));console.log(JSON.stringify({run,consumerOnly,sourceClasses:order?1:3,nativeCounterparts:order?1:0,interfaces:0,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
