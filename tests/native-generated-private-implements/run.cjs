...
const evidence=path.join(engine,'tests/nativeFlashOracle/file-local-implements'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-private-implements');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['FileLocalImplementsProbe','implementations.First','implementations.Second','contracts.IBase','contracts.IValue'])};
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   for(const [from,to,error] of [
    ...
    ['implements IValue','implements IValue,IValue',/duplicate file-private implements/],
    ['implements IValue','implements Missing',/exact source interface/]
   ]) {
    const source=original.replace(from,to);assert.notEqual(source,original);
    reject(()=>api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,'implementations.First':{source,sourceSha256:hash(source)}}}),error);
   }
   assert.equal(guards,10);
   assert.equal(plan.privateBindings.filter(b=>b.interfaces.length).length,2);
   assert(plan.interfaceContracts.implementations.some(c=>c.owner==='implementations.First#file:Child'&&c.implementationOwner==='implementations.First#file:Helper'));
