...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-native-interfaces');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-native-interfaces');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='NativeInterfaceProbe.as'){
...
const nativeProviders=Object.fromEntries(['IDataInput','IDataOutput'].map(name=>['flash.utils.'+name,{module:provider(name),exportName:name,nativeInterface:true}]));
const input={scope:'generated-native-interfaces',lexicalProviderModule:provider('AS3LexicalMembers'),providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources};
...
function rejectPlan(overrides){assert.throws(()=>api.createNativeGeneratedDeclarationPlan({...input,...overrides}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;}
rejectPlan({interfaceProviderModule:undefined});
rejectPlan({providers:{...nativeProviders,'flash.utils.IDataInput':{...nativeProviders['flash.utils.IDataInput'],nativeInterface:false}}});
rejectPlan({providers:{...nativeProviders,'flash.utils.IDataInput':{...nativeProviders['flash.utils.IDataInput'],nativeBase:'Event'}}});
function rejectSource(source,providers=nativeProviders){
 const owner='nativeio.Reader',changed={...sources,[owner]:{source,sourceSha256:hash(source)}};
 assert.throws(()=>{const guard=api.createNativeGeneratedDeclarationPlan({...input,providers,sources:changed});
  emit(parse(owner+'.as',source),source,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
const reader=sources['nativeio.Reader'].source;
rejectSource(reader,Object.fromEntries(Object.entries(nativeProviders).map(([name,binding])=>[name,{module:binding.module,exportName:binding.exportName}])));
rejectSource(reader.replace('public class Reader {','public class Reader implements IDataInput {'));
rejectSource(reader.replace('public class Reader {','public class Reader extends IDataInput {'));
rejectSource(reader.replace('public function Reader(){}','public function Reader(){new IDataInput();}'));
rejectSource(reader.replace('internal function parse(value:IDataInput):Boolean','internal function parse(value:IDataInput=null):Boolean'));
rejectSource(reader.replace('internal function parse(value:IDataInput):Boolean','internal function parse(value:Object):Boolean'));
rejectSource(reader.replace('internal function parse(value:IDataInput):Boolean','internal function parse(value:IDataInput):uint'));
assert.equal(rejectionGuards,10);
...
const names=['ByteArray','IDataInput','IDataOutput','IOError','AS3CanonicalErrorConstruction','trace','Dictionary','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-native-interfaces/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
...
(async()=>{const {chromium}=require(process.env.LAYA_PLAYWRIGHT_MODULE||process.env.PLAYWRIGHT_MODULE||require.resolve('playwright',{paths:[path.resolve('../op2-html5/game-client-laya'),engine]}));const browser=await chromium.launch({headless:true});const results=[];
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,runtimeGuards:6,comparisonNegativeControls:3,held:['Native interface implements/extends, native constructors, arbitrary native member dispatch, optional/rest internal signatures','Complete Zip/ZipFile and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:14,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
