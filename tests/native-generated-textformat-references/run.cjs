...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-textformat-references');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-textformat-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='TextFormatReferenceProbe.as'){
...
const consumerOnly=process.argv.includes('--consumer');if(consumerOnly)sources['formatcases.Reader'].referenceOnly=true;
...
const options={customVisitors:[],nativeTextFormatReferenceModule:provider('AS3CanonicalTextFormatReference'),importModules:{"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
const reader=sources['formatcases.Reader'].source;
for(const patch of [
 {nativeTextFormatReferenceModule:undefined},
 {nativeReferenceCoercion:undefined},

 {nativeTextFormatReferenceModule:'./wrong-provider'},
 {importModules:{...options.importModules,'flash.text.TextFormat':'./wrong-provider'}},
...
for(const body of [
 'public static var value:Boolean=null is TextFormat;',
 'public static var value:TextFormat=null as TextFormat;',
 'public function test(value:*,TextFormat:Class):Boolean{return value is TextFormat;}',
 'public function test(value:*,TextFormat:*):*{return value as TextFormat;}'
]) {
 const source='package formatcases {import flash.text.TextFormat;public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'display-reference-guard',providerModule:provider('AS3GeneratedClass'),sources:{'formatcases.Guard':{source,sourceSha256:hash(source),...(consumerOnly?{referenceOnly:true}:{})}},providers:nativeProviders});
...
for(const binding of Object.keys(sources).map(qname=>({qname}))){const source=sources[binding.qname].source,file=path.join(run,fileFor(binding.qname)+'.ts');fs.mkdirSync(path.dirname(file),{recursive:true});
 const opts=consumerOnly&&binding.qname==='formatcases.Reader'?consumerOptions(options):{...options};
...
const driverFile=path.resolve('tests/native-generated-textformat-references/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
