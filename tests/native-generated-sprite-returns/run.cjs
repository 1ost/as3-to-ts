...
const evidence=path.join(engine,'tests/nativeFlashOracle/sprite-returns'),expected=require(path.join(evidence,'verify.cjs'));
...
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider),spriteModule,provider('AS3CanonicalDisplayReference')];
   ...
   const compilerGuards=require('./guards.cjs')(api,input,config,hash);assert.equal(compilerGuards,4);
  ...
  await assert.rejects(()=>execute(mutated.outputFiles[0].text),/forged Sprite rejected/);
