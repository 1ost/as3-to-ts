...
const evidence=path.join(engine,'tests/nativeFlashOracle/local-function-intrinsics'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-local-function-intrinsics');fs.mkdirSync(cache,{recursive:true});
...
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  for(const [name,pattern,replacement,row]of [
   ['bypass-local-call-provider',/[\w$]+\.as3CallNamedProperty\(local, "call", /g,'(function(target,key,args){return target.call.apply(target,args());})(local,"call", ','call-null']
  ]){
