...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-dictionary-boundaries');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-dictionary-boundaries');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='DictionaryBoundariesProbe.as'){
...
function guardSource(body,imports=''){
 const source='package dictionarycases {'+imports+' public class Guard {'+body+'}}';
 const all={...sources,'dictionarycases.Guard':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'dictionary-guard',providers:{...nativeProviders,'flash.utils.ByteArray':{module:provider('ByteArray'),exportName:'ByteArray'}},providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),sources:all});
 return emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard-domain'},nativeReferenceCoercion:{plan:p,module:'./guard-domain',coercionModule:provider('AS3Type')}});
}
for(const expression of ['trace()','trace("a","b")','trace(7)','trace({})','trace','new trace("a")','Error()','Error("a",1)']){
 assert.throws(()=>guardSource('public function run():*{return '+expression+';}'),/AS3_(GLOBAL_MODULE|SOURCE_ERROR)_UNSUPPORTED/,expression);rejectionGuards++;
}
assert.throws(()=>guardSource('public function run():ByteArray{return null;}','import flash.utils.ByteArray;'),/native return type requires separate qualification/);rejectionGuards++;
for(const body of ['public function run(trace:Function):void{trace("shadow");}',
 'public function trace(value:String):void{} public function run():void{trace("shadow");}',
 'public function run(Error:Function):*{return Error("shadow");}']){
 const result=guardSource(body);assert.ok(!result.includes('__as3_global_trace')&&!result.includes('as3CreateError'),'authored binding must not become a builtin');rejectionGuards++;
}
...
const names=['trace','Dictionary','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-dictionary-boundaries/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const printing=require(path.join(engine,'tests/nativeFlashOracle/generated-logger-trace/verify.cjs'));
assert.equal(hash(fs.readFileSync(path.join(engine,'tests/nativeFlashOracle/generated-logger-trace/source/org/emvc/log/ELogger.as'))),sources['org.emvc.log.ELogger'].sourceSha256);
const wanted=captured.concat(printing.rows.map(r=>({...r,id:'trace-'+r.id})),[{id:'trace-output',value:printing.output}]);assert.equal(wanted.length,65);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='exchange-object').value[1]=1]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Derived Dictionary types, enumeration and weak-key collection timing','Source ArgumentError identity for generated arity failures (name/id only compared)', 'Full EMVC and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:64,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
