...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-textfield-storage');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-textfield-storage');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='TextFieldStorageProbe.as'){
...
for(const body of [
 'public static function getField():TextField{return null;}',
 'public static function test(value:*):Boolean{return value is TextField;}',
 'public static function cast(value:*):*{return value as TextField;}'
]){
 const source='package textcases {import flash.text.TextField;public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'textfield-held-guard',providerModule:provider('AS3GeneratedClass'),sources:{'textcases.Guard':{source,sourceSha256:hash(source)}},providers:nativeProviders});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}),/AS3_.*UNSUPPORTED/);
 rejectionGuards++;
}
const unboundPlan=api.createNativeGeneratedDeclarationPlan({scope:'textfield-unbound-guard',providerModule:provider('AS3GeneratedClass'),sources,providers:{}});
assert.throws(()=>emit(parse('TextSink.as',sources['textcases.TextSink'].source),sources['textcases.TextSink'].source,{...options,nativeGeneratedDeclarations:{plan:unboundPlan,module:'./guard'},nativeReferenceCoercion:{plan:unboundPlan,module:'./guard',coercionModule:provider('AS3Type')}}),/AS3_.*UNSUPPORTED/);
rejectionGuards++;
...
const driverFile=path.resolve('tests/native-generated-textfield-storage/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
