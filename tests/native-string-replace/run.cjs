...
const engine=path.resolve('../LayaAir-op2'),evidence=path.join(engine,'tests/nativeFlashOracle/string-replace-emission');
...
const base=path.resolve('.cache/native-string-replace');fs.mkdirSync(base,{recursive:true});const run=fs.mkdtempSync(path.join(base,'run-'));
...
const source=fs.readFileSync(path.join(evidence,'source/StringReplaceEmissionProbe.as'),'utf8');
const sources={StringReplaceEmissionProbe:{source,sourceSha256:hash(source),referenceOnly:true}};
...
const options={nativeStringIntrinsicsModule:provider('AS3StringIntrinsics'),customVisitors:[],definitionsByNamespace:{'':['StringReplaceEmissionProbe']},decoratorModules:{bound:helpers.bound,classBound:helpers.classBound},nativeClassHelperModules:{nativeClass:helpers.nativeClass,callableClass:helpers.callableClass},nativeGlobalModules:{Date:provider('AS3Date')},nativeComputedTypeTestModule:provider('AS3Type'),nativeNumericMethodParametersModule:provider('AS3Coercion'),nativeSourceErrorModule:modulePath(path.join(engine,'src/layaAir/flash/errors/AS3SourceError.ts')),nativeReferenceCoercion:{plan,module:'./domain',coercionModule:provider('AS3Type')}};
fs.writeFileSync(path.join(run,'domain.ts'),plan.moduleSource);
const output=emit(parse('StringReplaceEmissionProbe.as',source),source,options);
fs.writeFileSync(path.join(run,'StringReplaceEmissionProbe.ts'),output);
const emitted=[{name:'StringReplaceEmissionProbe',sourceSha256:hash(source),generatedSha256:hash(output)}];
let guards=0;
const reject=(body,config=options)=>{const text='package {public class Guard {'+body+'}}';assert.throws(()=>emit(parse('Guard.as',text),text,config),/AS3_(?:STRING_INTRINSIC|GENERATED_EMISSION)_UNSUPPORTED/);guards++;};
reject('public function f(s:String,p:*):String {return s.replace(p,1);}');
reject('public function f(s:String):String {return s.replace(/M/g);}');
reject('public function f(s:String):String {return s.replace(/M/g,1,2);}');
reject('public function f(s:String):String {return s.replace(/M/g,1);}',{...options,nativeReferenceCoercion:undefined});
const untouched='package {public class Guard {public function f(s:*):* {return s.replace(/M/g,1);}}}';assert.ok(!emit(parse('Guard.as',untouched),untouched,options).includes('sourceStringReplace'));guards++;
fs.writeFileSync(path.join(run,'driver.ts'),"import {StringReplaceEmissionProbe} from './StringReplaceEmissionProbe';\nexport function run(){return JSON.parse(JSON.stringify(new StringReplaceEmissionProbe().snapshot().observations));}\n");
...
async function main(){
 ...
 console.log('String replacement: '+guards+' guards, 13 AIR observations from unchanged source, Node/Chromium ES5/ES2015. '+run);
}
