...
const driverFile=path.resolve('tests/native-generated-vector-boundaries/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8').replace('@TOKENS@',JSON.stringify(Object.fromEntries(plan.interfaces.map(b=>[b.qname,b.tokenExport]))));
const wanted=captured.filter(r=>r.id==='default'||/^(read-|exchange-|assign-|local-|catch-|missing-argument|extra-argument)/.test(r.id));assert.equal(wanted.length,33);
