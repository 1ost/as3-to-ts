...
const evidence=path.join(engine,'tests/nativeFlashOracle/method-intrinsics'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-method-intrinsics');fs.mkdirSync(cache,{recursive:true});
...
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const mutation=originalFactory.replaceAll('\"self:\"','\"changed:\"');assert.notEqual(mutation,originalFactory);
