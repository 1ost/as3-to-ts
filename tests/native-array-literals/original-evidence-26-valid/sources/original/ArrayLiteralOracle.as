package {
 ...
 public class ArrayLiteralOracle extends Sprite {
  public function ArrayLiteralOracle(){
   ...
   for(var i:int=0;i<26;i++){try{rows.push({index:i,value:shape(new Subject(i).value)});}catch(error:*){rows.push({index:i,error:error.errorID,name:error.name,message:error.message,stack:error.getStackTrace()});}}
