...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-regex-split'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-regex-split');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['regexsplit.Subject'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const input={scope:'class-script-retry-'+cohort,sources,providers:nativeProviders,vectorProviderModule:provider('AS3Vector'),patternProviderModule:provider('AS3StringIntrinsics'),providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['regexsplit.Subject'],lexicalProviderModule:provider('AS3LexicalMembers')};
   ...
   assert.throws(()=>api.emitNativeSourceClassModule({...config,emitterOptions:{...options,nativeStringIntrinsicsModule:'./wrong'}}),/AS3_PATTERN_LOCAL_UNSUPPORTED/);rejectionGuards++;
   const limited='package regexsplit {public class Subject {public static function lines(value:String):Array{return value.split(/,+/,1);}}}';
   const limitedPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{'regexsplit.Subject':{source:limited,sourceSha256:hash(limited)}}});
   assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:limitedPlan,emitterOptions:{...options,nativeVectorTypes:{...options.nativeVectorTypes,plan:limitedPlan},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:limitedPlan}}}),/AS3_STRING_INTRINSIC_UNSUPPORTED: split requires exactly one authored arguments/);rejectionGuards++;
  ...
  const mutation=originalFactory.replaceAll('sourceStringSplit','sourceStringMatch');assert.notEqual(mutation,originalFactory);
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Unknown Function-valued receivers and complete game startup','Full startup and game account flow']},null,2));
