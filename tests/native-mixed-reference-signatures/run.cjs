...
const expected={mixed:require(path.join(evidence,'verify.cjs')),zip:require(path.join(engine,'tests/nativeFlashOracle/date-zip-prerequisite/verify.cjs')),throws:require(path.join(engine,'tests/nativeFlashOracle/reference-signature-throws/verify.cjs'))},hash=value=>crypto.createHash('sha256').update(value).digest('hex');
...
async function main(){
 ...
 console.log('Mixed reference signatures: '+guards+' guards, 51 AIR observations from three unchanged sources, Node/Chromium ES5/ES2015. '+run);
}
