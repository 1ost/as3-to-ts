...
exports.fixture=function(compiler,target,sources,commonSource,metadata,arrayModule) {
    ...
    const context=vm.createContext({performance,TextEncoder,TextDecoder,setTimeout,clearTimeout}),modules=new Map(),generated={};
    ...
    load(commonSource,'AS3MethodBinding');modules.set('CommonProvider',modules.get('AS3MethodBinding'));modules.set('ArrayFactory',modules.get('AS3MethodBinding'));
    ...
    for(const [qname,source] of Object.entries(sources)) {
        ...
        generated[name]=emit(parse(name+'.as',source),source,{customVisitors:[],definitionsByNamespace:{},nativeClassInitialization:{classes},
            nativeCallableMethodBindingModule:'./AS3MethodBinding',nativeCallableCoercionModule:'./AS3MethodBinding',nativeCallableClasses:sources,nativeCallableMetadata:metadata,nativeArrayCreationModule:arrayModule});
