...
const evidence=path.join(engine,'tests/nativeFlashOracle/text-field-returns'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-text-field-returns');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['cases.TextFieldReturns'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const textFieldModule=modulePath(path.join(engine,'src/layaAir/flash/utils/AS3CanonicalTextFieldReference.ts'));
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider),textFieldModule];
   const input={scope:'text-field-returns-'+cohort,sources,providers:{'flash.text.TextField':{module:textFieldModule,exportName:'TextField'}},providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};
   ...
   const options={customVisitors:[],definitionsByNamespace,nativeTextFieldReferenceModule:textFieldModule,importModules:{'flash.text.TextField':textFieldModule,'compiler.AS3Class':provider('AS3Class'),'compiler.AS3Invocation':provider('AS3Invocation')},
   ...
   const compilerGuards=require('./guards.cjs')(api,input,config,hash);assert.equal(compilerGuards,7);
  ...
  const build=(mutate=false)=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutate?[{name:'remove-return-coercion',setup(b){b.onLoad({filter:/subject-factory\.js$/},args=>{
   let contents=fs.readFileSync(args.path,'utf8');
   const re=/__as3_callable_generatedProperty\.coerceAS3PropertyValue\(value, \{ name: "flash\.text::TextField", reference: __as3_callable_declarationDomain\.native\d+ \}\)/g;
   assert.equal([...contents.matchAll(re)].length,3);
  ...
  await page.goto('http://loaded-generated.test/');await page.evaluate(()=>globalThis.completion);const web=await page.evaluate(()=>globalThis.result);await page.close();assert.deepEqual(web.rows,expected);assert.equal(web.checks.length,5);
  const mutated=await build(true);
  const bad=await browser.newPage();await bad.route('http://mutated.test/**',route=>route.request().url().endsWith('/bundle.js')?route.fulfill({contentType:'text/javascript',body:mutated.outputFiles[0].text}):route.fulfill({contentType:'text/html',body:'<!doctype html><body><script src="/bundle.js"></script></body>'}));
  await bad.goto('http://mutated.test/');await assert.rejects(()=>bad.evaluate(()=>globalThis.completion),/forged TextField rejected/);await bad.close();
  const negatives=['remove-return-coercion'];
  results.push({target,web,typechecks,artifacts,negatives,compilerGuards:7,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Other native return types','Full startup and game account flow']},null,2));
