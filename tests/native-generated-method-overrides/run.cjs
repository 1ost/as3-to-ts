...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-method-overrides'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-method-overrides');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['overrides.Base']),child:read('source',['overrides.Base','overrides.Child','overrides.Grandchild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const compilerGuards=require('./guards.cjs')(api,input);assert.equal(compilerGuards,8);
  ...
  const negatives=['missing-signature','missing-parent-selection'];
  for(const mutation of negatives){
   const mutated=(await build(mutation)).outputFiles[0].text;
   await assert.rejects(()=>execute(mutated),mutation==='missing-signature'?/AS3_GENERATED_CLASS_UNSUPPORTED: selected parent override/:/cohort did not reuse selected declaration: overrides::Base/);
  }
