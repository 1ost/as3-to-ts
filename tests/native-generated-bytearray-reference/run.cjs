...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-bytearray-reference');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-bytearray-reference');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['byteref.Reader'].source;
for(const key of ['nativeByteArrayReferenceModule','nativeReferenceCoercion','nativeComputedTypeTestModule']){
 assert.throws(()=>emit(parse('Reader.as',source),source,{...options,[key]:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const changed of [
 source.replace('cast(value:*)','cast(value:*,ByteArray:*)'),
 source.replace('public var calls:int=0;','public var calls:int=0;public var early:ByteArray=null as ByteArray;'),
 source.replace('value as ByteArray','value as flash.utils.ByteArray')
]){
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'bytearray-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'byteref.Reader':{source:changed,sourceSha256:hash(changed)}}});
...
]){assert.throws(()=>emit(parse('Reader.as',source),source,{...options,...override}),/AS3_BYTEARRAY_REFERENCE_UNSUPPORTED/);rejectionGuards++;}
...
const driverFile=path.resolve('tests/native-generated-bytearray-reference/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['ByteArray construction, extracted native methods and qualified or shadowed type operands','Complete Zip/ZipFile and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:15,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
