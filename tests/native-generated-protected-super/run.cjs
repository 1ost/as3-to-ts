...
const evidence=path.join(engine,'tests/nativeFlashOracle/protected-super'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-protected-super');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['protsuper.Base']),child:read('source',['protsuper.Base','protsuper.Child','protsuper.Grandchild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/child-factory\.js$/},args=>{
   ...
   if(mutation==='virtual-super'){
    const re=/"protected", false, true/g;assert.ok([...contents.matchAll(re)].length>=8);
    contents=contents.replace(re,'"protected", false, false');
  ...
  const negatives=['virtual-super','missing-parent-selection'];
  for(const mutation of negatives){
   const mutated=(await build(mutation)).outputFiles[0].text;
   if(mutation==='virtual-super')await assert.rejects(()=>execute(mutated),/stack|recursion/i);
   else await assert.rejects(()=>execute(mutated),/cohort did not reuse selected declaration: protsuper::Base/);
  }
