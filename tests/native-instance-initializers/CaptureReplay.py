...
commands = [
    ['java', '-Xmx384m', '-jar', str(FLEX / 'lib/mxmlc.jar'), '+flexlib=' + str(FLEX / 'frameworks'),
     '-target-player=26.0', '-swf-version=37', '-source-path=' + str(HERE / 'replay-original'),
     '-output=' + str(OUTPUT / 'oracle.swf'), str(HERE / 'replay-original/ReplayOracle.as')],
...
files = sorted((HERE / 'replay-original').rglob('*.as')) + [Path(__file__), OUTPUT / 'capture.cjs', OUTPUT / 'oracle.swf', OUTPUT / 'flash.json',
