...
const sourceClasses = Object.fromEntries(names.map(name => ['instanceorder.' + name,
    fs.readFileSync(path.join(sourceDirectory, name + '.as'), 'utf8')]));
const reports = [];
for (const target of [ts.ScriptTarget.ES5, ts.ScriptTarget.ES2015]) {
    ...
    function load(source, name) {
        const result = name==='AS3MethodBinding'?{outputText:source,diagnostics:[]}:ts.transpileModule(source, {compilerOptions:{
    ...
    for (const name of ['bound', 'classBound', 'nativeClass', 'callableClass'])
    ...
    for (const name of names) {
        ...
        const native = emit(parse(name + '.as', source), source, {customVisitors:[], definitionsByNamespace:{},
            nativeClassInitialization:{classes}, nativeCallableMethodBindingModule:"./AS3MethodBinding",nativeCallableCoercionModule:"./AS3MethodBinding",nativeCallableClasses:sourceClasses});
...
assert(reports.every(report => report.exactMatch), 'All original Flash construction observations must match');
