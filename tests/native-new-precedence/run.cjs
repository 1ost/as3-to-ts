...
const evidence=path.join(engine,'tests/nativeFlashOracle','new-expression-precedence');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-new-precedence');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Factory','Box','Other'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/cases',name+'.as'),'utf8');return ['cases.'+name,{source,sourceSha256:hash(source)}];}));
const source=sources['cases.Factory'].source;
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'new-expression-precedence',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['cases.Factory'],sources});
...
const driverFile=path.resolve('tests/native-new-precedence/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
