...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-optional-rest');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-optional-rest');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources={};function visit(dir){for(const file of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,file.name);if(file.isDirectory())visit(full);else if(file.name.endsWith('.as')&&file.name!=='GeneratedOptionalRestProbe.as'){const qname=path.relative(path.join(evidence,'source'),full).replaceAll('\\','.').replaceAll('/','.').slice(0,-3),source=fs.readFileSync(full,'utf8');sources[qname]={source,sourceSha256:hash(source)};}}}visit(path.join(evidence,'source'));assert.equal(Object.keys(sources).length,9);
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-optional-rest',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources});
...
for(const name of ['RestMethods','RestBase']){
 const source=sources['calls.'+name].source;
 for(const key of ['nativeTypedLocals','nativeTypedLocalReferenceModule','nativeComputedTypeTestModule']){
  assert.throws(()=>emit(parse(name+'.as',source),source,{...options,[key]:key==='nativeTypedLocals'?false:undefined}),/AS3_[A-Z_]+UNSUPPORTED/,name+': '+key);rejectionGuards++;
 }
}
for(const body of [
 'public function f(v:int=NaN):void{}',
 ...
 'public function f(...rest):void{rest++;}',
 'public function f(...rest):void{var rest:Array;}',
 'public function f(...rest):void{trace(arguments);}',
 'public function Guard(...rest){trace(arguments);}',
 'public function f(...rest):void{try{}catch(rest:*){rest=null;}}'
]){
...
const driverFile=path.resolve('tests/native-generated-optional-rest/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;
assert.equal(wanted.length,56);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='defaults-undefined').value[0][1]=7,v=>v.find(r=>r.id==='rebind-object').value[1]=[],v=>v.find(r=>r.id==='method-length').value[4]=1]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
