...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-security-catch');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-security-catch');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const driverFile=path.resolve('tests/native-generated-security-catch/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
