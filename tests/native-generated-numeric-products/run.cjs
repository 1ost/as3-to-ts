...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-numeric-products'),captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const base=path.join(compiler,'.cache/native-generated-numeric-products');fs.mkdirSync(base,{recursive:true});const run=fs.mkdtempSync(path.join(base,'run-'));
...
const sources={};
for(const q of ["productcases.Tracker","productcases.Products","productcases.Consumer","cn.kyiax.game.config.GameConfig"]){
...
const plan=api.createNativeGeneratedDeclarationPlan({scope:'native-generated-numeric-products',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources});
...
for(const [declaration,expression] of [
 ['public static const C:int=2+3;','Guard.C'],
 ['public static const C:int=(2*3);','Guard.C'],
 ['public static const C:int=2*make(); public static function make():int{return 3;}','Guard.C'],
 ['public static var V:int=3; public static const C:int=2*V;','Guard.C'],
 ['public static const C:String=2*3;','Guard.C'],
 ['public static const C:Boolean=2*3;','Guard.C'],
 ['public static const C:int=2*3;','Guard.C=7'],
 ['public static const C:int=2*3;','Guard.C++'],
 ['public static const C:int=2*3;','delete Guard.C']
]) {
 const source='package productcases {public class Guard {'+declaration+' public function check():*{return '+expression+';}}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'guard-products',providerModule:provider('AS3GeneratedClass'),sources:{'productcases.Guard':{source,sourceSha256:hash(source)}}});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}),/AS3_[A-Z_]+UNSUPPORTED/); guards++;
}
...
const wanted=captured;assert.equal(wanted.length,8);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='first-read').value[0]=0]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({status,guards,emitted,results,providerGraph,
 ...
 observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},comparisonNegativeControls:3,scope:'Four complete subjects including maintained OP2 GameConfig, eight AIR observations. Other computed primitive operators and operands remain held.'},null,2));
