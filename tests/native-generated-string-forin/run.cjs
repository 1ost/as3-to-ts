...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-string-forin');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-string-forin');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['stringloops.Enumeration'].source;
...
for(const changed of [
 source.replace('var key:String=null','var key:int=0'),
 ...
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'string-forin-guard',providerModule:provider('AS3GeneratedClass'),sources:{'stringloops.Enumeration':{source:changed,sourceSha256:hash(changed)}}});
...
const driverFile=path.resolve('tests/native-generated-string-forin/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
