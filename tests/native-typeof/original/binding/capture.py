...
out=Path(tempfile.mkdtemp(prefix='flash-',dir=here));shutil.copytree(here/'original',out/'sources');shutil.copyfile(here/'capture.cjs',out/'capture.cjs')
...
commands=[['java','-Xmx384m','-jar',str(flex/'lib/mxmlc.jar'),'+flexlib='+str(flex/'frameworks'),'-debug=true','-target-player=26.0','-swf-version=37','-source-path='+str(out/'sources'),'-output='+str(out/'oracle.swf'),str(out/'sources/MetadataOracle.as')],[str(electron),str(out/'capture.cjs'),str(out/'oracle.swf'),str(out/'flash.json'),str(plugin)]]
logs=[]
for cmd in commands:
...
v=json.loads((out/'flash.json').read_text(encoding='utf8'));assert not v['failure'],v
...
print(json.dumps({'output':str(out),'rows':v['rows']}))
