...
const cache=path.resolve('.cache/native-generated-private-interfaces-full');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['FileLocalInterfacesProbe','localinterfaces.First','localinterfaces.Second','contracts.IBase'])};
...
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   reject(()=>api.createNativeGeneratedDeclarationPlan({...input,classScriptSources:['localinterfaces.First']}),/multi-declaration Class script retry/);
   ...
   for(const [replacement,error] of [['IValue()',/interface coercion requires one argument/],['new IValue(value)',/interface token is not a source constructor/]]) {
    const source=sources['localinterfaces.First'].source.replace('IValue(value)',replacement);
    const invalid=api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,'localinterfaces.First':{source,sourceSha256:hash(source)}}});
    reject(()=>api.emitNativeSourceClassModule({...config,plan:invalid,emitterOptions:{...options,nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:invalid}}}),error);
   }
   const original=sources['localinterfaces.First'].source;
   for(const [from,to,error] of [
    ...
    ['public function set value(n:int):void','private function set value(n:int):void',/missing public instance interface member/],
    ['public function add(n:int):int','public function add(n:Number):int',/incompatible interface signature/],
    ['override public function add(n:int):int','override public function add(n:int):Number',/incompatible interface signature/],
    ['class Helper implements IValue','class Helper implements IValue,IValue',/duplicate file-private implements/],
    ['class Helper implements IValue','class Helper implements Missing',/exact source interface/]
   ]) {
    const source=original.replace(from,to);assert.notEqual(source,original);
    reject(()=>api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,'localinterfaces.First':{source,sourceSha256:hash(source)}}}),error);
   }
   assert.equal(guards,12);
   assert.equal(plan.privateBindings.filter(b=>b.interfaces.length).length,2);
   assert(plan.interfaceContracts.implementations.some(c=>c.owner==='localinterfaces.First#file:Child'&&c.implementationOwner==='localinterfaces.First#file:Helper'));
   ...
   assert.equal(artifact.generatedSources.length,Object.keys(sources).length+1+plan.privateBindings.length+plan.privateInterfaces.length);
   const files=[];
  ...
  const observer=fs.readFileSync(path.join(__dirname,'full-observer.ts'),'utf8').replaceAll('@FLASH@',modulePath(path.join(engine,'src/layaAir/flash'))).replaceAll('@ENGINE@',modulePath(engine));
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'full-observer.ts')))},null,2));
 console.log(JSON.stringify({out,status:'passed',observations:21,targets:2}));
}
