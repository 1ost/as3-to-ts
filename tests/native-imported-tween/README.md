...
missing query arguments remain held. Existing bare `to()` behavior is unchanged.
...
all handles from a snapshot. Four additional checks cover target expression
evaluation once, identity, target isolation and cancellation. The consumer and
