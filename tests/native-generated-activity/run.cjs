...
const evidence=path.resolve('../op2-html5/game-client-laya/tests/activity-step-generated'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
const driverFile=path.join(evidence,'runtime-driver.js'),driver=fs.readFileSync(driverFile,'utf8');
assert(fs.readFileSync(path.join(__dirname,'observer.ts'),'utf8').includes(driver.replace('globalThis.result=rows;','return {rows};')));
...
const cache=path.resolve('.cache/native-generated-activity');fs.mkdirSync(cache,{recursive:true});
...
const provenance=require(path.join(evidence,'source-provenance.json'));const cohorts={parent:read('source',provenance.files.map(f=>f.qname))};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const modules=['AS3StringIntrinsics','Dictionary','AS3GeneratedClass','AS3ScriptGlobal','AS3Type','AS3Class','AS3Invocation','AS3LexicalMembers','AS3Property','AS3MethodBinding','AS3Coercion','AS3String','AS3Addition','AS3ArrayCreation','NativeSourceClassLoadingSession'];
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const input={scope:'activity-'+cohort,sources,providers:{'flash.utils.Dictionary':{module:provider('Dictionary'),exportName:'Dictionary'}},patternProviderModule:provider('AS3StringIntrinsics'),providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};
   ...
   const options={customVisitors:[],definitionsByNamespace,nativeStringIntrinsicsModule:provider('AS3StringIntrinsics'),nativeStringLocalCoercionModule:provider('AS3String'),nativeEnumeration:{dictionaryModule:provider('Dictionary'),coercionModule:provider('AS3Coercion'),stringModule:provider('AS3String')},importModules:{'flash.utils.Dictionary':provider('Dictionary'),'compiler.AS3Class':provider('AS3Class'),'compiler.AS3Invocation':provider('AS3Invocation')},
  ...
  const build=mutation=>esbuild.build({plugins:mutation?[{name:'restore-host-array-read',setup(b){b.onLoad({filter:/parent-factory\.js$/},args=>{let contents=fs.readFileSync(args.path,'utf8');const pattern=mutation==='length'?/[A-Za-z0-9_]+\.as3GetProperty\(this\.items, \"length\"\)/g:/[A-Za-z0-9_]+\.as3GetProperty\(this\.fragments, _loc3_\)/g;assert.equal([...contents.matchAll(pattern)].length,mutation==='length'?2:target==='ES5'?3:4);contents=contents.replace(pattern,mutation==='length'?'this.items.length':'this.fragments[_loc3_]');return {contents,loader:'js'};});}}]:[],entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'}});
  ...
  const mutations=['length','index'];
  for(const mutation of mutations){const changed=(await build(mutation)).outputFiles[0].text;await assert.rejects(()=>execute(changed),e=>e.name==='TypeError'&&e.errorID===undefined&&/null/.test(e.message));}
  results.push({target,node,web,mutations,typechecks,artifacts,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,driverSha256:hash(driver),runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Document Sprite adapter','Full startup and game account flow']},null,2));
