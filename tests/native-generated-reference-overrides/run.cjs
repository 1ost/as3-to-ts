...
const evidence=path.join(engine,'tests/nativeFlashOracle/reference-method-overrides'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-reference-overrides');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['refov.IValue','refov.Value','refov.Base']),child:read('source',['refov.IValue','refov.Value','refov.Base','refov.Child','refov.Grandchild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const input={scope:'method-overrides-'+cohort,sources,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};
   const compilerGuards=require('../native-generated-method-overrides/guards.cjs')(api,input);assert.equal(compilerGuards,8);
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/child-factory\.js$/},args=>{
   ...
   if(mutation==='missing-signature'){
    const re=/,\s*instanceMethods:\s*\[[^\n]*?\],\s*declaration:/g;
    const matches=[...contents.matchAll(re)];assert.equal(matches.length,4);
    contents=contents.replace(re,',declaration:');
   }else if(mutation==='same-name-reference'){
    const provider=contents.match(/(AS3Type_\d+)\.defineAS3Interface\("refov::IValue", \[\]\)/);assert.ok(provider);
    assert.equal([...contents.matchAll(/exports\.interface0 =/g)].length,1);
    contents=contents.replace('exports.interface0 =','exports.__wrongInterface = '+provider[1]+'.defineAS3Interface("refov::IValue", []); exports.interface0 =');
    const re=/parameters: \[\{ name: "refov::IValue", reference: ([\w$]+)\.interface0 \}\]/g;
    assert.equal([...contents.matchAll(re)].length,3);
    contents=contents.replace(re,'parameters: [{ name: "refov::IValue", reference: $1.__wrongInterface }]');
   }else if(mutation==='wrong-reference-signature'){
    const re=/parameters: \[\{ name: "refov::IValue", reference: ([\w$]+)\.interface0 \}\]/g;
    assert.equal([...contents.matchAll(re)].length,3);
    contents=contents.replace(re,'parameters: [{ name: "refov::Value", reference: $1.type3 }]');
   }else if(mutation==='missing-parent-selection'){
    const re=/AS3ScriptGlobal_\d+\.selectAS3ScriptDomainClass\(cohortDomain_\d+\.scriptDomain, "[^"]+"\)/g;
    assert.equal([...contents.matchAll(re)].length,4);contents=contents.replace(re,'undefined');
  ...
  const node=await execute(code);assert.deepEqual(node.rows,expected);assert.equal(node.checks.length,7);
  ...
  const negatives=['missing-signature','missing-parent-selection','wrong-reference-signature','same-name-reference'];
  for(const mutation of negatives){
   const mutated=(await build(mutation)).outputFiles[0].text;
   await assert.rejects(()=>execute(mutated),mutation==='missing-parent-selection'?/cohort did not reuse selected declaration: refov::Base/:/AS3_GENERATED_CLASS_UNSUPPORTED: selected parent override/);
  }
  results.push({target,node,web,typechecks,artifacts,negatives,compilerGuards:8,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 ...
 console.log(JSON.stringify({out,status:'passed',observations:10,targets:2,realms:2}));
}
