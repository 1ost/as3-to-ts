...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-definition-lookup');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-lookup');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['LookupReader','ShadowReader','InheritedReader','WildcardReader'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/cases',name+'.as'),'utf8');return ['cases.'+name,{source,sourceSha256:hash(source)}];}));
const source=sources['cases.LookupReader'].source;
const consumerOnly=process.argv.includes('--consumer');
const consumers=new Set(consumerOnly?['cases.LookupReader','cases.WildcardReader']:[]);
...
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-definition-lookup',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
const options={customVisitors:[],importModules:{"flash.utils.getDefinitionByName":provider("DefinitionRegistry"),"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace:{...definitionsByNamespace,"flash.utils":["getDefinitionByName"]},
...
const {rejected:rejectionGuards}=require('./guards.cjs');
...
const names=['AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass','DefinitionRegistry','AS3FunctionReturn'];
...
const driverFile=path.resolve('tests/native-generated-lookup/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,18);
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({consumerOnly,combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Lookup function identity, qualified package-value syntax, full UIComponent']},null,2));console.log(JSON.stringify({run,consumerOnly,sourceClasses:4,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
