...
const evidence=path.join(engine,'tests/nativeFlashOracle/protected-super-fields'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-protected-super-fields');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['superfields.Base']),child:read('source',['superfields.Base','superfields.Child','superfields.Grandchild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const input={scope:'super-fields-'+cohort,sources,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};
   ...
   assert.equal(require('./guards.cjs')(api,input,config,hash),8);
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/child-factory\.js$/},args=>{
    ...
    const re=mutation==='wrong-visibility'?/"protected", false, true\)/g:/AS3ScriptGlobal_\d+\.selectAS3ScriptDomainClass\(cohortDomain_\d+\.scriptDomain, "[^"]+"\)/g;
    assert.ok([...contents.matchAll(re)].length>=3);
    contents=contents.replace(re,mutation==='wrong-visibility'?'"private", false, true)':'undefined');
  ...
  for(const mutation of ['wrong-visibility','missing-parent-selection'])await assert.rejects(()=>build(mutation).then(b=>execute(b.outputFiles[0].text)),mutation==='wrong-visibility'?/AS3_LEXICAL_UNSUPPORTED: source lexical lookup/:/cohort did not reuse selected declaration/);
  results.push({target,node,web,typechecks,artifacts,compilerGuards:8,mutations:2,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 ...
 console.log(JSON.stringify({out,status:'passed',observations:12,targets:2,realms:2}));
}
