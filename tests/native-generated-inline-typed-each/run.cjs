...
const evidence=path.join(engine,'tests/nativeFlashOracle/inline-typed-each'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-inline-typed-each');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['InlineTypedEachProbe','eachcases.Item'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   for(const [before,after] of [['var before:Boolean','var item:int;var before:Boolean'],['for each(var item:Item','for each(var item:Missing']]){
    const original=sources.InlineTypedEachProbe.source,source=original.replace(before,after);assert.notEqual(source,original);
    const changed=api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,InlineTypedEachProbe:{source,sourceSha256:hash(source)}}});
 ...
 console.log(JSON.stringify({out,status:'passed',observations:9,targets:2}));
}
