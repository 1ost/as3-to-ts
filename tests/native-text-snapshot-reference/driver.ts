...
import {TextSnapshot,TextSnapshotDeclaration} from 'ENGINE/src/layaAir/flash/utils/AS3CanonicalSpriteValueReferences';
...
(globalThis as any).done=(async()=>{
 ...
 const Holder:any=domain.getDefinition('TextSnapshotHolder');
 const rows:any[]=[],specs:any[]=[
 ['TextSnapshot',TextSnapshot,TextSnapshotDeclaration,new Sprite().textSnapshot,new Sprite().textSnapshot]];
 ...
 check(!as3Is(new Sprite(),token)&&!as3Is(new Sprite().transform,token),'display and other value family rejected');
