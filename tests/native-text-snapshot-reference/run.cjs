...
const evidence=path.join(engine,'tests/nativeFlashOracle','text-snapshot-reference');const expected=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-text-snapshot-reference');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const text=fs.readFileSync(path.join(evidence,'source/TextSnapshotHolder.as'),'utf8');const sources={'TextSnapshotHolder':{source:text,sourceSha256:hash(text)}};
const source=sources['TextSnapshotHolder'].source;
const applicationDomain=provider('AS3CanonicalSpriteValueReferences');
const names=['flash.text.TextSnapshot'];
const nativeProviders=Object.fromEntries(names.map(name=>[name,{module:applicationDomain,exportName:name.split('.').pop()}]));
const input={scope:'generated-text-snapshot-reference',inheritScriptClasses:true,providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},sources};
...
for(const patch of [
 ...
 {importModules:{...options.importModules,'flash.text.TextSnapshot':'./wrong'}},
 ...names.slice(1).map(name=>({importModules:{...options.importModules,[name]:'./wrong'}})),
]){
 assert.throws(()=>emit(parse('TextSnapshotHolder.as',text),text,{...options,...patch}),/AS3_[A-Z_]+UNSUPPORTED/,Object.keys(patch).join(','));bindingGuards++;
}
const altered=source=>{
 const changed=api.createNativeGeneratedDeclarationPlan({...input,sources:{TextSnapshotHolder:{source,sourceSha256:hash(source)}}});
...
for(const name of names.map(name=>name.split('.').pop())){
 ...
 assert.throws(()=>altered(text.replace('public class TextSnapshotHolder {','public class TextSnapshotHolder extends '+name+' {')),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);bindingGuards++;
}
(async()=>{
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  await page.route('http://text-snapshot-reference.test/**',route=>route.request().url().endsWith('/bundle.js')?route.fulfill({contentType:'text/javascript',body:code}):route.fulfill({contentType:'text/html',headers:{'Content-Security-Policy':"script-src 'self'"},body:'<script src="/bundle.js"></script>'}));
  await page.goto('http://text-snapshot-reference.test/');const web=await page.evaluate(async()=>{await globalThis.done;return globalThis.result;});await page.close();
 ...
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({scope:'complete generated TextSnapshotHolder source, native TextSnapshot reference binding; no generated native subclass or full Sprite admission',sourceSha256:hash(text),expected,results,bindingGuards},null,2));
