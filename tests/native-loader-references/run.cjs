...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-loader-references');const expected=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-loader-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const text=fs.readFileSync(path.join(evidence,'source/LoaderHolder.as'),'utf8');const sources={'LoaderHolder':{source:text,sourceSha256:hash(text)}};
const source=sources['LoaderHolder'].source;
const applicationDomain=provider('AS3CanonicalLoaderReferences');
const names=['flash.display.Loader','flash.display.Bitmap','flash.net.URLLoader','flash.media.Sound'];
const nativeProviders=Object.fromEntries(names.map(name=>[name,{module:applicationDomain,exportName:name.split('.').pop()}]));
const input={scope:'generated-generated-loader-references',inheritScriptClasses:true,providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},sources};
...
Object.assign(options,{nativeLoaderReferenceModule:applicationDomain,nativeReferenceCoercion:{plan,module:"./declarationDomain",coercionModule:provider("AS3Type")},nativeSignaturePropertyModule:provider("AS3Property"),nativeDynamicConstructionModule:provider('AS3Invocation')});
...
let bindingGuards=0;
for(const patch of [
 {nativeLoaderReferenceModule:undefined},
 {nativeLoaderReferenceModule:'./wrong'},
 {nativeReferenceCoercion:undefined},
 {nativeComputedTypeTestModule:undefined},
 {nativeGeneratedDeclarations:undefined},
 {importModules:{...options.importModules,'flash.display.Loader':'./wrong'}},
 ...names.slice(1).map(name=>({importModules:{...options.importModules,[name]:'./wrong'}})),
]){
 assert.throws(()=>emit(parse('LoaderHolder.as',text),text,{...options,...patch}),/AS3_[A-Z_]+UNSUPPORTED/,Object.keys(patch).join(','));bindingGuards++;
}
const altered=source=>{
 const changed=api.createNativeGeneratedDeclarationPlan({...input,sources:{LoaderHolder:{source,sourceSha256:hash(source)}}});
...
for(const name of names.map(name=>name.split('.').pop())){
 ...
 assert.throws(()=>altered(text.replace('public class LoaderHolder {','public class LoaderHolder extends '+name+' {')),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);bindingGuards++;
}
(async()=>{
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  assert.deepEqual(errors,[]);assert.deepEqual(web.rows,expected);assert.equal(web.guards,18);
  const mutated=module.moduleSource.replace(/\b[A-Za-z_$][\w$]*\.as3As\(input,\s*[A-Za-z_$][\w$]*\.Bitmap\)/g,'input');
  assert.notEqual(mutated,module.moduleSource,'Bitmap cast mutation applied');
  fs.writeFileSync(path.join(run,'mutated-module-'+target+'.js'),mutated);
  fs.writeFileSync(path.join(run,'module.js'),mutated);
  try {
   const broken=await esbuild.build({entryPoints:[path.join(run,'driver.ts')],bundle:true,write:false,platform:'browser',format:'iife',target:'es2020',loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},logLevel:'warning'});
   const mutationPage=await browser.newPage();
   try {
    await mutationPage.route('http://loader-mutation.test/**',route=>route.request().url().endsWith('/bundle.js')?route.fulfill({contentType:'text/javascript',body:broken.outputFiles[0].text}):route.fulfill({contentType:'text/html',headers:{'Content-Security-Policy':"script-src 'self'"},body:'<!doctype html><body><script src="/bundle.js"></script>'}));
    await mutationPage.goto('http://loader-mutation.test/');
    await assert.rejects(mutationPage.evaluate(async()=>{await globalThis.done;}),/prototype forgery/);
   } finally {await mutationPage.close();}
  } finally {fs.writeFileSync(path.join(run,'module.js'),module.moduleSource);}
  results.push({target,web,types,castRemovalMutations:1,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 }}finally{await browser.close();}
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({scope:'complete generated LoaderHolder source, four exact native reference bindings in real Laya Chromium; no generated native subclasses or full loader admission',sourceSha256:hash(text),expected,results,bindingGuards},null,2));
 console.log(JSON.stringify({run,targets:results.map(r=>r.target),rowsPerRuntime:expected.length,guardsPerRuntime:18,bindingGuards,castRemovalMutationsPerTarget:1,typeErrors:0}));
