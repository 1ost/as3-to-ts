...
const evidence=path.join(engine,'tests/nativeFlashOracle/xml-construction'),air=require(path.join(evidence,'verify.cjs'));
const callsOnly=false;
const expected=[...air,...require(path.join(engine,'tests/nativeFlashOracle/xml-construction-edges/verify.cjs')).map(row=>({...row,id:'edge-'+row.id}))];
...
const cache=path.resolve('.cache/native-generated-xml-construction');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['XMLBuilder'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const modules=['AS3CanonicalDisplayReference','AS3CanonicalEventConstruction','AS3CanonicalErrorConstruction','AS3CanonicalXMLReference','AS3XML','AS3JSON','AS3ReflectionQuery','describeType','AS3Vector','AS3StringIntrinsics','Dictionary','AS3GeneratedClass','AS3ScriptGlobal','AS3Type','AS3Class','AS3Invocation','AS3LexicalMembers','AS3Property','AS3MethodBinding','AS3Coercion','AS3String','AS3Addition','AS3ArrayCreation','NativeSourceClassLoadingSession'];
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const options={customVisitors:[],definitionsByNamespace,nativeGlobalModules:{trace,XML:provider('AS3CanonicalXMLReference'),XMLList:provider('AS3CanonicalXMLReference')},nativeVectorTypes:{plan,module:'./__native_declarations'},nativeStringIntrinsicsModule:provider('AS3StringIntrinsics'),nativeStringLocalCoercionModule:provider('AS3String'),nativeEnumeration:{dictionaryModule:provider('Dictionary'),coercionModule:provider('AS3Coercion'),stringModule:provider('AS3String')},importModules:{...Object.fromEntries(Object.entries(nativeProviders).map(([q,p])=>[q,p.module])),'flash.utils.describeType':provider('describeType'),'compiler.AS3Property':provider('AS3Property'),'compiler.AS3Class':provider('AS3Class'),'compiler.AS3Invocation':provider('AS3Invocation')},
    ...
    nativeXMLModule:provider('AS3XML'),nativeObjectPropertyModule:provider('AS3Property'),nativeTypedLocals:true,nativeTypedLocalReferenceModule:provider('AS3Type'),nativeTypedLocalAdditionModule:provider('AS3Addition'),nativeArrayCreationModule:provider('AS3ArrayCreation'),
   ...
   for(const transform of [s=>s.replace('new XML(value as String)','new XML(value)'),s=>s.replace('fromData(value:*)','fromData(value:*,String:Class)'),s=>s.replace('build(value:String)','build(value:String,XML:Class)'),s=>s.replace('new XML(value);','new XML(value,value);'),s=>s.replace('new XML(value);','new XMLList(value);')]){
    const source=transform(sources.XMLBuilder.source);assert.notEqual(source,sources.XMLBuilder.source);
    const guardPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{XMLBuilder:{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeVectorTypes:{plan:guardPlan,module:'./__native_declarations'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}}),/AS3_[A-Z_]+UNSUPPORTED/,source);rejectionGuards++;
   }
   for(const patch of [{nativeComputedTypeTestModule:undefined},{nativeXMLModule:undefined},{nativeGlobalModules:{trace,XML:'./wrong'}}]){
  ...
  const mutation=originalFactory.replace(/[A-Za-z_$][\w$]*\.as3As\(([^,()]+), String\)/g,'($1)');assert.notEqual(mutation,originalFactory);
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),callsOnly,held:['Full ResourceLoader and game account flow']},null,2));
