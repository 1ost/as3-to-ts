...
main().then(()=>{assert(!process.argv.includes('--expect-held'),'Expected compiler hold no longer occurs');}).catch(error=>{
 if(process.argv.includes('--expect-held')&&/AS3_GENERATED_LEXICAL_UNSUPPORTED: anonymous typed return held/.test(error.message)){console.log(JSON.stringify({status:'expected-compiler-hold',message:error.message}));return;}
