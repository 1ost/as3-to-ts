...
for(const changed of [
 ...
 source.replace('private var i:int','protected var i:int'),
 source.replace('private var i:int','private static var i:int'),
 source.replace('this.i++','delete this.i')
]){
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'numeric-update-guard',providerModule:provider('AS3GeneratedClass'),sources:{'updatecases.Counter':{source:changed,sourceSha256:hash(changed)}}});
