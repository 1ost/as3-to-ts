...
const evidence=path.join(engine,'tests/nativeFlashOracle','shared-object-local');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-shared-object');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='SharedObjectProbe.as'){
...
for(const [body,bound,error] of [
 ['return value;',false,/unresolved|reference.*qualification/],
 ['return;',true,/generated typed bare return/],
 ['if(value)return value;',true,/generated typed fallthrough completion/],
 ['if(value is SharedObject)return value;return null;',true,/reference type operation requires class-evaluation authority/]
]) {
 const source='package {import flash.net.SharedObject;public class Guard {public function Guard(){super();} public function echo(value:*):SharedObject {'+body+'}}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'shared-object-guard',providerModule:provider('AS3GeneratedClass'),sources:{Guard:{source,sourceSha256:hash(source)}},providers:bound?nativeProviders:{}});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}),error);
 rejectionGuards++;
}
...
const names=['SharedObject','EventDispatcherCore','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
const moduleFor=n=>'src/layaAir/flash/'+(n==='SharedObject'?'net':n==='EventDispatcherCore'?'events':['AS3SourceError','IllegalOperationError'].includes(n)?'errors':'utils')+'/'+n;
...
const driverFile=path.resolve('tests/native-generated-shared-object/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,28);
