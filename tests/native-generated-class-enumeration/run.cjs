...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-class-enumeration'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-class-enumeration');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['classenum.A','classenum.B','classenum.Scanner'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const reject=transform=>{
    const source=transform(sources['classenum.Scanner'].source),p=api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,'classenum.Scanner':{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:p,emitterOptions:{...options,nativeVectorTypes:{...options.nativeVectorTypes,plan:p},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p}}}),/AS3_TYPED_LOCAL_UNSUPPORTED/);rejectionGuards++;
   };
   reject(s=>s.replace(/var current:Class/g,'var current:Function'));
   reject(s=>s.replace(/var current:Class/g,'const current:Class'));
   reject(s=>s.replace('stop:Boolean=false','stop:Boolean=false,current:Class=null').replace('var current:Class=A;',''));
  ...
  const mutation=originalFactory.replace(/[A-Za-z_$][\w$]*\.as3CoerceClass\(([^()]*)\)/g,'$1');assert.notEqual(mutation,originalFactory);
