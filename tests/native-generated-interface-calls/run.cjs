...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-interface-calls');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-interface-calls');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InterfaceCallsProbe.as'){
...
for(const [body,missing,reason]of [
 ...
 const p=api.createNativeGeneratedDeclarationPlan({scope:'interface-calls-guard',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources:{...sources,'contracts.Guard':{source,sourceSha256:hash(source)}}});
 ...
 if(missing)delete opts[missing];assert.throws(()=>emit(parse('Guard.as',source),source,opts),reason);rejectionGuards++;
}
