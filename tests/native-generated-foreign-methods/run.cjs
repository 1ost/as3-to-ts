...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-foreign-methods');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-foreign-methods');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&['Target.as','Owner.as'].includes(item.name)){
...
const source=sources['methodcases.Owner'].source;
for(const changed of [
 source.replace('private var target:Target','private var target:*'),
 source.replace('private var target:Target','private var target:Object'),
 source.replace('this.target.work(value)','this.target.work=work'),
 source.replace('this.target.work(value)','delete this.target.work'),
 source.replace('this.target.work(value)','this.target.work++')
]) {
 assert.throws(()=>{
  const guardSources={...sources,'methodcases.Owner':{source:changed,sourceSha256:hash(changed)}};
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'foreign-method-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:guardSources});
...
assert.throws(()=>{
 const changed=sources['methodcases.Target'].source.replace('public function work','private function work');
 const guardSources={...sources,'methodcases.Target':{source:changed,sourceSha256:hash(changed)}};
 const guard=api.createNativeGeneratedDeclarationPlan({scope:'foreign-private-method-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:guardSources});
...
const driverFile=path.resolve('tests/native-generated-foreign-methods/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,9);
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Unknown/chained receivers, method writes/deletes and callable fields/accessors','Complete SoundManager and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:9,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
