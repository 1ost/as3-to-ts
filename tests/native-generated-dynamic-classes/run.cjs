...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-dynamic-classes');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-dynamic-classes');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Base','Record','Child','Sealed','Access'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/dynamiccase',name+'.as'),'utf8');return ['dynamiccase.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-dynamic-classes',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),sources});
...
const original=sources['dynamiccase.Record'].source;
for(const body of ['this[key]()','this[key]++','this[key]+=1','(this)[key]','loose','this.extra+=1']){
 const source=original.replace('return this[key];','return '+body+';'),changed={...sources,'dynamiccase.Record':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'dynamic-guard',providerModule:provider('AS3GeneratedClass'),sources:changed});
 assert.throws(()=>emit(parse('Record.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
for(const key of ['nativeDynamicPropertyReadsModule','nativeDynamicPropertyWritesModule']){
 assert.throws(()=>emit(parse('Record.as',original),original,{...options,[key]:undefined}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-dynamic-classes/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,21);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='constant-write').value[1]=0]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['arbitrary computed receivers, compound updates, bare unresolved names and dynamic calls']},null,2));console.log(JSON.stringify({run,sourceClasses:5,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
