...
const root=path.resolve('.cache/native-generated-json-receivers');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&['Reader.as','JSONToken.as'].includes(item.name)){
...
const reader=sources['jsoncases.Reader'].source;
for(const changed of [
 reader.replace('private var token:JSONToken','private var token:*'),
 reader.replace('private var token:JSONToken','private var token:Object'),
 reader.replace('this.token.value=value','this.token.value+=value'),
 reader.replace('this.token.value=value','this.token.value++'),
 reader.replace('this.token.value=value','delete this.token.value'),
 reader.replace('this.token.value=value','this.token.value()')
]){
 const changedSources={...sources,'jsoncases.Reader':{source:changed,sourceSha256:hash(changed)}};
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'foreign-receiver-guard',providerModule:provider('AS3GeneratedClass'),sources:changedSources});
...
const driverFile=path.resolve('tests/native-generated-json-receivers/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured.filter(row=>row.id.startsWith('receiver-'));assert.equal(wanted.length,5);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='receiver-read').value[0]='wrong']){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Foreign receiver calls, compound writes, updates and deletes','Remaining 34 JSON-library observations and full application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:5,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
