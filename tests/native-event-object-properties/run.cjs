...
const evidence=path.join(engine,'tests/nativeFlashOracle/event-object-properties'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-event-object-properties');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['eventprops.Reader','eventprops.Subject'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const modules=['AS3GeneratedClass','AS3ScriptGlobal','AS3Type','AS3Class','AS3Invocation','AS3LexicalMembers','AS3Property','AS3MethodBinding','AS3Coercion','AS3String','AS3Addition','AS3ArrayCreation','NativeSourceClassLoadingSession','AS3CanonicalEventConstruction','AS3CanonicalEventDispatcherConstruction'];
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const providers={'flash.events.Event':{module:provider('AS3CanonicalEventConstruction'),exportName:'Event',nativeBase:'Event'},'flash.events.EventDispatcher':{module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'EventDispatcher',nativeBase:'EventDispatcher'}};
   const input={scope:'event-object-properties',sources,providers,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};

   const plan=api.createNativeGeneratedDeclarationPlan(input);
   const definitionsByNamespace={};for(const q of [...Object.keys(sources),...Object.keys(providers)]){const parts=q.split('.'),n=parts.pop();(definitionsByNamespace[parts.join('.')]??=[]).push(n);}
   const options={nativeObjectPropertyModule:provider('AS3Property'),customVisitors:[],definitionsByNamespace,importModules:{'compiler.AS3Property':provider('AS3Property'),...Object.fromEntries(Object.entries(providers).map(([q,b])=>[q,b.module])),'compiler.AS3Class':provider('AS3Class'),'compiler.AS3Invocation':provider('AS3Invocation')},
   ...
   assert.equal(require('./guards.cjs')(api,input,config,hash),3);
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/subject-factory\.js$/},args=>{
   let contents=fs.readFileSync(args.path,'utf8');
   if(mutation==='raw-getter') {
    const re=/AS3Property_\d+\.as3GetProperty\(event, "(target|currentTarget)"\)/g;assert.ok([...contents.matchAll(re)].length>=8);
    contents=contents.replace(re,'event.$1');
   }else if(mutation==='skip-argument-effects'){
    const re=/__as3_generated_lexicalProvider_\d+\.as3CallLexicalMember\((?:_this|this), __as3_generated_access_\d+, (?:function \(\) \{ return \[\]; \}|\(\) => \[\])\)/g;
    assert.ok([...contents.matchAll(re)].length>=4);contents=contents.replace(re,'"argument"');
  ...
  const vm=require('node:vm');const execute=async code=>{const context=vm.createContext({performance,console,setTimeout,clearTimeout,AbortController,AbortSignal,DOMException});context.window=context;context.document={};new vm.Script(code).runInContext(context);await context.completion;return JSON.parse(JSON.stringify(context.result));};
  const node=await execute(code);assert.deepEqual(node.rows,expected);assert.equal(node.checks.length,3);
  ...
  const negatives=['raw-getter','skip-argument-effects'];
  for(const mutation of negatives){
   const mutated=(await build(mutation)).outputFiles[0].text;
   const result=await execute(mutated);assert.notDeepEqual(result.rows,expected,mutation);
   if(mutation==='raw-getter')assert.notDeepEqual(result.rows[0].error,expected[0].error);
   else assert.equal(result.rows[1].calls,0);
  }
  results.push({target,node,web,typechecks,artifacts,negatives,compilerGuards:3,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 ...
 console.log(JSON.stringify({out,status:'passed',observations:11,targets:2,realms:2}));
}
