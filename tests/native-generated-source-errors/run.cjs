...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-source-errors');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-source-errors');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='SourceErrorsProbe.as'){
...
function guardSource(body){
 const source='package errorcases {public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'source-error-guard',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),sources:{...sources,'errorcases.Guard':{source,sourceSha256:hash(source)}}});
...
for(const name of ['Error','ArgumentError','ReferenceError'])for(const args of ['', '"message",7']){
 assert.throws(()=>guardSource('public function run():*{return '+name+'('+args+');}'),/AS3_SOURCE_ERROR_UNSUPPORTED/);rejectionGuards++;
}
for(const body of ['public function run():void{var fn:Function;fn += "x";}',
 'public function run():void{var fn:Function;fn++;}',
 'public function run():void{const fn:Function=null;}',
 'public function run():void{try{throw ArgumentError("x");}catch(e:ArgumentError){}}']){
 assert.throws(()=>guardSource(body),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const name of ['Error','ArgumentError','ReferenceError']){
 const output=guardSource('public function run('+name+':Function):*{return '+name+'("shadow");}');
 assert.ok(!output.includes('as3Create'+name),'source parameter must shadow builtin');rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-source-errors/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,52);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='missing').value[2]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Subclass-specific typed catches, Error Class values and Error inheritance','Full EMVC and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:52,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
