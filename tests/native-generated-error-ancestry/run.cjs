...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-error-ancestry');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-error-ancestry');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='ErrorAncestryProbe.as'){
...
const reject=(source,providers=nativeProviders)=>{
 assert.throws(()=>{
  const guardPlan=api.createNativeGeneratedDeclarationPlan({scope:'error-guard',providerModule:provider('AS3GeneratedClass'),providers,sources:{Guard:{source,sourceSha256:hash(source)}}});
  emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:guardPlan,module:'./guard'},nativeReferenceCoercion:{plan:guardPlan,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
};
const guardSource='package {public class Guard extends Error {public function Guard(){super("g");}}}';
reject(guardSource,{Error:{module:provider('AS3CanonicalErrorConstruction'),exportName:'Error'}});
reject(guardSource,{Error:{module:provider('AS3CanonicalErrorConstruction'),exportName:'Error',nativeBase:'Date'}});
reject(guardSource,{Error:{module:provider('AS3CanonicalErrorConstruction'),exportName:'Fake',nativeBase:'Error'}});
reject(guardSource,{'other.Error':{module:provider('AS3CanonicalErrorConstruction'),exportName:'Error',nativeBase:'Error'}});
for(const body of ['public function Guard(){}','public function Guard(){super("g");super("g");}',
 'public function Guard(){if(true)super("g");}',
 'public function Guard(){super("g");} public var message:String;',
 'public function Guard(){super("g");} public function inspect():*{return super.getStackTrace();}'])
 reject('package {public class Guard extends Error {'+body+'}}');
...
const driverFile=path.resolve('tests/native-generated-error-ancestry/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured.filter(row=>!row.id.startsWith('metadata-'));assert.equal(wanted.length,23);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='entry-before').value[2]=99]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
