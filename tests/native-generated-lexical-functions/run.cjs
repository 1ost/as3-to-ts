...
const evidence=path.join(engine,'tests/nativeFlashOracle/lexical-function-intrinsics');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-lexical-functions');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources={};function visit(dir){for(const file of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,file.name);if(file.isDirectory())visit(full);else if(file.name.endsWith('.as')&&file.name!=='LexicalFunctionFieldsProbe.as'){const qname=path.relative(path.join(evidence,'source'),full).replaceAll('\\','.').replaceAll('/','.').slice(0,-3),source=fs.readFileSync(full,'utf8');sources[qname]={source,sourceSha256:hash(source)};}}}visit(path.join(evidence,'source'));assert.equal(Object.keys(sources).length,3);
const plan=api.createNativeGeneratedDeclarationPlan({scope:'lexical-function-intrinsics',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),sources});
...
const withoutGlobal=api.createNativeGeneratedDeclarationPlan({scope:'without-global',providerModule:provider('AS3GeneratedClass'),sources});
assert.throws(()=>emit(parse('FieldCalls.as',sources['callbacks.FieldCalls'].source),sources['callbacks.FieldCalls'].source,{...options,nativeGeneratedDeclarations:{plan:withoutGlobal,module:'./declarationDomain'},...(combined?{nativeReferenceCoercion:{plan:withoutGlobal,module:'./declarationDomain',coercionModule:provider('AS3Type')}}:{})}),/caller script global authority/);rejectionGuards++;
for(const body of ['protected var listener:*;public function f():*{return listener();}', 'public static var count:int=1;', 'protected var listener:Function;public function f():*{return listener.apply;}']){
 const source='package guard {public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'guard',providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),sources:{'guard.Guard':{source,sourceSha256:hash(source)}}});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},...(combined?{nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}:{})}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const selection of [{scriptGlobalSources:['callbacks.FieldCalls']},{scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['missing.Class']},{scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['callbacks.FieldCalls','callbacks.FieldCalls']}]){
 assert.throws(()=>api.createNativeGeneratedDeclarationPlan({scope:'bad-selection',providerModule:provider('AS3GeneratedClass'),sources,...selection}),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);rejectionGuards++;
}
...
const names=['AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass','getQualifiedClassName'];
...
const driverFile=path.resolve('tests/native-generated-lexical-functions/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;
assert.equal(wanted.length,38);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='inherited').value[0][0]='bare-receiver',v=>v.find(r=>r.id==='qualified-replace').value[2][1]='original',v=>v.find(r=>r.id==='null-receiver').value[2]=[]]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
try{for(const target of [ts.ScriptTarget.ES5,ts.ScriptTarget.ES2015]){
 ...
 const nodeDomainChecks=globalThis.domainChecks;assert.equal(nodeDomainChecks.length,9);const page=await browser.newPage();await page.addScriptTag({content:script});const web=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.result)));const webDomainChecks=await page.evaluate(()=>globalThis.domainChecks);assert.deepEqual(webDomainChecks,nodeDomainChecks);await page.close();
 for(const actual of [node,web]){assert.deepEqual(actual,wanted);}
 results.push({target,node,web,domainChecks:nodeDomainChecks});
}}finally{await browser.close();}
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:5,held:['Full error text and builtin Error catch identity are not compared','Static-initializer retry script identity and arbitrary global reflection remain held','SlotList.NIL and full Signal runtime remain prerequisites']},null,2));console.log(JSON.stringify({run,sourceClasses:3,sourceInterfaces:0,airRows:38,domainChecks:9,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],typeErrors:0}));
