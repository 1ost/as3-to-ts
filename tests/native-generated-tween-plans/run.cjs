...
const evidence=path.join(engine,'src/extensions/greensock/bezier-tests/evidence/original');
for(const file of JSON.parse(fs.readFileSync(path.join(evidence,'packet-files.json'))).files||JSON.parse(fs.readFileSync(path.join(evidence,'packet-files.json'))))assert.equal(hash(fs.readFileSync(path.join(evidence,file.path))),file.sha256);
assert.deepEqual(JSON.parse(fs.readFileSync(path.join(__dirname,'cases.json'))),JSON.parse(fs.readFileSync(path.join(evidence,'capture-a/flash.json'))).cases);
...
const capture=name=>JSON.parse(fs.readFileSync(path.join(evidence,name,'flash.json'))).rows;
assert.deepEqual(capture('capture-a'),capture('capture-b'));
const expected=capture('capture-a').filter(r=>r.id.includes('-object-'));assert.equal(expected.length,42);
const cache=path.resolve('.cache/native-generated-tween-plans');fs.mkdirSync(cache,{recursive:true});const out=fs.mkdtempSync(path.join(cache,'run-'));
const source=fs.readFileSync(path.join(__dirname,'BezierMigration.as'),'utf8');
const cohorts={subject:{BezierMigration:{source,sourceSha256:hash(source)},Unrelated:{source:'package { public class Unrelated {} }',sourceSha256:hash('package { public class Unrelated {} }')}}};
const parse=require('../../lib/parse'),K=require('../../lib/syntax/nodeKind').default;
const calls=[];function walk(n){if(!n)return;if(n.kind===K.CALL&&source.slice(n.start,n.end).startsWith('TweenMax.to('))calls.push({start:n.start,end:n.end,callSha256:hash(source.slice(n.start,n.end)),initialization:['scaleX','alpha','bezier','scaleY']});n.children.forEach(walk);}walk(parse('BezierMigration.as',source));assert.equal(calls.length,2);
const sourcePlans={BezierMigration:{source,calls}};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const artifacts={},typechecks=[];let guardCount=0;
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider),tweenModule];
   ...
   const config={plan,target,tweenSourcePlans:sourcePlans,emitterOptions:options,externalModules,loadingSessionModule:provider('NativeSourceClassLoadingSession')};
   guardCount=require('./guards.cjs')(api,config,sourcePlans,hash);assert.equal(guardCount,13);
  ...
  fs.writeFileSync(path.join(dir,'observer.ts'),observer);fs.copyFileSync(path.join(__dirname,'cases.json'),path.join(dir,'cases.json'));
  ...
  const changed=original.replace(/__vars.sourcePlan = [^;]+;/g,'');assert.notEqual(changed,original);
  ...
  try {
   ...
   await assert.rejects(()=>require(path.join(dir,'mutated.cjs')).completion,/Bezier requires an explicit source property plan/);
  } finally {fs.writeFileSync(factory,original);}
  results.push({target,web,node,typechecks,artifacts,compilerGuards:guardCount,mutation:'removed plan rejects Bezier',inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
  console.log(JSON.stringify({target,observations:web.rows.length,typeErrors:0,compilerGuards:guardCount}));
 }}finally{await browser.close();}
 for(const item of [...compilerInputs,...results.flatMap(r=>[...r.inputs,...r.typechecks.flatMap(c=>c.inputs)])])assert.equal(hash(fs.readFileSync(item.file)),item.sha256,item.file);
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,sourcePlans,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts')))},null,2));
 console.log(JSON.stringify({out,status:'passed',observations:42,targets:2}));
}
