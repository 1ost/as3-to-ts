...
const evidence=path.join(engine,'tests/nativeFlashOracle/point-returns'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-point-returns');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['cases.PointReturns'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const pointModule=modulePath(path.join(engine,'src/layaAir/flash/utils/AS3CanonicalPointReference.ts'));
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider),pointModule];
   const input={scope:'point-returns-'+cohort,sources,providers:{'flash.geom.Point':{module:pointModule,exportName:'Point'}},providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};
   ...
   const options={customVisitors:[],definitionsByNamespace,nativePointReferenceModule:pointModule,importModules:{'flash.geom.Point':pointModule,'compiler.AS3Class':provider('AS3Class'),'compiler.AS3Invocation':provider('AS3Invocation')},
  ...
  const build=(mutate=false)=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutate?[{name:'remove-return-coercion',setup(b){b.onLoad({filter:/subject-factory\.js$/},args=>{
   ...
   const re=/__as3_callable_generatedProperty\.coerceAS3PropertyValue\(value, \{ name: "flash\.geom::Point", reference: __as3_callable_declarationDomain\.native\d+ \}\)/g;
  ...
  await assert.rejects(()=>execute(mutated.outputFiles[0].text),/forged Point rejected/);
