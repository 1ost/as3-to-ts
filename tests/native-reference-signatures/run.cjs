...
const engine=path.resolve('../LayaAir-op2'),evidence=path.join(engine,'tests/nativeFlashOracle/reference-signatures');
...
const base=path.resolve('.cache/native-reference-signatures');fs.mkdirSync(base,{recursive:true});const run=fs.mkdtempSync(path.join(base,'run-'));
...
const source=fs.readFileSync(path.join(evidence,'source/ReferenceSignaturesProbe.as'),'utf8');
const sources={ReferenceSignaturesProbe:{source,sourceSha256:hash(source),referenceOnly:true}};
...
const options={customVisitors:[],definitionsByNamespace:{'':['ReferenceSignaturesProbe']},decoratorModules:{bound:helpers.bound,classBound:helpers.classBound},nativeClassHelperModules:{nativeClass:helpers.nativeClass,callableClass:helpers.callableClass},nativeGlobalModules:{Date:provider('AS3Date')},nativeComputedTypeTestModule:provider('AS3Type'),nativeNumericMethodParametersModule:provider('AS3Coercion'),nativeSourceErrorModule:modulePath(path.join(engine,'src/layaAir/flash/errors/AS3SourceError.ts')),nativeReferenceCoercion:{plan,module:'./domain',coercionModule:provider('AS3Type')}};
fs.writeFileSync(path.join(run,'domain.ts'),plan.moduleSource);
const output=emit(parse('ReferenceSignaturesProbe.as',source),source,options);
fs.writeFileSync(path.join(run,'ReferenceSignaturesProbe.ts'),output);
const emitted=[{name:'ReferenceSignaturesProbe',sourceSha256:hash(source),generatedSha256:hash(output)}];
let guards=0;
...
for(const body of [
 ...
 'public function f(value:Date,other:String):Date {return value;}',
 ...
 'public function f(value:Date):String {return "";}',
])reject(body);
...
fs.writeFileSync(path.join(run,'driver.ts'),"import {ReferenceSignaturesProbe} from './ReferenceSignaturesProbe';\nexport function run(){return JSON.parse(JSON.stringify(new ReferenceSignaturesProbe().snapshot().observations));}\n");
...
async function main(){
 ...
 console.log('Reference signatures: '+guards+' guards, 29 AIR observations from unchanged source, Node/Chromium ES5/ES2015. '+run);
}
