...
const evidence=path.join(engine,'tests/nativeFlashOracle','json-parse-failures');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-json-parse');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Parser'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/jsoncase',name+'.as'),'utf8');return ['jsoncase.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-json-parse',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['jsoncase.Parser'],sources});
...
for(const expression of ['JSON.parse()','JSON.parse(text,reviver,1)','JSON.parse(3)','JSON.parse(text,3)']){
 const source=sources['jsoncase.Parser'].source.replace('JSON.parse(text,reviver)',expression),changed={...sources,'jsoncase.Parser':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'json-guard',providerModule:provider('AS3GeneratedClass'),sources:changed});
 assert.throws(()=>emit(parse('Parser.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}),/AS3_JSON_UNSUPPORTED/);rejectionGuards++;
}
for(const module of ['', 'bad"module']){const source=sources['jsoncase.Parser'].source;assert.throws(()=>emit(parse('Parser.as',source),source,{...options,nativeJSONModule:module}));rejectionGuards++;}
const shadow='package {public class Shadow {public function test(JSON:Object):* {return JSON.parse("3");}}}';
assert.ok(!emit(parse('Shadow.as',shadow),shadow,{nativeJSONModule:provider('AS3JSON')}).includes('parseSourceJSON'));rejectionGuards++;
...
const names=['AS3JSON','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-json-parse/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,35);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='invalid-0').value[1]=0]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
