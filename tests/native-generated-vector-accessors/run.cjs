...
const evidence=path.join(engine,'tests/nativeFlashOracle/vector-accessors'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-vector-accessors');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['vectoraccess.Subject'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const input={scope:'class-script-retry-'+cohort,sources,providers:nativeProviders,vectorProviderModule:provider('AS3Vector'),patternProviderModule:provider('AS3StringIntrinsics'),providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['vectoraccess.Subject'],lexicalProviderModule:provider('AS3LexicalMembers')};
  ...
  const mutation=originalFactory.replaceAll('\"en_Eu\"','\"changed\"');assert.notEqual(mutation,originalFactory);
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Accessor overrides and complete Vector reflection','Full startup and game account flow']},null,2));
