...
const evidence=path.join(engine,'tests/nativeFlashOracle','movieclip-vectors');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-movieclip-vectors');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='MovieVectorProbe.as'){
...
const nativeProviders={'flash.display.MovieClip':{module:provider('AS3CanonicalMovieClipReference'),exportName:'MovieClip',nativeVector:true}};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'movieclip-vectors',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
for(const binding of [undefined,{...nativeProviders['flash.display.MovieClip'],nativeVector:undefined},{...nativeProviders['flash.display.MovieClip'],exportName:'Sprite'}]){
 assert.throws(()=>api.createNativeGeneratedDeclarationPlan({scope:'native-vector-guard',providerModule:provider('AS3GeneratedClass'),vectorProviderModule:provider('AS3Vector'),sources,providers:binding?{'flash.display.MovieClip':binding}:{}}),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);rejectionGuards++;
}
for(const body of ['public function make(MovieClip:*):Vector.<MovieClip>{return new Vector.<MovieClip>();}','public function make(v:*):Vector.<MovieClip>{return new Vector.<MovieClip>(v);}','public var v:Vector.<Vector.<MovieClip>>;']){
 const source='package movievectors {import flash.display.MovieClip;public class Guard {'+body+'}}';
 assert.throws(()=>{const p=api.createNativeGeneratedDeclarationPlan({scope:'native-vector-guard',providerModule:provider('AS3GeneratedClass'),vectorProviderModule:provider('AS3Vector'),sources:{'movievectors.Guard':{source,sourceSha256:hash(source)}},providers:nativeProviders});return emit(parse('Guard.as',source),source,{...options,nativeVectorTypes:{plan:p,module:'./guard'},nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}});},/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
for(const patch of [{importModules:{...options.importModules,'flash.display.MovieClip':'./wrong-provider'}},{nativeVectorTypes:{plan:{...plan},module:'./declarationDomain'}}]){
 const source=sources['movievectors.Consumer'].source;
 assert.throws(()=>emit(parse('Consumer.as',source),source,{...options,nativeVectorTypes:{plan,module:'./declarationDomain'},...patch}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-movieclip-vectors/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,39);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='default').value=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
