...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-bytearray-methods');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-bytearray-methods');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name==='Owner.as'){
...
const nativeProviders={'flash.utils.ByteArray':{module:provider('AS3CanonicalByteArrayReference'),exportName:'ByteArray'},'flash.utils.Dictionary':{module:provider('Dictionary'),exportName:'Dictionary'}};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-bytearray-methods',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
options.nativeByteArrayReferenceModule=provider('AS3CanonicalByteArrayReference');
let rejectionGuards=0;
const source=sources['bytecalls.Owner'].source;
for(const key of ['nativeByteArrayReferenceModule','nativeReferenceCoercion']){
 assert.throws(()=>emit(parse('Owner.as',source),source,{...options,[key]:undefined}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const changed of [
 source.replace('private var content:ByteArray','private var content:*'),
 source.replace('private var content:ByteArray','private var content:Object'),
 source.replace('this.content.compress(this.argument(value))','this.content.compress=this.compress'),
 source.replace('this.content.compress(this.argument(value))','delete this.content.compress'),
 source.replace('this.content.compress(this.argument(value))','this.content.compress++'),
 source.replace('this.content.compress(this.argument(value))','this.content.child.compress(this.argument(value))')
]){
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'bytearray-method-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'bytecalls.Owner':{source:changed,sourceSha256:hash(changed)}}});
...
for(const override of [
 {nativeByteArrayReferenceModule:provider('ByteArray')},
 {importModules:{...options.importModules,'flash.utils.ByteArray':provider('ByteArray')}}
]){assert.throws(()=>emit(parse('Owner.as',source),source,{...options,...override}),/AS3_BYTEARRAY_REFERENCE_UNSUPPORTED/);rejectionGuards++;}
...
const names=['AS3CanonicalByteArrayReference','ByteArray','AS3CanonicalErrorConstruction','trace','Dictionary','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-bytearray-methods/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,16);
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['ByteArray construction, arbitrary native members and unknown/chained receivers','Complete Zip/ZipFile and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:16,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
