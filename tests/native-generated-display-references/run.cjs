...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-display-references');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-display-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='DisplayReferencesProbe.as'){
...
const options={customVisitors:[],nativeDisplayObjectReferenceModule:provider('AS3CanonicalDisplayReference'),importModules:{"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
const reader=sources['displaycases.Reader'].source;
for(const patch of [
...
for(const body of [
 'public static var value:Boolean=null is DisplayObject;',
 'public static var value:DisplayObject=null as DisplayObject;',
 'public function test(value:*,DisplayObject:Class):Boolean{return value is DisplayObject;}',
...
const names=['AS3CanonicalDisplayReference','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-display-references/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
