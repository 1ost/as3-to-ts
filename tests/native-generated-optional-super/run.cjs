...
const evidence=path.join(engine,'tests/nativeFlashOracle/optional-super'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-optional-super');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['optsuper.Base']),child:read('source',['optsuper.Base','optsuper.Child','optsuper.Grandchild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   assert.equal(require('./guards.cjs')(api,input,config,hash),6);
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/child-factory\.js$/},args=>{
   ...
   if(mutation==='pad-omitted'){
    const re=/(__as3_callable_superMethod0, [^,]+, )(__as3_callable_superCallArguments)(\))/g;
    assert.equal([...contents.matchAll(re)].length,5);
    contents=contents.replace(re,'$1($2.length ? $2 : [void 0,void 0])$3');
  ...
  const negatives=['pad-omitted','missing-parent-selection'];
  for(const mutation of negatives){
   const mutated=(await build(mutation)).outputFiles[0].text;
   if(mutation==='pad-omitted')assert.notDeepEqual((await execute(mutated)).rows,expected);
   else await assert.rejects(()=>execute(mutated),/cohort did not reuse selected declaration: optsuper::Base/);
  }
  results.push({target,node,web,typechecks,artifacts,negatives,compilerGuards:14,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
