...
export async function run(parentModule:NativeSourceClassModule,childModule:NativeSourceClassModule){
 ...
 const Base=parent.getDefinition('optsuper.Base') as Function;
 ...
 const Child=loaded.getDefinition('optsuper.Child') as Function,Grandchild=loaded.getDefinition('optsuper.Grandchild') as Function;
 check('selected parent reused',loaded.getDefinition('optsuper.Base')===Base&&Object.getPrototypeOf(Child.prototype)===Base.prototype);
 ...
 check('sibling class distinct',sibling.getDefinition('optsuper.Child')!==Child);
