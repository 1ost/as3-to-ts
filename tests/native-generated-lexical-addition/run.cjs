...
const evidence=path.join(engine,'tests/nativeFlashOracle/lexical-property-addition'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-lexical-addition');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['lexadd.Adder','lexadd.Slot'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const input={scope:'class-script-retry-'+cohort,sources,providers:nativeProviders,vectorProviderModule:provider('AS3Vector'),patternProviderModule:provider('AS3StringIntrinsics'),providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['lexadd.Adder','lexadd.Slot'],lexicalProviderModule:provider('AS3LexicalMembers')};
   ...
   for(const operation of ['value[key] -= rhs()','value[key]++']){
    const source='package lexadd {public class Adder {public function add(value:Object,key:*,rhs:Function):* {return '+operation+';}}}';
    const badPlan=api.createNativeGeneratedDeclarationPlan({...input,classScriptSources:['lexadd.Adder'],sources:{'lexadd.Adder':{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:badPlan,emitterOptions:{...options,nativeVectorTypes:{...options.nativeVectorTypes,plan:badPlan},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:badPlan}}}),/AS3_.*UNSUPPORTED/);rejectionGuards++;
   }
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Other compound operators and computed receiver families','Full startup and game account flow']},null,2));
