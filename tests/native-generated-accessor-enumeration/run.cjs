...
const evidence=path.join(engine,'tests/nativeFlashOracle/accessor-enumeration'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-accessor-enumeration');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['accessorenum.Subject'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const input={scope:'class-script-retry-'+cohort,sources,providers:nativeProviders,vectorProviderModule:provider('AS3Vector'),patternProviderModule:provider('AS3StringIntrinsics'),providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['accessorenum.Subject'],lexicalProviderModule:provider('AS3LexicalMembers')};
   ...
   for(const [body,reason] of [
    ['for(key in _map){}','for-in target has no source local ownership'],
    ['var key:int=0;for(key in _map){}','typed for-in target held'],
    ['var key:String=null;try{}catch(key:*){for(key in _map){}}','catch-shadow for-in target held']
   ]){
    const source='package accessorenum { public class Subject { private var _map:Object; public function get keys():Object {'+body+'return null;} } }';
    const badPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{'accessorenum.Subject':{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:badPlan,emitterOptions:{...options,nativeVectorTypes:{...options.nativeVectorTypes,plan:badPlan},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:badPlan}}}),error=>error.message.includes(reason));rejectionGuards++;
   }
  ...
  const mutation=originalFactory.replaceAll('.next()).done', '.next()).done && false');assert.notEqual(mutation,originalFactory);
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Accessor closures and enumeration targets outside the admitted local types','Full startup and game account flow']},null,2));
