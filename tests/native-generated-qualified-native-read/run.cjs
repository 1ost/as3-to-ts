...
const evidence=path.join(engine,'tests/nativeFlashOracle/qualified-native-read'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-qualified-native-read');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['qualifiedread.Subject'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const nativeProviders={'flash.system.Security':{module:modulePath(path.join(engine,'src/layaAir/flash/system/Security.ts')),exportName:'Security'},'flash.events.ErrorEvent':{module:provider('AS3CanonicalErrorEventReference'),exportName:'ErrorEvent'}};
   ...
   const q='qualifiedread.Subject';
   const emitSource=source=>{const guardPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{[q]:{source,sourceSha256:hash(source)}}});return api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeVectorTypes:{plan:guardPlan,module:'./__native_declarations'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}});};
   for(const body of ['flash.system.Security.sandboxType="x";','flash.system.Security.allowDomain("*");','return flash.system.Security;']){
    assert.throws(()=>emitSource('package qualifiedread {public class Subject { public function run():*{'+body+'}}}'),/AS3_(QUALIFIED_NATIVE_READ|CLASS_INITIALIZER)_UNSUPPORTED/);rejectionGuards++;
   }
   for(const declaration of ['public function run(flash:Object):*{return flash.system.Security.sandboxType;}','private var flash:Object;public function run():*{return flash.system.Security.sandboxType;}']){
    assert.throws(()=>emitSource('package qualifiedread {public class Subject {'+declaration+'}}'),/AS3_CLASS_INITIALIZER_UNSUPPORTED/);rejectionGuards++;
   }
   assert.throws(()=>api.emitNativeSourceClassModule({...config,emitterOptions:{...options,importModules:{...options.importModules,'flash.system.Security':'./wrong'}}}),/exact native import binding/);rejectionGuards++;
  ...
  const mutation=originalFactory.replace(/\.REMOTE/g,'.APPLICATION');assert.notEqual(mutation,originalFactory);
  try{fs.writeFileSync(factoryFile,mutation);const changed=await build();assert.notDeepEqual((await execute(changed.outputFiles[0].text)).rows,expected);}finally{fs.writeFileSync(factoryFile,originalFactory);}
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['Qualified native mutation, invocation and standalone Class values','Full startup and game account flow']},null,2));
