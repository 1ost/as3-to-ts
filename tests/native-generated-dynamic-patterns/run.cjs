...
const base=path.join(compiler,'.cache/native-generated-dynamic-patterns');fs.mkdirSync(base,{recursive:true});const run=fs.mkdtempSync(path.join(base,'run-'));
...
const sources={};
for(const q of ["patterns.DynamicReplace"]){
...
const plan=api.createNativeGeneratedDeclarationPlan({scope:'native-generated-dynamic-patterns',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources});
...
const options={nativeStringIntrinsicsModule:provider("AS3StringIntrinsics"),customVisitors:[],nativeGlobalModules:{Date:provider("AS3Date")},importModules:{"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
for(const body of [
 ...
 'public function f(s:String):String{return s.replace(new RegExp("a"),1);}',
 'public function f(s:String):String{return s.replace(new RegExp("a","g",1),1);}',
 'public function f(s:String,RegExp:Class):String{return s.replace(new RegExp("a","g"),1);}',
 'public function f(s:String):String{return s.replace(new Object("a","g"),1);}'
]){const source='package patterns {public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'guard-patterns',providerModule:provider('AS3GeneratedClass'),sources:{'patterns.Guard':{source,sourceSha256:hash(source)}}});
...
const names=['AS3StringIntrinsics','AS3JSON','ByteArray','AS3Date','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass','AS3CanonicalEventConstruction'];
...
const wanted=captured.filter(r=>r.id.startsWith('dynamic-'));assert.equal(wanted.length,14);
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({status,guards,emitted,results,providerGraph,
 ...
 observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},comparisonNegativeControls:3,scope:'Complete DynamicReplace subject and fourteen AIR rows. Complete StringUtil remains a separate application prerequisite; RegExp-copy/Class identity and broader pattern grammar remain held.'},null,2));
