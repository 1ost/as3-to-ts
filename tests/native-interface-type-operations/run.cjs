...
const evidence=path.join(engine,'tests/nativeFlashOracle','interface-type-operations');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-interface-type-operations');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InterfaceOperationsProbe.as'){
...
const consumerOnly=process.argv.includes('--consumer');if(consumerOnly)sources['contracts.Consumer'].referenceOnly=true;
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'interface-type-operations',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
function guardSource(body,missingHelper=false){
 const source='package contracts {public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'interface-as-guard',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources:{...sources,'contracts.Guard':{source,sourceSha256:hash(source),...(consumerOnly?{referenceOnly:true}:{})}}});
 const opts={...options,nativeGeneratedDeclarations:{plan:p,module:'./guard-domain'},nativeReferenceCoercion:{plan:p,module:'./guard-domain',coercionModule:provider('AS3Type')}};
 if(consumerOnly){for(const key of ['nativeArrayCreationModule','nativeGeneratedDeclarations','nativeClassTraitsModule','nativeTypedLocals','nativeLexicalMembersModule','nativeGeneratedPropertyModule','nativeCallableMethodBindingModule','nativeCallableCoercionModule','nativeCallableStringModule','nativeTypedLocalReferenceModule'])delete opts[key];}
 if(missingHelper)delete opts[missingHelper===true?'nativeComputedTypeTestModule':missingHelper];
 return emit(parse('Guard.as',source),source,opts);
}
for(const [body,missing,reason] of [
 ...
 ['public function run(value:*):Boolean{return value is IRoot;}',true,/explicit module binding required/],
 ['public static var value:Boolean=null is IRoot;',false,/source is during class initialization/],
 ['public function run(value:*):int{return (value as IRoot).ping(1);}','nativeDynamicPropertyReadsModule',/explicit module binding required/],
 ['public function run(value:*):int{return (value as IRoot).value;}','nativeDynamicPropertyReadsModule',/explicit module binding required/],
 ['public function run(value:*):void{(value as IRoot).value=1;}','nativeDynamicPropertyWritesModule',/explicit module binding required/],
 ['public function run(value:*):*{return (value as IRoot)["value"];} ',false,/computed interface-cast properties/],
 ['public function run(value:*):void{(value as IRoot).value++;}',false,/cannot substitute/],
 ['public function run(value:*):void{(value as IRoot).value+=1;}',false,/compound assignment held/]
...
const driverFile=path.resolve('tests/native-interface-type-operations/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,64);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='exact').value[0]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
