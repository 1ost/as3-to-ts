...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-io-catch');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-io-catch');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
for(const changed of [
 source.replaceAll('catch (error:IOError)', 'catch (error:TypeError)'),
 source.replace('finally { log.push("finally"); }', 'catch (other:*) { return other; } finally { log.push("finally"); }'),
 source.replace('select(value:*, log:Array)', 'select(value:*, log:Array, IOError:*)')
]) {
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'io-catch-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'catchcases.Catcher':{source:changed,sourceSha256:hash(changed)}}});
  emit(parse('Catcher.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
const unbound=api.createNativeGeneratedDeclarationPlan({scope:'io-catch-unbound',providerModule:provider('AS3GeneratedClass'),sources});
assert.throws(()=>emit(parse('Catcher.as',source),source,{...options,nativeGeneratedDeclarations:{plan:unbound,module:'./unbound'},nativeReferenceCoercion:{plan:unbound,module:'./unbound',coercionModule:provider('AS3Type')}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
...
const driverFile=path.resolve('tests/native-generated-io-catch/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
