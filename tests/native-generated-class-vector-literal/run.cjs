...
const api=require('../../lib'),parse=require('../../lib/parse'),emit=require('../../lib/emit');
...
const ts=modern;
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-class-vector-literal');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-class-vector-literal');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['vectorliteral.ClassValues'].source;
// Primitive specializations other than Class remain outside this qualification.
for(const type of ['int','String','*']) {
 const changed=source.replace(/<Class>/g,'<'+type+'>');
 const guardSources={...sources,'vectorliteral.ClassValues':{source:changed,sourceSha256:hash(changed)}};
 assert.throws(()=>api.createNativeGeneratedDeclarationPlan({scope:'literal-guard',providerModule:provider('AS3GeneratedClass'),vectorProviderModule:provider('AS3Vector'),sources:guardSources}),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-class-vector-literal/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,6);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='invalid-first').value[1].push('b')]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,collisionMutation:process.argv.includes('--collision'),emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Other vector literal element types','Production module factory and full application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:6,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
