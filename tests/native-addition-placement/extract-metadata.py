...
records={}
pairs=zip(raw['reflection'],raw['instances']) if 'reflection' in raw else [(raw['classXML'],raw['instanceXML'])]
for class_text,instance_text in pairs:
 ...
 source=capture/'sources/original'/Path(qname.replace('.','/')+'.as')
