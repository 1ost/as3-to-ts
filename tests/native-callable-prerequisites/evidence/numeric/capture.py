...
PLUGIN = Path('C:/Users/admin/Desktop/GITHUB REPO/bleach-services/game-client/electron_desktop/plugins/pepflashplayer.dll')
assert PLUGIN.is_file()
shutil.copyfile(Path('C:/Users/admin/Desktop/GITHUB REPO/as3-to-ts-op2/tests/native-class-initializers/oracle/capture.cjs'), OUTPUT / 'capture.cjs')
commands = [
    ['java', '-Xmx384m', '-jar', str(FLEX / 'lib/mxmlc.jar'), '+flexlib=' + str(FLEX / 'frameworks'),
     '-target-player=26.0', '-swf-version=37', '-source-path=' + str(HERE / 'oracle'),
     '-output=' + str(OUTPUT / 'oracle.swf'), str(HERE / 'oracle/NumberOracle.as')],
...
files = sorted((HERE / 'oracle').rglob('*.as')) + [Path(__file__), OUTPUT / 'capture.cjs', OUTPUT / 'oracle.swf', OUTPUT / 'flash.json',
