...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-object-forin');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-object-forin');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['objectloops.Enumeration'].source;
...
for(const changed of [
 source.replace('var key:Object=null','var key:int=0'),
 source.replace('for(key in value)','for(value.key in value)'),
 source.replace('for(key in value)','for(var inline:String in value)'),
 source.replace('for(key in value){out.push([typeof key,key]);}','try{throw null;}catch(key:*){for(key in value){out.push(key);}}')
]){
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'object-forin-guard',providerModule:provider('AS3GeneratedClass'),sources:{'objectloops.Enumeration':{source:changed,sourceSha256:hash(changed)}}});
...
const driverFile=path.resolve('tests/native-generated-object-forin/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,25);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='dictionary-2').value[0][1]=null]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Other typed, inline, member and catch-shadow enumeration targets','Complete Zip/ZipFile and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:25,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
