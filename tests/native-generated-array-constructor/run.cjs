...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-array-constructor');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-array-constructor');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Required','Optional'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/entry',name+'.as'),'utf8');return ['entry.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-array-constructor',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
for(const value of ['[]','undefined','1']){
 const source=sources['entry.Optional'].source.replace('=null','='+value),changed={...sources,'entry.Optional':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'array-guard',providerModule:provider('AS3GeneratedClass'),sources:changed});
 assert.throws(()=>emit(parse('Optional.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},...(combined?{nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}:{})}),/Array constructor default requires literal null/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-array-constructor/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,11);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='required-undefined').value=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
