...
const rows=[];for(const q of ['probe.ForeignLocals','probe.QualifiedLocals','probe.SamePackage','cycle.A','cycle.B','init.DeferredHolder','foreign.Child']){
 ...
 assert.match(error,q==='foreign.Child'?/Object-root/:/AS3_DECLARATION_DOMAIN_UNSUPPORTED: (unresolved or ambiguous source annotation|unknown qualified source annotation)/);rows.push({qname:q,completeSource:true,sourceSha256:crypto.createHash('sha256').update(sources[q]).digest('hex'),error});
}
const cache=path.join(compiler,'.cache/native-declaration-domain');fs.mkdirSync(cache,{recursive:true});fs.writeFileSync(path.join(cache,'holds.json'),JSON.stringify({sourceRows:captures.rows.length,heldCompleteClasses:rows,initializationRows:'These single-class maps omit required foreign declaration targets and remain held. The full original domain remains held by ancestry; native-foreign-typed-locals retains that complete group and separately captures its supported Object-root domain.'},null,2));console.log(JSON.stringify({heldClasses:rows.length}));
