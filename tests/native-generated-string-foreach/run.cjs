...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-string-foreach');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-string-foreach');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['stringeach.Enumeration'].source;
...
for(const changed of [
 source.replace('var text:String="seed"','var text:int=0'),
 source.replace('for each(text in input)','for each(input.text in input)'),
 source.replace('for each(text in input)','for each(var inline:String in input)'),
 source.replace('for each(text in input){result.push([typeof text,text]);}','try{throw null;}catch(text:*){for each(text in input){result.push(text);}}')
]){
 assert.notEqual(changed,source,'guard must mutate its source');
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'string-foreach-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'stringeach.Enumeration':{source:changed,sourceSha256:hash(changed)}}});
...
const driverFile=path.resolve('tests/native-generated-string-foreach/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
