...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-dynamic-mutations');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-dynamic-mutations');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const sources=Object.fromEntries(['Mutator','Slot'].map(name=>{const source=fs.readFileSync(path.join(evidence,'source/mutation',name+'.as'),'utf8');return ['mutation.'+name,{source,sourceSha256:hash(source)}];}));
const nativeProviders={};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-dynamic-mutations',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['mutation.Mutator'],sources});
...
for(const expression of ['target[key]()','(target[key])()','target[key]-=1','target[key]++','++target[key]','target[key]*=1']){
 const source=sources['mutation.Mutator'].source.replace('return target[key]+=value;','return '+expression+';'),changed={...sources,'mutation.Mutator':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'read-guard',providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['mutation.Mutator'],sources:changed});
 assert.throws(()=>emit(parse('Mutator.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}),/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
for(const module of ['', 'bad"module']){const source=sources['mutation.Mutator'].source;assert.throws(()=>emit(parse('Mutator.as',source),source,{...options,nativeDynamicPropertyWritesModule:module}),/explicit common AS3Property module/);rejectionGuards++;}
for(const expression of ['target[key]+=value','delete target[key]']){
 const source=sources['mutation.Mutator'].source.replace('return target[key]+=value;','return '+expression+';');
 const changed={...sources,'mutation.Mutator':{source,sourceSha256:hash(source)}};
 const p=api.createNativeGeneratedDeclarationPlan({scope:'write-provider-guard',providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:['mutation.Mutator'],sources:changed});
 assert.throws(()=>emit(parse('Mutator.as',source),source,{...options,nativeDynamicPropertyWritesModule:undefined,nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:p,module:'./guard'}}),/AS3_DYNAMIC_PROPERTY_UNSUPPORTED/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-dynamic-mutations/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,19);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='add-number').value[0]=999]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{file:driverFile,sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Computed receiver expressions, direct dynamic calls and other compound updates','Complete ObjectUtil runtime parity']},null,2));console.log(JSON.stringify({run,sourceClasses:2,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
