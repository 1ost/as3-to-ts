...
const evidence=path.join(engine,'tests/nativeFlashOracle/lexical-name-collision'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-name-collision');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['methodintr.Subject','methodintr.Target'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   const input={scope:'class-script-retry-'+cohort,sources,providers:nativeProviders,vectorProviderModule:provider('AS3Vector'),patternProviderModule:provider('AS3StringIntrinsics'),providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['methodintr.Subject','methodintr.Target'],lexicalProviderModule:provider('AS3LexicalMembers')};
   ...
   const inaccessible=sources['methodintr.Target'].source.replace('public static function join','private static function join');assert.notEqual(inaccessible,sources['methodintr.Target'].source);
   const privatePlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,'methodintr.Target':{source:inaccessible,sourceSha256:hash(inaccessible)}}});
   assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:privatePlan,emitterOptions:{...options,nativeVectorTypes:{...options.nativeVectorTypes,plan:privatePlan},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:privatePlan}}}),/AS3_GENERATED_LEXICAL_UNSUPPORTED/);rejectionGuards++;
  ...
  const mutation=originalFactory.replaceAll('\"|\"','\"~\"');assert.notEqual(mutation,originalFactory);
