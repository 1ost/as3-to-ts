...
const evidence=path.resolve('../op2-html5/game-client-laya/tests/game-config-generated'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
const cache=path.resolve('.cache/native-generated-game-config');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['cn.kyiax.game.config.GameConfig']),child:read('source',['cn.kyiax.game.config.GameConfig'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const input={scope:'game-config-'+cohort,sources,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true};
   const compilerGuards=require('./guards.cjs')(api,input);assert.equal(compilerGuards,14);
  ...
  const build=mutation=>esbuild.build({entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'},plugins:mutation?[{name:mutation,setup(b){b.onLoad({filter:/parent-factory\.js$/},args=>{
   let contents=fs.readFileSync(args.path,'utf8');const pattern=/defineAS3GeneratedStaticConstant\(GameConfig, "MAX_WIDTH", "int", \([A-Za-z0-9_]+\.callableClassIntrinsics\.number\(1440\) \| 0\)\);/g;
   assert.equal([...contents.matchAll(pattern)].length,1);contents=contents.replace(pattern,match=>match.replace('number(1440)','number(1441)'));return {contents,loader:'js'};
  });}}]:[]});
  ...
  const node=await execute(code);assert.deepEqual(node.rows,expected);assert.equal(node.checks.length,9);
 ...
 console.log(JSON.stringify({out,status:'passed',observations:expected.length,sourceConstants:1378,targets:2,realms:2}));
}
