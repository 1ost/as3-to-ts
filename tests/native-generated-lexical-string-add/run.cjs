...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-lexical-string-add');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-lexical-string-add');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const source=sources['addcases.StringStore'].source;
for(const changed of [
 source.replace('private static var text:String','private static var text:*'),
 source.replace('private static var text:String','private static var text:Number'),
 source.replace('text+=','text-='),
 source.replace('private static var text:String;','private static const text:String="constant";')
]){
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'string-add-guard',providerModule:provider('AS3GeneratedClass'),sources:{'addcases.StringStore':{source:changed,sourceSha256:hash(changed)}}});
  emit(parse('StringStore.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
 },/AS3_[A-Z_]+UNSUPPORTED/);rejectionGuards++;
}
assert.throws(()=>emit(parse('StringStore.as',source),source,{...options,nativeTypedLocalAdditionModule:undefined}),/explicit common coercion, String and addition modules required/);rejectionGuards++;
...
const driverFile=path.resolve('tests/native-generated-lexical-string-add/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,20);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='scalar-0-2').value[0]='1']){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,collisionMutation:process.argv.includes('--collision'),emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Non-String/accessor/other compound assignments','Production module factory and full application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:20,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
