...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-wildcard-foreach');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-wildcard-foreach');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const options={customVisitors:[],importModules:{"flash.utils.getQualifiedClassName":provider("getQualifiedClassName"),"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
const source=sources['wildloops.Enumeration'].source;
for(const key of ['nativeEnumeration','nativeDictionaryPropertyModule']){
 assert.throws(()=>emit(parse('Enumeration.as',source),source,{...options,[key]:undefined}),/AS3_[A-Z_]+UNSUPPORTED/,key);rejectionGuards++;
}
for(const body of [
 ...
 'for each(missing in value){}'
]){
 const changed='package wildloops {public class Guard {public function test(value:*):void {'+body+'}}}';
 const guard=api.createNativeGeneratedDeclarationPlan({scope:'wildcard-foreach-guard',providerModule:provider('AS3GeneratedClass'),sources:{'wildloops.Guard':{source:changed,sourceSha256:hash(changed)}}});
 assert.throws(()=>emit(parse('Guard.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}}),/AS3_[A-Z_]+UNSUPPORTED/,body);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-wildcard-foreach/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['Other typed, inline, member and catch-shadow enumeration targets','Complete Debug and Game integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:wanted.length,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
