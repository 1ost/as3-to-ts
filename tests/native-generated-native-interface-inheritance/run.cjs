...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-native-interface-inheritance'),air=require(path.join(evidence,'verify.cjs'));
const expected=air.filter(row=>row.id!=="native-contract");
...
const cache=path.resolve('.cache/native-generated-native-interface-inheritance');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source',['nativecontract.IParser','nativecontract.IChild','nativecontract.Parser','nativecontract.Derived','nativecontract.Consumer','nativecontract.Standalone'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const nativeProviders={'flash.events.EventDispatcher':{module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'EventDispatcher',nativeBase:'EventDispatcher'},'flash.events.IEventDispatcher':{module:modulePath(path.join(engine,'src/layaAir/flash/events/IEventDispatcher.ts')),exportName:'IEventDispatcher',nativeInterface:true},'flash.events.Event':{module:provider('AS3CanonicalEventConstruction'),exportName:'Event',nativeBase:'Event'}};
   ...
   const nativeMembers=plan.interfaceContracts.members.filter(m=>m.owner==='flash.events.IEventDispatcher');
   const signatures=nativeMembers.map(m=>'method:'+m.name+':'+m.returnType.replace(/\.([^.]*)$/,'::$1')+':'+m.parameters.map(p=>p.type.replace(/\.([^.]*)$/,'::$1')+':'+p.optional).join(',')).sort();
   assert.deepEqual(signatures,air.find(row=>row.id==='native-contract').value);
   assert(plan.interfaceContracts.implementations.some(i=>i.owner==='nativecontract.Derived'&&i.implementationOwner==='flash.events.EventDispatcher'));
   const reject=change=>{assert.throws(()=>api.createNativeGeneratedDeclarationPlan({...input,...change}),/AS3_GENERATED_(DECLARATIONS|INTERFACE_CONTRACT)_UNSUPPORTED/);rejectionGuards++;};
   reject({providers:{...nativeProviders,'flash.events.IEventDispatcher':{module:nativeProviders['flash.events.IEventDispatcher'].module,exportName:'IEventDispatcher'}}});
   const withoutEvent={...nativeProviders};delete withoutEvent['flash.events.Event'];reject({providers:withoutEvent});
   const changeSource=(q,transform)=>{const source=transform(sources[q].source);reject({sources:{...sources,[q]:{source,sourceSha256:hash(source)}}});};
   changeSource('nativecontract.IParser',s=>s.replace('function get key():String;','function get key():String; function hasEventListener(type:String):String;'));
   changeSource('nativecontract.Parser',s=>s.replace('extends EventDispatcher',''));
   changeSource('nativecontract.Parser',s=>s.replace('public function Parser()', 'override public function hasEventListener(type:int):Boolean{return false;} public function Parser()'));
   ...
   const program=ts.createProgram(files,{target:ts.ScriptTarget.ES2020,module:ts.ModuleKind.CommonJS,strict:true,strictNullChecks:false,useUnknownInCatchVariables:false,experimentalDecorators:true,noEmit:true,skipLibCheck:true,types:[],lib:['lib.es2020.d.ts','lib.dom.d.ts','lib.dom.iterable.d.ts']});
  ...
  const mutation=originalFactory.replace(/(defineAS3Interface(?:<[^>]+>)?\)?\("nativecontract::IParser",\s*)\[[^\]]+\]/,'$1[]');assert.notEqual(mutation,originalFactory);
  try{fs.writeFileSync(factoryFile,mutation);const changed=await build(),actual=await execute(changed.outputFiles[0].text);assert.notDeepEqual(actual.rows,expected);}finally{fs.writeFileSync(factoryFile,originalFactory);}
  const hostGetter=originalFactory.replace(/(?:[A-Za-z_$][\w$]*\.as3GetProperty|\(0,\s*[A-Za-z_$][\w$]*\.as3GetProperty\))\(value, "key"\)/g,'value.key');assert.notEqual(hostGetter,originalFactory);
  try{fs.writeFileSync(factoryFile,hostGetter);const changed=await build();await assert.rejects(()=>execute(changed.outputFiles[0].text),/Unsupported source property representation/);}finally{fs.writeFileSync(factoryFile,originalFactory);}
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),held:['ResourceLoader complete dependency integration','Full startup and game account flow']},null,2));
