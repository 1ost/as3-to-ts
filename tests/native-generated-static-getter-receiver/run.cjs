...
const evidence=path.join(engine,'tests/nativeFlashOracle/static-getter-receiver'),expected=require(path.join(evidence,'verify.cjs'));
const cache=path.resolve('.cache/native-generated-static-getter-receiver');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={subject:read('source',['StaticGetterReceiverProbe','receivercases.Caller','receivercases.Factory','receivercases.Target','receivercases.Child'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const input={scope:'static-getter-receiver-'+cohort,sources,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},inheritScriptClasses:true,classScriptSources:['receivercases.Factory']};
   ...
   assert.equal(require('./guards.cjs')(api,input,config),7);
  ...
  results.push({target,web,node,typechecks,artifacts,rejectionGuards:7});
 ...
 console.log(JSON.stringify({out,status:'passed',observations:13,targets:2}));
}
