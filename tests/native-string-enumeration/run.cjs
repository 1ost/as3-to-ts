...
const engine=path.resolve('../LayaAir-op2'),evidence=path.join(engine,'tests/nativeFlashOracle/string-enumeration');
...
const base=path.resolve('.cache/native-string-enumeration');fs.mkdirSync(base,{recursive:true});const run=fs.mkdtempSync(path.join(base,'run-'));
...
const source=fs.readFileSync(path.join(evidence,'source/StringEnumerationProbe.as'),'utf8');
const sources={StringEnumerationProbe:{source,sourceSha256:hash(source),referenceOnly:true}};
...
const options={nativeStringLocalCoercionModule:provider('AS3String'),importModules:{'flash.utils.Dictionary':provider('Dictionary')},nativeDictionaryPropertyModule:provider('AS3Property'),nativeEnumeration:{dictionaryModule:provider('Dictionary'),coercionModule:provider('AS3Coercion'),stringModule:provider('AS3String')},customVisitors:[],definitionsByNamespace:{'':['StringEnumerationProbe']},decoratorModules:{bound:helpers.bound,classBound:helpers.classBound},nativeClassHelperModules:{nativeClass:helpers.nativeClass,callableClass:helpers.callableClass},nativeGlobalModules:{Date:provider('AS3Date')},nativeComputedTypeTestModule:provider('AS3Type'),nativeNumericMethodParametersModule:provider('AS3Coercion'),nativeSourceErrorModule:modulePath(path.join(engine,'src/layaAir/flash/errors/AS3SourceError.ts')),nativeReferenceCoercion:{plan,module:'./domain',coercionModule:provider('AS3Type')}};
fs.writeFileSync(path.join(run,'domain.ts'),plan.moduleSource);
const output=emit(parse('StringEnumerationProbe.as',source),source,options);
fs.writeFileSync(path.join(run,'StringEnumerationProbe.ts'),output);
const emitted=[{name:'StringEnumerationProbe',sourceSha256:hash(source),generatedSha256:hash(output)}];
let guards=0;
fs.writeFileSync(path.join(run,'driver.ts'),"import {StringEnumerationProbe} from './StringEnumerationProbe';\nexport function run(){return JSON.parse(JSON.stringify(new StringEnumerationProbe().snapshot().observations));}\n");
...
async function main(){
 ...
 console.log('String enumeration: '+guards+' guards, 13 AIR observations from unchanged source, Node/Chromium ES5/ES2015. '+run);
}
