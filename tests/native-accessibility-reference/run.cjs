...
const evidence=path.join(engine,'tests/nativeFlashOracle','accessibility-reference');const expected=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-accessibility-reference');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const text=fs.readFileSync(path.join(evidence,'source/AccessibilityHolder.as'),'utf8');const sources={'AccessibilityHolder':{source:text,sourceSha256:hash(text)}};
const source=sources['AccessibilityHolder'].source;
const applicationDomain=provider('AS3CanonicalAccessibilityReference');
const nativeProviders={'flash.accessibility.AccessibilityImplementation':{module:applicationDomain,exportName:'AccessibilityImplementation'}};
const input={scope:'generated-accessibility-reference',inheritScriptClasses:true,providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},sources};
...
Object.assign(options,{nativeAccessibilityReferenceModule:applicationDomain,nativeReferenceCoercion:{plan,module:"./declarationDomain",coercionModule:provider("AS3Type")},nativeSignaturePropertyModule:provider("AS3Property"),nativeDynamicConstructionModule:provider('AS3Invocation')});
...
let bindingGuards=0;
for(const patch of [
 {nativeAccessibilityReferenceModule:undefined},
 {nativeAccessibilityReferenceModule:'./wrong'},
 {nativeReferenceCoercion:undefined},
 {nativeGeneratedDeclarations:undefined},
 {importModules:{...options.importModules,'flash.accessibility.AccessibilityImplementation':'./wrong'}},
]){
 assert.throws(()=>emit(parse('AccessibilityHolder.as',text),text,{...options,...patch}),/AS3_[A-Z_]+UNSUPPORTED/,Object.keys(patch).join(','));bindingGuards++;
}
const altered=source=>{
 const changed=api.createNativeGeneratedDeclarationPlan({...input,sources:{AccessibilityHolder:{source,sourceSha256:hash(source)}}});
...
assert.throws(()=>altered(text.replace('inspect(input:*)','inspect(input:*,AccessibilityImplementation:Class)')),/AS3_[A-Z_]+UNSUPPORTED/);bindingGuards++;
assert.throws(()=>altered(text.replace('public class AccessibilityHolder {','public class AccessibilityHolder extends AccessibilityImplementation {')),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);bindingGuards++;
(async()=>{
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const typeOptions={target:modern.ScriptTarget.ES2020,module:modern.ModuleKind.CommonJS,strict:true,strictNullChecks:false,useUnknownInCatchVariables:false,experimentalDecorators:true,noEmit:true,skipLibCheck:true,types:[],lib:['lib.es2020.d.ts','lib.dom.d.ts']};
  ...
  await page.route('http://accessibility-reference.test/**',route=>route.request().url().endsWith('/bundle.js')?route.fulfill({contentType:'text/javascript',body:code}):route.fulfill({contentType:'text/html',headers:{'Content-Security-Policy':"script-src 'self'"},body:'<script src="/bundle.js"></script>'}));
  await page.goto('http://accessibility-reference.test/');const web=await page.evaluate(async()=>{await globalThis.done;return globalThis.result;});await page.close();
  assert.deepEqual(errors,[]);assert.deepEqual(node.rows,expected);assert.deepEqual(web.rows,expected);assert.equal(node.guards,12);assert.equal(web.guards,12);
 ...
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({scope:'complete generated AccessibilityHolder source, native reference binding; no generated native subclass or full Sprite admission',sourceSha256:hash(text),expected,results,bindingGuards},null,2));
 console.log(JSON.stringify({run,targets:results.map(r=>r.target),rowsPerRuntime:expected.length,guardsPerRuntime:12,bindingGuards,typeErrors:0}));
