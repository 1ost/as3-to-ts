...
commands=[['java','-Xmx384m','-jar',str(flex/'lib/mxmlc.jar'),'+flexlib='+str(flex/'frameworks'),'-target-player=26.0','-swf-version=37','-source-path='+str(here/'original'),'-output='+str(out/'oracle.swf'),str(here/'original/ClassConversionOracle.as')],
...
inputs={str(p.relative_to(here)):p for p in [*sorted((here/'original').rglob('*.as')),here/'capture.cjs',Path(__file__)]}
