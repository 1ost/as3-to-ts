package {import flash.display.Sprite;import flash.utils.describeType;import flash.external.ExternalInterface;import probe.*;import com.greensock.core.PropTween;
...
var reflections:Array=[];var instances:Array=[];for each(var t:Class in [Journal,StringPair,OptionalString,SelfNode,SelfChild,PropTween]){reflections.push(describeType(t).toXMLString());var obj:*=t===StringPair?new t('x'):t===PropTween?new t({},'x',0,1,'x',false):new t();instances.push(describeType(obj).toXMLString());}
ExternalInterface.addCallback('snapshot',function():String{return encodeURIComponent(JSON.stringify({rows:rows,reflection:reflections,instances:instances}));});
}}}
