...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-external-interface');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-external-interface');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='ExternalInterfaceProbe.as'){
...
const nativeProviders={'flash.external.ExternalInterface':{module:modulePath(path.join(engine,'src/layaAir/flash/external/ExternalInterface.ts')),exportName:'ExternalInterface'}};
const plan=api.createNativeGeneratedDeclarationPlan({scope:'generated-external-interface',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptGlobalSources:[],sources});
...
const original=sources['hostcases.HostConsumer'].source;
assert.throws(()=>emit(parse('HostConsumer.as',original),original,{...options,nativeGeneratedDeclarations:{plan:unbound,module:'./unbound'},nativeReferenceCoercion:{plan:unbound,module:'./unbound',coercionModule:provider('AS3Type')}}),/unresolved class-value identity: ExternalInterface/);rejectionGuards++;
...
const names=['ExternalInterface','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-external-interface/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,7);
