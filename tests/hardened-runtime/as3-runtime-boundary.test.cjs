...
test("real CLI authority package installs, seals, bundles, and exposes no registrar", () => {
    ...
    try {
        ...
        assert.equal(manifest.runtimeAuthorityQNames.length, 29);
        ...
        write(timerConsumer, `"use strict";const assert=require('node:assert/strict');const fs=require('node:fs');const path=require('node:path');const runtime=require.resolve('@bleach/as3-runtime/AS3Type');require(runtime);const afterAuthority=performance.now();while(performance.now()-afterAuthority<5){}const timerPath=require.resolve('@bleach/as3-runtime/AS3Timer');assert.notEqual(timerPath,runtime);assert.equal(timerPath,path.join(path.dirname(runtime),'AS3Timer.js'));for(const extension of ['js','ts'])assert.equal(fs.existsSync(path.join(path.dirname(runtime),'internal','AS3TimerRuntime.'+extension)),false);assert.throws(()=>require('@bleach/as3-runtime/internal/AS3TimerRuntime'),error=>error.code==='ERR_PACKAGE_PATH_NOT_EXPORTED');const timer=require('@bleach/as3-runtime/AS3Timer');const absolute=require(timerPath);assert.equal(absolute,timer);assert.deepEqual(Object.keys(timer),['clearInterval','clearTimeout','getTimer','setInterval','setTimeout']);assert.equal(Object.getPrototypeOf(timer),null);assert.equal(Object.isFrozen(timer),true);assert.equal(timer.AS3TimerRuntime,undefined);assert.equal(timer.getTimer()>=5,true,'timer epoch must be captured with authority startup before the later facade import');for(const key of Object.keys(timer)){const descriptor=Object.getOwnPropertyDescriptor(timer,key);assert.equal(typeof descriptor.value,'function');assert.equal(descriptor.writable,false);assert.equal(descriptor.configurable,false);assert.equal(Reflect.set(timer,key,()=>{throw new Error('intercepted')}),false);assert.throws(()=>Object.defineProperty(timer,key,{value(){throw new Error('intercepted')}}),TypeError);}const id=timer.setTimeout(()=>{throw new Error('cancel failed')},1000);assert.equal(Number.isInteger(id)&&id>0&&id<=0xffffffff,true);timer.clearInterval(id);timer.clearTimeout(id);const interval=timer.setInterval(()=>{throw new Error('cross cancel failed')},1000);timer.clearTimeout(interval);timer.clearInterval(interval);\n`);
        childProcess.execFileSync(process.execPath, [timerConsumer], { cwd: consumerRoot, stdio: "inherit" });
        const rejectedClockConsumer = path.join(consumerRoot, "rejected-clock-consumer.cjs");
        write(rejectedClockConsumer, `"use strict";const assert=require('node:assert/strict');const scenario=process.argv[2];let clockReads=0;if(scenario==='missing')Object.defineProperty(globalThis,'performance',{configurable:true,value:undefined});else if(scenario==='throwing')Object.defineProperty(globalThis,'performance',{configurable:true,get(){clockReads++;throw new Error('clock trap')}});else{const reading=scenario==='nan'?NaN:scenario==='positive-infinity'?Infinity:-Infinity;Object.defineProperty(globalThis,'performance',{configurable:true,value:{now(){clockReads++;return reading}}});}const scheduled=[];const cancelled=[];globalThis.setTimeout=(callback,delay)=>{const handle={kind:'timeout',sequence:scheduled.length+1};scheduled.push({callback,delay,handle});return handle};globalThis.clearTimeout=handle=>cancelled.push(handle);globalThis.setInterval=(callback,delay)=>{const handle={kind:'interval',sequence:scheduled.length+1};scheduled.push({callback,delay,handle});return handle};globalThis.clearInterval=handle=>cancelled.push(handle);const timer=require('@bleach/as3-runtime/AS3Timer');assert.deepEqual(Object.keys(timer),['clearInterval','clearTimeout','getTimer','setInterval','setTimeout']);assert.equal(Object.getPrototypeOf(timer),null);assert.equal(Object.isFrozen(timer),true);const timeout=timer.setTimeout(()=>{},1);const interval=timer.setInterval(()=>{},2);assert.deepEqual([timeout,interval],[1,2]);timer.clearInterval(timeout);timer.clearTimeout(interval);assert.deepEqual(cancelled.map(handle=>handle.kind),['timeout','interval']);assert.throws(()=>timer.getTimer(),/captured epoch and monotonic clock/);assert.equal(clockReads,scenario==='missing'?0:1);\n`);
        for (const scenario of ["missing", "throwing", "nan", "positive-infinity", "negative-infinity"]) {
            childProcess.execFileSync(process.execPath, [rejectedClockConsumer, scenario],
                { cwd: consumerRoot, stdio: "inherit" });
        }
