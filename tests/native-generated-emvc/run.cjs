...
const evidence=path.resolve('../op2-html5/game-client-laya/tests/emvc-generated'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-emvc');fs.mkdirSync(cache,{recursive:true});
...
const provenance=require(path.join(evidence,'source-provenance.json'));const cohorts={parent:read('source',[...provenance.files.map(f=>f.qname),'emvcprobe.ViewAccess'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const nativeProviders={'flash.utils.Dictionary':{module:provider('Dictionary'),exportName:'Dictionary'},'flash.events.Event':{module:provider('AS3CanonicalEventConstruction'),exportName:'Event',nativeBase:'Event'},'flash.events.EventDispatcher':{module:modulePath(path.join(engine,'src/layaAir/flash/events/EventDispatcher.ts')),exportName:'EventDispatcher'},'flash.geom.Rectangle':{module:modulePath(path.join(engine,'src/layaAir/flash/geom/Rectangle.ts')),exportName:'Rectangle'},'flash.display.DisplayObject':{module:provider('AS3CanonicalDisplayReference'),exportName:'DisplayObject'},Error:{module:provider('AS3CanonicalErrorConstruction'),exportName:'Error',nativeBase:'Error'},XML:{module:provider('AS3CanonicalXMLReference'),exportName:'XML'},XMLList:{module:provider('AS3CanonicalXMLReference'),exportName:'XMLList'}};
  ...
  const build=mutation=>esbuild.build({plugins:mutation?[{name:'wrong-protected-initializer',setup(b){b.onLoad({filter:/parent-factory\.js$/},args=>{const source=fs.readFileSync(args.path,'utf8'),needle='"Mediator name registered."';assert.equal(source.split(needle).length-1,1);return {contents:source.replace(needle,'"wrong:"'),loader:'js'};});}}]:[],entryPoints:[entry],bundle:true,write:false,format:'iife',platform:'browser',target:'es2020',metafile:true,loader:{'.glsl':'text','.vs':'text','.fs':'text','.wgsl':'text'}});
  ...
  const changed=(await build(true)).outputFiles[0].text;
  await assert.rejects(()=>execute(changed),/sibling protected storage isolated/);
  results.push({target,node,web,guards:12,mutations:['wrong protected String initializer'],typechecks,artifacts,inputs:Object.keys(built.metafile.inputs).map(file=>({file,sha256:hash(fs.readFileSync(file))}))});
 ...
 fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({results,cohorts,compilerInputs,runnerSha256:hash(fs.readFileSync(__filename)),observerSha256:hash(fs.readFileSync(path.join(__dirname,'observer.ts'))),guardsSha256:hash(fs.readFileSync(path.join(__dirname,'guards.cjs'))),held:['CfgItem/ContextUtil/DataStoreProxy integration','Document Sprite adapter','Full startup and game account flow']},null,2));
