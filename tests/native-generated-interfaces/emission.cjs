...
for(const body of [
 ...
 'public function Guard(v:IRoot) {super();}',
...
async function main(){
 ...
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,guards,emitted,results,typecheck:{files:program.getSourceFiles().length,diagnostics},comparisonNegativeControls:5,held:['Three interface-reflection AIR rows','Interface constructors and reference locals','Ordinary consumer interface coercion','Static lexical variables, accessors and protected methods','Optional/rest methods and constructors, typed exception-return regions']},null,2));
