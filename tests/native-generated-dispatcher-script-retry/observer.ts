...
export async function run(module){
 ...
 const failedClass=item('classes',0),failedInstance=as3ConstructClass(failedClass),hasReadyListener=value=>as3CallValue(get(value,'hasEventListener'),()=>['ready']);
 row('failed-instance',[as3Is(failedInstance,EventDispatcher),as3Is(failedInstance,IEventDispatcher),hasReadyListener(failedInstance)]);
 ...
 row('successful-instance',[as3Is(instance,EventDispatcher),as3Is(instance,IEventDispatcher),as3Is(instance,selected),as3Is(failedInstance,selected),hasReadyListener(instance),hasReadyListener(failedInstance)]);
 row('parent-identity',[as3Is(failedInstance,EventDispatcher),item('classes',0)===failedClass,EventDispatcher===EventDispatcher,count()]);
 const calls=[],invoke=(object,name,args=[])=>as3CallValue(get(object,name),()=>args);
 const listener=event=>{calls.push([event.target===failedInstance,event.currentTarget===failedInstance]);event.preventDefault();};
 invoke(failedInstance,'addEventListener',['ready',listener]);
 row('listener-isolation',[hasReadyListener(failedInstance),hasReadyListener(instance)]);
 row('failed-generation-dispatch',[invoke(failedInstance,'dispatchEvent',[as3ConstructClass(FlashEvent,['ready',false,true])]),calls]);
 invoke(failedInstance,'removeEventListener',['ready',listener]);
 row('listener-removal',[hasReadyListener(failedInstance),invoke(instance,'dispatchEvent',[as3ConstructClass(FlashEvent,['ready'])]),calls.length]);
 ...
 set(otherTrace,'fail',false);const otherClass=sibling.getDefinition('retrycases.Retry'),otherInstance=as3ConstructClass(otherClass);
 const domainChecks=[otherTrace!==Trace,get(otherTrace,'globals')!==array('globals'),get(get(otherTrace,'globals'),'length')===1,child.getDefinition('retrycases.Trace')===Trace,child.getDefinition('retrycases.Retry')===selected,
 otherClass!==selected,otherInstance!==instance,as3Is(otherInstance,EventDispatcher),hasReadyListener(otherInstance)===false];
