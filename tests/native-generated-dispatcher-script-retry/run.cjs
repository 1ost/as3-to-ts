...
const evidence=path.join(engine,'tests/nativeFlashOracle/generated-dispatcher-script-retry'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-dispatcher-script-retry');fs.mkdirSync(cache,{recursive:true});
...
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   const nativeProviders={'flash.events.EventDispatcher':{module:provider('AS3CanonicalEventDispatcherConstruction'),exportName:'EventDispatcher',nativeBase:'EventDispatcher'}};
   ...
   for(const change of [
    {scriptGlobalProviderModule:undefined},
    {classScriptSources:['missing.Type']},
   ...
   const unsupported=sources['retrycases.Retry'].source.replace('import flash.events.EventDispatcher;','').replace('extends EventDispatcher','extends Error');
   assert.throws(()=>api.createNativeGeneratedDeclarationPlan({...input,providers:{Error:{module:provider('AS3CanonicalErrorConstruction'),exportName:'Error',nativeBase:'Error'}},sources:{...sources,'retrycases.Retry':{source:unsupported,sourceSha256:hash(unsupported)}}}),/non-retrying source root parent/);rejectionGuards++;
