...
const evidence=path.join(engine,'tests/nativeFlashOracle','stage-reference');const expected=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-stage-reference');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
const text=fs.readFileSync(path.join(evidence,'source/StageHolder.as'),'utf8');const sources={'StageHolder':{source:text,sourceSha256:hash(text)}};
const source=sources['StageHolder'].source;
const applicationDomain=provider('AS3CanonicalSpriteOwnerReferences');
const names=['flash.display.Stage'];
const nativeProviders=Object.fromEntries(names.map(name=>[name,{module:applicationDomain,exportName:name.split('.').pop()}]));
const input={scope:'generated-stage-reference',inheritScriptClasses:true,providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),scriptGlobalProviderModule:provider('AS3ScriptGlobal'),scriptDomainProvider:{module:'./cohortDomain',exportName:'scriptDomain'},sources};
...
for(const patch of [
 ...
 {importModules:{...options.importModules,'flash.display.Stage':'./wrong'}},
 ...names.slice(1).map(name=>({importModules:{...options.importModules,[name]:'./wrong'}})),
]){
 assert.throws(()=>emit(parse('StageHolder.as',text),text,{...options,...patch}),/AS3_[A-Z_]+UNSUPPORTED/,Object.keys(patch).join(','));bindingGuards++;
}
const altered=source=>{
 const changed=api.createNativeGeneratedDeclarationPlan({...input,sources:{StageHolder:{source,sourceSha256:hash(source)}}});
...
for(const name of names.map(name=>name.split('.').pop())){
 ...
 assert.throws(()=>altered(text.replace('public class StageHolder {','public class StageHolder extends '+name+' {')),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);bindingGuards++;
}
(async()=>{
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  await page.route('http://stage-reference.test/**',route=>route.request().url().endsWith('/bundle.js')?route.fulfill({contentType:'text/javascript',body:code}):route.fulfill({contentType:'text/html',headers:{'Content-Security-Policy':"script-src 'self'"},body:'<script src="/bundle.js"></script>'}));
  await page.goto('http://stage-reference.test/');const web=await page.evaluate(async()=>{await globalThis.done;return globalThis.result;});await page.close();
  assert.deepEqual(errors,[]);assert.deepEqual(node.rows,expected);assert.deepEqual(web.rows,expected);assert.equal(node.guards,22);assert.equal(web.guards,22);
 ...
 fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({scope:'complete generated StageHolder source, canonical Stage reference only; no Stage Class or native ancestry; no generated native subclass or full Sprite admission',sourceSha256:hash(text),expected,results,bindingGuards},null,2));
 console.log(JSON.stringify({run,targets:results.map(r=>r.target),rowsPerRuntime:expected.length,guardsPerRuntime:22,bindingGuards,typeErrors:0}));
