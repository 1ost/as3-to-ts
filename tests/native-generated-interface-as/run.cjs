...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-interface-as');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-interface-as');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='InterfaceAsProbe.as'){
...
function guardSource(body,missingHelper=false){
 ...
 const p=api.createNativeGeneratedDeclarationPlan({scope:'interface-as-guard',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),sources:{...sources,'contracts.Guard':{source,sourceSha256:hash(source)}}});
 ...
 if(missingHelper)delete opts.nativeComputedTypeTestModule;
...
const driverFile=path.resolve('tests/native-generated-interface-as/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
