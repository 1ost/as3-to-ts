...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-xml-scalars');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-xml-scalars');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name==='ScalarReader.as'){
...
const source=sources['xmlcases.ScalarReader'].source;
for(const changed of [
 source.replace('node.localName()', 'node.localName(1)'),
 source.replace('node.toString()', 'node.toString(1)'),
 source.replace('node.localName()', 'node.localName')
]){
 assert.notEqual(changed,source);
 assert.throws(()=>{
  const guard=api.createNativeGeneratedDeclarationPlan({scope:'xml-boundary-guard',providers:nativeProviders,providerModule:provider('AS3GeneratedClass'),sources:{'xmlcases.ScalarReader':{source:changed,sourceSha256:hash(changed)}}});
  emit(parse('ScalarReader.as',changed),changed,{...options,nativeGeneratedDeclarations:{plan:guard,module:'./guard'},nativeReferenceCoercion:{plan:guard,module:'./guard',coercionModule:provider('AS3Type')}});
...
const driverFile=path.resolve('tests/native-generated-xml-scalars/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
...
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({combined,emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,comparisonNegativeControls:3,held:['General XML construction, QName, filters, mutation and reflection','Complete ClientProperties and application integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:7,comparisons:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
