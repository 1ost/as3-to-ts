...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-movieclip-references');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-movieclip-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='MovieReferenceProbe.as'){
...
const consumerOnly=process.argv.includes('--consumer');if(consumerOnly)sources['moviecases.Reader'].referenceOnly=true;
...
const options={customVisitors:[],nativeMovieClipReferenceModule:provider('AS3CanonicalMovieClipReference'),importModules:{"compiler.AS3Invocation":provider("AS3Invocation"),"compiler.AS3Class":provider("AS3Class"),...Object.fromEntries(Object.keys(sources).map(q=>[q,'./'+fileFor(q)])),...Object.fromEntries(Object.entries(nativeProviders).map(([q,b])=>[q,b.module]))},definitionsByNamespace,
...
const reader=sources['moviecases.Reader'].source;
for(const patch of [
 {nativeMovieClipReferenceModule:undefined},
 {nativeReferenceCoercion:undefined},

 {nativeMovieClipReferenceModule:'./wrong-provider'},
 {importModules:{...options.importModules,'flash.display.MovieClip':'./wrong-provider'}},
...
for(const body of [
 'public static var value:Boolean=null is MovieClip;',
 'public static var value:MovieClip=null as MovieClip;',
 'public function test(value:*,MovieClip:Class):Boolean{return value is MovieClip;}',
 'public function test(value:*,MovieClip:*):*{return value as MovieClip;}'
]) {
 const source='package moviecases {import flash.display.MovieClip;public class Guard {'+body+'}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'display-reference-guard',providerModule:provider('AS3GeneratedClass'),sources:{'moviecases.Guard':{source,sourceSha256:hash(source),...(consumerOnly?{referenceOnly:true}:{})}},providers:nativeProviders});
...
for(const binding of Object.keys(sources).map(qname=>({qname}))){const source=sources[binding.qname].source,file=path.join(run,fileFor(binding.qname)+'.ts');fs.mkdirSync(path.dirname(file),{recursive:true});
 const opts=consumerOnly&&binding.qname==='moviecases.Reader'?consumerOptions(options):{...options};
...
const driverFile=path.resolve('tests/native-generated-movieclip-references/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
