...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-security');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-security');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='SecurityProbe.as'){
...
const unbound=api.createNativeGeneratedDeclarationPlan({scope:'external-unbound',providerModule:provider('AS3GeneratedClass'),sources,providers:{}});
const original=sources['hostcases.SecurityConsumer'].source;
assert.throws(()=>emit(parse('SecurityConsumer.as',original),original,{...options,nativeGeneratedDeclarations:{plan:unbound,module:'./unbound'},nativeReferenceCoercion:{plan:unbound,module:'./unbound',coercionModule:provider('AS3Type')}}),/unresolved class-value identity: Security/);rejectionGuards++;
...
const names=['Security','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
const moduleFor=n=>'src/layaAir/flash/'+(n==='Security'?'system':['AS3SourceError','IllegalOperationError'].includes(n)?'errors':'utils')+'/'+n;
...
const driverFile=path.resolve('tests/native-generated-security/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,6);
...
(async()=>{const {chromium}=require(require.resolve('playwright',{paths:[path.resolve('../op2-html5/game-client-laya'),engine]}));const browser=await chromium.launch({headless:true});const results=[];const server=require('node:http').createServer((req,res)=>{res.end('<!doctype html><title>Security HTTP host</title>');});await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const filePage=path.join(run,'host.html');fs.writeFileSync(filePage,'<!doctype html><title>Security file host</title>');const hostPages=['http://127.0.0.1:'+server.address().port+'/',require('node:url').pathToFileURL(filePage).href];
try{for(const target of [ts.ScriptTarget.ES5,ts.ScriptTarget.ES2015]){
 ...
 const web=[];for(const url of hostPages){const page=await browser.newPage();await page.goto(url);await page.addScriptTag({content:script});const actual=await page.evaluate(()=>JSON.parse(JSON.stringify(globalThis.result)));await page.close();assert.deepEqual(actual,wanted);web.push({url,actual});}
 assert.deepEqual(node,wanted);results.push({target,node,web});
}}finally{await browser.close();await new Promise(resolve=>server.close(resolve));}
fs.writeFileSync(path.join(run,'report.json'),JSON.stringify({emitted,results,providerGraph,observer:{files:[driverFile],sha256:hash(observer)},typecheck:{files:program.getSourceFiles().length,diagnostics},rejectionGuards,hostControls:2,comparisonNegativeControls:3,held:['Security full reflection and domain APIs','Complete Debug and Game integration']},null,2));console.log(JSON.stringify({run,sourceClasses:plan.bindings.length,airRows:wanted.length,rejectionGuards,targets:['ES5','ES2015'],runtimes:['Node','Chromium'],generatedTypeErrors:0,dependencyTypeErrors:diagnostics.length}));
