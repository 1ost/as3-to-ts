...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-static-singletons');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-static-singletons');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='StaticSingletonsProbe.as'){
...
for(const body of [
 ...
 'protected static function value():int{return 0;}',
 'private static var fn:Function;public static function call():void{fn();}'
]){
 const source='package singletoncases {public class Guard {'+body+'}}';
 assert.throws(()=>{
  const p=api.createNativeGeneratedDeclarationPlan({scope:'static-guard',providerModule:provider('AS3GeneratedClass'),interfaceProviderModule:provider('AS3Type'),vectorProviderModule:provider('AS3Vector'),sources:{...sources,'singletoncases.Guard':{source,sourceSha256:hash(source)}}});
...
{
 const source='package singletoncases {public class Guard extends ProtectedSingleton {}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'static-inheritance-guard',providerModule:provider('AS3GeneratedClass'),sources:{...sources,'singletoncases.Guard':{source,sourceSha256:hash(source)}}});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGeneratedDeclarations:{plan:p,module:'./guard-domain'},nativeReferenceCoercion:{plan:p,module:'./guard-domain',coercionModule:provider('AS3Type')}}),/inherited static lexical ownership/);rejectionGuards++;
}
...
const driverFile=path.resolve('tests/native-generated-static-singletons/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,32);
for(const mutate of [v=>v.pop(),v=>v.reverse(),v=>v.find(r=>r.id==='private-undefined').value[0]=false]){const bad=structuredClone(wanted);mutate(bad);assert.throws(()=>assert.deepEqual(bad,wanted));}
