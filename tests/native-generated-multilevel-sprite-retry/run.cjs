...
const evidence=path.join(engine,'tests/nativeFlashOracle/multilevel-sprite-retry');
...
const cache=path.resolve('.cache/native-generated-multilevel-sprite-retry');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('direct-source',['retrycases.Middle','retrycases.Trace','retrycases.Retry','cn.kyiax.base.impl.BaseModule','cn.kyiax.base.IBaseModule'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   for(const change of [
    ...
    {classScriptSources:['retrycases.Trace','retrycases.Retry','retrycases.Middle']},
   ...
   for(const [q,source] of [
    ['retrycases.Retry',sources['retrycases.Retry'].source.replace('import retrycases.Middle;','import flash.display.Sprite;').replace('extends Middle','extends Sprite')],
   ...
   const badProvider={...nativeProviders['flash.display.Sprite'],nativeBase:undefined};
   assert.throws(()=>api.createNativeGeneratedDeclarationPlan({...input,providers:{...nativeProviders,'flash.display.Sprite':badProvider}}),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);rejectionGuards++;
   for(const source of [sources['retrycases.Middle'].source.replace('extends BaseModule','extends Middle'),sources['cn.kyiax.base.impl.BaseModule'].source.replace('extends Sprite','')]){
    const q=source.includes('class Middle')?'retrycases.Middle':'cn.kyiax.base.impl.BaseModule';
    assert.throws(()=>api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,[q]:{source,sourceSha256:hash(source)}}}),/AS3_GENERATED_DECLARATIONS_UNSUPPORTED/);rejectionGuards++;
   }
