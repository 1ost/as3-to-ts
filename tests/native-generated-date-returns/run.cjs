...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-date-returns');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-date-returns');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='DateReturnsProbe.as'){
...
for(const [body,bound,error] of [
 ['return value;',false,/generated native return type requires separate qualification/],
 ['return;',true,/generated typed bare return/],
 ['if(value)return value;',true,/generated typed fallthrough completion/]
]) {
 const source='package {public class Guard {public function Guard(){super();} public function echo(value:*):Date {'+body+'}}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'date-return-guard',providerModule:provider('AS3GeneratedClass'),sources:{Guard:{source,sourceSha256:hash(source)}},providers:nativeProviders});
 assert.throws(()=>emit(parse('Guard.as',source),source,{...options,nativeGlobalModules:bound?options.nativeGlobalModules:{},nativeGeneratedDeclarations:{plan:p,module:'./guard'},nativeReferenceCoercion:{plan:p,module:'./guard',coercionModule:provider('AS3Type')}}),error);
 rejectionGuards++;
}
...
const names=['AS3Date','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-date-returns/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
