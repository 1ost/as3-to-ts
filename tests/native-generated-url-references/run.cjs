...
const evidence=path.join(engine,'tests/nativeFlashOracle','generated-url-references');const captured=require(path.join(evidence,'verify.cjs'));
const hash=v=>crypto.createHash('sha256').update(v).digest('hex');
const root=path.resolve('.cache/native-generated-url-references');fs.mkdirSync(root,{recursive:true});const run=fs.mkdtempSync(path.join(root,'run-'));
...
function walk(dir){for(const item of fs.readdirSync(dir,{withFileTypes:true})){const file=path.join(dir,item.name);
 if(item.isDirectory())walk(file);else if(item.name.endsWith('.as')&&item.name!=='URLReferencesProbe.as'){
...
for(const name of ['URLVariables','URLRequest'])for(const [body,bound]of [
 ['var local:'+name+'=value;return local;',false],
 ['return value is '+name+';',true],
 ['return value as '+name+';',true]
]){
 const source='package {import flash.net.'+name+';public class Guard {public static function test(value:*):*{'+body+'}}}';
 const p=api.createNativeGeneratedDeclarationPlan({scope:'url-reference-guard',providerModule:provider('AS3GeneratedClass'),sources:{Guard:{source,sourceSha256:hash(source)}},providers:bound?nativeProviders:{}});
...
const names=['AS3CanonicalURLReferences','getQualifiedClassName','AS3Vector','AS3MethodBinding','AS3Coercion','AS3String','AS3Type','AS3Property','AS3Class','AS3Invocation','AS3DeclarationType','FlashTypeMetadata','AS3LexicalMembers','AS3ArrayCreation','AS3Addition','QName','AS3DynamicObject','AS3SourceError','AS3ScriptGlobal','AS3GeneratedClass'];
...
const driverFile=path.resolve('tests/native-generated-url-references/runtime-driver.js'),observer=fs.readFileSync(driverFile,'utf8');
const wanted=captured;assert.equal(wanted.length,77);
