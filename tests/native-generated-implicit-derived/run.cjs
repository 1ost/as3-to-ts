...
const evidence=path.join(engine,'tests/nativeFlashOracle/implicit-derived-constructor'),air=require(path.join(evidence,'verify.cjs'));
const expected=air;
...
const cache=path.resolve('.cache/native-generated-implicit-derived');fs.mkdirSync(cache,{recursive:true});
...
const cohorts={parent:read('source-v2',['implicitctor.Base','implicitctor.Child','implicitctor.Grandchild','implicitctor.OptionalBase','implicitctor.OptionalChild'])};
async function main(){
 ...
 try{for(const target of ['ES5','ES2015']){
  ...
  const externalModules=[...Object.values(helpers),sourceError,...modules.map(provider)];
   ...
   {
    const q='implicitctor.OptionalBase',source=sources[q].source.replace('input:int=9','input:int');
    assert.notEqual(source,sources[q].source);
    const guardPlan=api.createNativeGeneratedDeclarationPlan({...input,sources:{...sources,[q]:{source,sourceSha256:hash(source)}}});
    assert.throws(()=>api.emitNativeSourceClassModule({...config,plan:guardPlan,emitterOptions:{...options,nativeVectorTypes:{plan:guardPlan,module:'./__native_declarations'},nativeReferenceCoercion:{...options.nativeReferenceCoercion,plan:guardPlan}}}),/synthesized derived constructor requires zero-argument source base entry/);rejectionGuards++;
   }
  ...
  const mutation=originalFactory.replace(/([A-Za-z_$][\w$]*\.apply\([^;]+,\s*this,\s*\[\]\);)/g,'');assert.notEqual(mutation,originalFactory);
  try{fs.writeFileSync(factoryFile,mutation);const changed=await build();await assert.rejects(()=>execute(changed.outputFiles[0].text));}finally{fs.writeFileSync(factoryFile,originalFactory);}
